...
|---|---|---|
| **v3196** | `(this commit)` | v3196 — Building the gate the removal door names in its own prose found that the door was wrong about itself. The ladder fork set has fifty one members and the owned list is one of them while the removal journal was in neither, so a removal made on the main profile could be restored into the ladder as finds he never made. The journal now forks with the store it describes. |
