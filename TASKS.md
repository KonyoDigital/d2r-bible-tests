...
|---|---|---|
| **v3213** | `(this commit)` | v3213 — HEART: chronicle_apply board_mask and the new rw_restore door all hop into the board frame by SHADOWING window as a parameter - v3209 assigned the global window which is a silent no-op so the write door reached nothing and refused every restore. ledger_restore.proposal_from shipped wouldAdd as a dict while bible.html calls forEach on it so the restore door had never applied anything. Measured on his live board: foundLog 363 to 445, setPieces 83 to 133, chronFound 280 to 309, runewordsMade 0 to 99. |
