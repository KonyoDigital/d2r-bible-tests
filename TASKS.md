...
|---|---|---|
| **v3063** | `(this commit)` | v3063 — the cross-family eye found two real defects in the shipped v3058 - the stamp was hashed from a separate read so a save mid-request could stamp bytes that were never served, and mixing the version into the signature made a version-only bump report a current window as stale |
