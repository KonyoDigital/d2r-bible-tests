...
|---|---|---|
| **v3251** | `(this commit)` | v3251 — filed was the size of the assignment map rather than the owned names it files, so orphan rows counted and an empty locker value put one name in both totals. an unreadable set pieces or assign store collapsed into a confident none of them. a list of objects crashed the door. and the sentence claimed to be the dock count while being a wider population by three names |
