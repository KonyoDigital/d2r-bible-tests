...
|---|---|---|
| **v3070** | `(this commit)` | v3070 — v3068 closed the sentence for an unreadable river and left the arithmetic - a failed snapshot carries empty rules so stillNeverFired emptied and fired became the entire target set, and a caller keying on the fields rather than the sentence saw the terminus that would justify deleting footage |
