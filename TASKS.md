...
|---|---|---|
| **v3060** | `(this commit)` | v3060 — split_sessions cut a new session whenever the id differed from the preceding row, so two reels recording at once shattered into fragments each carrying a share of the frames - he opened the fragment saying 0 and the player had nothing to play; a reel is now all its rows, and the dossier prints the witness that matches disk |
