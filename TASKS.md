...
|---|---|---|
| **v3237** | `(this commit)` | v3237 — measure collapsed an unreadable console page and an unreadable watcher into the same signal, so the refusal accused control_ui.html whenever a watcher was blind even though the page was fine. a watcher only failure now withholds the coverage figure and keeps the page fact true. my own law never went through measure so it could not see the proxy |
