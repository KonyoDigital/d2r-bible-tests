...
|---|---|---|
| **v3195** | `(this commit)` | v3195 — The shelf gains a filter for where a reel is in the river rather than only what it contains or when it happened, and the header stops implying a deletion it does not perform. Measured the gap between the keep eight design and the twenty reels on disk, and the design is not missing: it is working, while the test suite pins eight reels holding most of the space. |
