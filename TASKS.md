...
|---|---|---|
| **v3247** | `(this commit)` | v3247 — the board now asks the console which names the ledger can prove and shows a count beside each locker, marking rather than filtering because the fourteen that clear the bar are potions and charms. on the public site there is no console so the chip is absent rather than zero |
