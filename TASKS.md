...
|---|---|---|
| **v3113** | `(this commit)` | v3113 — five measured things: the writer split into journal_drain because the planner own law forbids any write in its module and I had broken it for several versions; the amnesty release no longer wears the proof label on the line he reads before approving a deletion; the river loader can no longer race itself; stash takes the headline cell on session cards with its source and its open-defect caveat; and the activity chart gets a sqrt scale at 108px because linear against a 62-run spike put 1 and 5 runs on the same floored stub. |
