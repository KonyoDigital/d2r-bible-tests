...
|---|---|---|
| **v3186** | `(this commit)` | v3186 — A real Grok actor for the guest seat on the box and the Mac-side puller that feeds it, which the brief said to keep and improve and which did not exist anywhere. The guest vault rendered every locker empty because there was no mirror at all, not because the vault was broken. The rewrite is a redaction rather than a relabel: this repo is public and the first run proved the board HTML still carried four of his machine identifiers, so the run refuses to copy anything that still leaks. |
