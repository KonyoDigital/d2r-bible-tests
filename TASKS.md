...
|---|---|---|
| **v3102** | `(this commit)` | v3102 — the second eye second and third findings on v3100, both mine: my own gate counted paintGrok() which the DECLARATION contains, so deleting the real call stayed green; and capWindow circuit means at least one max is 0 while the renderer overwrote BOTH bars, printing a denominator that was not the hours. The gate now extracts paint and paintGrok and runs them in node against a stub DOM, so it judges the rendered text rather than the source it is written beside. |
