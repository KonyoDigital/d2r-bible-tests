...
|---|---|---|
| **v3389** | `(this commit)` | v3389 — Two copies of a truthiness test on a container let a board that never synced publish three zeros as progress while the ledger verdict inside the same object already said UNSYNCED |
| **v3388** | `(this commit)` | v3388 — Both git pathspecs listed py mjs and sh but never js, so v3387 shipped two Cloudflare function files that the second eye never saw and that were never even candidates to be reported missing |
