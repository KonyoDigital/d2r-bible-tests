...
ATTACKS = (
    ...
    # ── more distinct states. Each one is a different field wearing a value that is not the
    # declaration the deleter is allowed to trust. ───────────────────────────────────────────
    ("examinedEmpty while it still names rows",
     {"extracted": [], "examinedEmpty": True, "rows": 7}, False,
     "a declaration of emptiness over 7 rows is the v2772 shape — the flag is not a denominator"),
    ("examinedEmpty is the integer 1",
     {"extracted": [], "examinedEmpty": 1, "rows": 0}, False,
     "only the boolean True is a declaration; 1 is truthy and is not that declaration"),
    ("examinedEmpty is the string true",
     {"extracted": [], "examinedEmpty": "true", "rows": 0}, False,
     "a word that says true is not the flag the writer sets"),
    ("examinedEmpty with no extracted list",
     {"examinedEmpty": True, "rows": 0}, False,
     "the flag without an extracted list is a seal that never said what it looked at"),
    ("rows is the string zero",
     {"extracted": [], "examinedEmpty": True, "rows": "0"}, False,
     "the characters 0 are not a count of rows"),
    ("rows is the bool False",
     {"extracted": [], "examinedEmpty": True, "rows": False}, False,
     "False == 0 in Python, so a flag would release frames from a seal that recorded no count"),
    ("rows is the float zero",
     {"extracted": [], "examinedEmpty": True, "rows": 0.0}, False,
     "a float is not the integer zero the empty-examination branch requires"),
    ("rows is negative",
     {"extracted": [], "examinedEmpty": True, "rows": -1}, False,
     "a negative count is not an empty examination"),
    ("rows was never recorded",
     {"extracted": [], "examinedEmpty": True}, False,
     "a missing count is not a measured zero"),
    ("a lookalike fact name",
     {"extracted": ["nаme", "location", "provenance"], "rows": 2}, False,
     "a cyrillic а in name is not the contract's name"),
    ("fact names with trailing spaces",
     {"extracted": ["name ", "location ", "provenance "], "rows": 2}, False,
     "the contract is the exact string, not the string plus whitespace"),
    ("the contract nested one level down",
     {"extracted": [["name", "location", "provenance"]], "rows": 2}, False,
     "a list of one list names no fact"),
    ("facts that are integers",
     {"extracted": [1, 2, 3], "rows": 2}, False,
     "positions are not the names of the facts"),
    ("a seal that is a list",
     ["name", "location", "provenance"], False,
     "a list of facts with no record around it is not a seal"),
    ("a seal that is a string",
     "name,location,provenance", False,
     "the contract written as one string is not a seal"),
    ("a seal that is an integer",
     0, False,
     "zero is not a record that anyone looked"),
    ("extracted is bytes",
     {"extracted": b"name,location,provenance", "rows": 2}, False,
     "bytes are not a list of facts, even when they contain the words"),
    ("blank fact names",
     {"extracted": ["", " ", "name"], "rows": 2}, False,
     "empty strings do not satisfy the two facts they are standing in for"),
    ("a why that never says nothing and never declares",
     {"extracted": [], "extractedWhy": "examined and empty", "rows": 0}, False,
     "empty is not the word nothing, and there is no examinedEmpty flag"),
    ("the contract carried as a tuple",
     {"extracted": ("name", "location", "provenance"), "rows": 2}, True,
     "a tuple is a list of facts; demanding a list() would refuse a sweep that named all three"),
    ("the contract carried as a set",
     {"extracted": {"name", "location", "provenance"}, "rows": 4}, True,
     "a set names the facts once each; order is not part of the contract"),
    ("a seal that is a tuple of pairs",
     (("extracted", ("name", "location", "provenance")), ("rows", 0)), False,
     "a tuple of pairs is not a record, even when the pairs name the contract"),
)
