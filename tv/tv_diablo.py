...
VERSION = "v1234"   # 👻⏪ GHOST MODE arc round 5 — COHESION / TIME MACHINE UNIFICATION. After r3+r4 the debugger had TWO stacked grail readouts (frame-scrub grail-at-T + months journey) both showing N/403 = redundant, read as two features. Now ONE Time Machine cockpit, two scales: (1) removed the redundant r3 grail strip entirely (element+builder+CSS) -> the GRAIL JOURNEY readout is the single grail display; (2) LINKED scales — scrubbing THE FILM (frame axis, this session) drives the JOURNEY marker to that moment (verified film-scrub moved marker 100%->97.5%), and dragging the journey directly still sweeps 29->351; (3) one visual language — both axes wear the same accent-band + scale sub-label "⏱ THE FILM · this session, frame by frame" (secs/reels) and "🏆 THE JOURNEY · your grail across all time — drag the marker, or scrub the film above" (months). Reads as ONE instrument, two scales, not two scrubbers stacked. Net SIMPLER (7ins/26del). Truthful, pure add-on, GHOST-off = zero trace. control_ui.html only. GHOST MODE 5/50+. ×3 parity
