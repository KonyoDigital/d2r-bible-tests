...
VERSION = "v1327"   # 🗺📊 B8 SESSION SCENE FINGERPRINT (engine, builds on B4). The session payload had sceneReads {scene:count} + areas[], but not the truthful Diablo-native aggregate. _session_scene_fingerprint(sess_rows) runs each sessions deep reads (scene,area) through the B4 _diablo_scene_label and returns {farmingReads, townReads, portals, townTrips, farmingPct, topArea, areas[], sceneReads}: farmingPct = farming/(farming+town) reads (menus stash/inventory/loot + unclear EXCLUDED); townTrips = distinct town visits (maximal runs of town-kind reads); portals = transition/loading reads (ENTERING events); topArea = most-read FARMING area. HONEST: returns None when the session read no scene/area; farmingPct None when 0 world reads (only stash/portal) - never a fake 0. Wired additively into /api/sessions as sceneFingerprint (honest-absent contract, alongside sceneReads/coverage/classFrames). Verified on the REAL journal: 27/107 carry a fingerprint (rest stub/thin -> honest null); real examples "89% farming · 1 town trip · 1 portal · top Throne of Destruction" (Baal run), "100% farming · Dark Wood" (Countess grind), "0% farming · 3 portals · Rogue Encampment" (the deterministic town list correctly reads gameplay-in-Rogue-Encampment as TOWN not farming). py_compile OK, 8/8 unit tests, G3 sweep + G4 flags unaffected. FRAMING (for the UI label): farmingPct is READ-PROPORTION not wall-time (the reader reads panels more than gameplay) - label it "farming (of reads)" so it doesnt imply a time measurement. control_app.py only. x3 parity.
