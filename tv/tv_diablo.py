...
VERSION = "v1247"   # 🖥⚡ CONSOLE UPGRADE round 1 - THE LIVE PULSE (the console now shows the KAI brain thinking, live; all truthful, all ride the existing /api/status poll, no new timers/fetches). (#1) LIVE READ FEED: st.liveRing surfaced as a premium stream directly under the reader-net eyes - "KAI retro · Harlequin Crest · grail" as reads land, verdict-colored (grail gold/keep mint/toss dim), labeled amber LIVE GUESS with a tooltip (provisional; the SEALED reel wins in retro, per _kai_engine_frame_effective) - never authoritative. Shows only ON AIR; lifted _erLiveFeed logic, restyled text-forward (no thumbnails = zero image loads while gaming). (#4) CAPTURE LOCK-ON PILL: the watching value is now a glanceable pill - green "Locked · Diablo II" on the game window, amber "whole screen" otherwise, grey "holding" idle (answers Konyos repeated is-it-targeting-the-D2R-window). (#3) READ-LANDED STAGE PULSE: brief premium toast on a NEW keep/grail live verdict, auto-dismiss 2.6s, never the first-poll backlog, reduced-motion honored. BONUS: the LIVE EYE now pulses while actually thinking on a frame (real visionBusyMs). FLAGGED: #2 reader-pool eyes (N-of-6) needs poolInFlight/poolN/poolWarm forwarded in control_app.py health first - flagged not invented. Inherits eyes-pulse honesty. Did NOT touch ENGINE HEALTH data model / anti-lag / fault lamp / parked GHOST MODE. control_ui.html only (110ins/7del). CONSOLE UPGRADE 1. x3 parity
