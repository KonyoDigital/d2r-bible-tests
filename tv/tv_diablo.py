...
VERSION = "v1324"   # 🔒 VISUAL-LOCK - closed the `font:` SHORTHAND weight BLIND SPOT (weight lock now TRULY complete + test-enforced on both surfaces). After v1321/v1323 declared weights locked, sessions-visual caught 2 more raw weights in control_ui.html hiding in `font:NNN` SHORTHAND (font: 400 .. / font: 600 ..) that the font-weight:-only pattern never scanned - and the SAME blind spot left 13 shorthand weights in bible.html (g3s passes only folded font-weight:NNN). Fixed all: sessions-visual folded control_uis 2 (font:var(--fw-regular/medium)); team-lead folded bibles 13 (font:700->font:var(--fw-semibold) etc., identity, bible already has the full --fw set incl --fw-regular). WIDENED the invariant test (visual_lock_invariant.py): added RAW_SHORTHAND = font: *(400|500|600|700|800|900)\b (boundary-terminated so font:14px / font:var(--fw-*) never match) alongside the RAW_WEIGHT pattern, so BOTH the font-weight: and font: shorthand weight classes are now caught. PROVEN: test GREEN on the clean tree (both files 0 raw weights of either syntax + --fw sets intact), and injecting a font:700 shorthand into bible triggers exit-1 naming bible.html:LINE (reverted clean). Both bible.html + control_ui.html now grep-clean for font-weight:NNN AND font:NNN. THE WEIGHT TYPE SYSTEM IS FULLY LOCKED + DRIFT-TESTED (both syntaxes, both surfaces). control_ui.html (2 folds) + bible.html (13 folds) + visual_lock_invariant.py (widened). x3 parity.
