...
VERSION = "v1364"   # 🗺 B9 item-read Diablo precision (rarity CLASS the engine can PROVE, honest). _kai_item_class(name,tier,grounded) + _kai_item_phrase + _kai_base_names exclusion lexicon; enriches the forensics synthesis + adds itemClass to every forensicRead. Two provable classes only: runeword (derivable via _kai_runeword_bare) · grail (ONLY grounder-proven or register tier==grail), everything else PLAIN (no class). Bases explicitly EXCLUDED (the guard caught Shako correctly returning None - Shako is the base of Harlequin Crest, not a grail; no "the grail Battle Boots"). unique-vs-set NOT faked - stays client-side tipOf (honest engine limit). Combines with B10: "grounded the grail War Traveler, matched the base · FARMING · Act 1 · Dark Wood" (verified on the reel), "identified the runeword Spirit", plain "read Super Healing Potion". ZERO count/routing impact - pure labeling of the projection. Verified: 7/7 B9 (runeword · grounded-grail · tier-grail · base->None-even-when-grounded · plain->None · phrase/synthesis), forensics 8/8, 197/197 control, 181/181 routes, invariant GREEN, py OK. control_app + test_control. x3.
