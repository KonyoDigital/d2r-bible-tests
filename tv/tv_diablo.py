...
VERSION = "v1283"   # 🔎 SESSIONS FLAGSHIP D9 - SHELF FILTER BAR (polish-ui-2, UI). Extended the D8 seed-chips into a proper composable filter bar (refactor, NOT a duplicate - the old chips are gone). A persistent row under the search box, three grouped clusters: grail (independent toggle - runs that found a grail); clean / flagged (verdict filter, mutually exclusive); today / this week / older (date window off each cards t0, mutually exclusive; this week = last 7 days incl today). ALL compose with the live search box AND each other (AND logic) - e.g. grail + "spirit" = the intersection; one honest "N of M" count reflects the combined result, honest no-match when over-filtered; chips show active state; the whole filter state + search query persist across the 12s poll. Each card carries data-grail / data-verdict / data-t0 so the filter checks them directly (no re-derivation). Pure UI off the card data - no engine. demo 7/7 (:17980), JS/brace/tests green, no page errors; probe: grail->10 of 107, grail AND spirit->6, clean/flagged exclusive (15/44), date filters exclusive (today 30 / older 0), grail+clean->1. control_ui.html only (55/19 net). x3 parity.
