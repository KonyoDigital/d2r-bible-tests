...
VERSION = "v1212"   # 🧠🖥 ENGINE ROOM arc round 3 — LIVE HEALTH-STATE PILLS ("which engine is hot/cold/stalled RIGHT NOW"). The spine's engine-health was encoded ONLY by a small corner dot (color+pulse) — the bottleneck was technically visible but didn't pop; you had to decode dot colors + dig buried metrics. FIX: a plain-language pill per organ on the cockpit LIVE spine — HEALTHY(green)/STRAINED(amber)/DOWN(red)/IDLE(grey), colored by the organ's existing pulse (no new health semantics). Strained organs pop against calm healthy ones → the choke point reads instantly (e.g. capture healthy → readers·router·funnels STRAINED → ledger healthy = mid-pipeline bottleneck). Scoped: big cockpit organs only (home small-cards + retro spine verified 0 pills — retro shows per-frame verdicts instead). control_ui.html only. ×3 parity (40/80 → v1272)
