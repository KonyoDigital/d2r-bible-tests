...
VERSION = "v1276"   # 🎬 SESSIONS FLAGSHIP D5 ENGINE - coverage meter + classFrames montage feed (team-lead; lights up the two D5 sections polish-ui-2 built honest-absent). /api/sessions now carries per-session coverage + classFrames, sourced from the sealed reel OWN kai_report.json (the reconciled ground truth built at seal time), NOT the journal. WHY the report: deep item-reads reference loose capture frames while the reel archives f_*.jpg stills - two frame namespaces - so a raw journal read/frame ratio mixes them and lies; the report completeness already reconciles reads<->film, and its classFrames already picks one representative REEL still per real scene, servable in the exact /hist/reel_<sid>/f_*.jpg form the shelf thumb uses. coverage = {read, total(=item-text moments KAI saw = reads+unread), gaps(=text seen but unread)} so read/total is the real read-completeness % (omitted when the reel had no item text at all). classFrames = up to 6 {scene, thumb, frameId, ts}, one per scene the run visited, click->jump via ts. SEALED-REELS-ONLY: unswept/live reels have no report so both fields stay absent and the UI hides the meter+montage (honest-absent, no fake zeros). Memoized reel-report reader (_reel_report_cached, path+mtime keyed) so the ~12s poll never re-parses a hot report. Verified on the real journal: 72/107 sessions carry coverage, 81 carry montages, every thumb resolves on disk, low %s on old pre-guard reels are the TRUTH (they under-read - exactly the audit A2/A3 finding). NOTE for polish-ui-2: the report scene vocabulary is richer than the live dictionary - adds tooltip + stash-gems/materials/runes sub-tabs - title-case/label as you like. 156/156 engine tests green. NEEDS a console restart at a safe moment (Konyo not scanning) to serve; verify on a throwaway port meanwhile. control_app.py only. x3 parity.
