...
VERSION = "v1373"   # FORENSICS ENHANCEMENT - the v1366 unresolvedKind delta consumed (the scary "unresolved 103" now reads honest+calm). Two additive features on the READS FORENSICS drawer: (1) a "0 grail misses" reassurance headline from summary.grailMisses - grailMisses===0 -> mint "0 grail misses - the AI dropped no grails this reel; the needs-your-eye reads below are screen text and unreadable scraps, not lost loot"; >0 -> danger "N grail misses - grail-tier reads went unresolved, check needs-your-eye, pinpoint+re-scan"; null (older reels) -> hidden (honest-absent). (2) the NEEDS YOUR EYE section now splits by unresolvedKind - screen-text-not-loot (non-item) vs truly-unreadable (unreadable-item), with an Unclassified fallback so nothing is dropped; BACKWARD-COMPATIBLE (a reel with no unresolvedKind renders flat as before). Verified all paths in-browser: 0->mint, 2->danger, null->hidden, split shows both subgroups, legacy reel flat. invariant GREEN, JS 0, no-scroll, demo baseline-equivalent. control_ui only. x3.
