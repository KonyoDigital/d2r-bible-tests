...
VERSION = "v1259"   # 🛡🔬 AUDIT FIX A2+A3 - gate honesty + kaiVer re-seal (finishes the wallpaper-Gems defense). A2 PHANTOM-OCR GATE HONESTY: the closer copied the grid-derived eye cls into _ocr_cls -> a phantom votes['ocr'] (pixel) alongside the real votes['grid'] (layout) -> _router_conf counted ONE detector as TWO classes -> conf 2 self-certified the gate. FIX (no naive drop, which would kill TRUE gems where grid is legitimately the only signal): the phantom is removed (_ocr_cls=_eye_cls now guarded by ocr in _eye_src), and a true tally tab fires via an EXPLICITLY SANCTIONED single-signal route - the gate clears grid-solo at conf 1 ONLY when grid is the sole witness AND the closer flagged its tighter gridSolo bar (a definite gems/runes/materials pick on a panel-open dark-cell lattice). _kai_gate_check gains grid_solo_ok; _kai_build_routing computes it; the closer adds gridSolo to routing rows. TestSingleGridSignalIsOneWitness (_router_conf([grid])==1) preserved. A3 kaiVer 3->4 RE-SEAL: seal stamps kaiVer:4, reclose gate raised (_KAIVER_TARGET=4, if _kv<4) so already-sealed reels re-sweep and pick up v1254/v1256/v1258/v1259. Left _kai_engine_frame_effective threshold at <3 intentionally (bumping blanks valid kaiVer-3 seals till re-sweep = FIX E territory). VERIFIED (resim): real gems s_1784736270319 still routes tally:gems (conf 1, gatePass True); wallpaper reels re-classify all-gameplay 0-gems. Floor 525 (+5). control_app.py + tests (NOT control_ui.html). AUDIT FIX A2+A3. x3 parity
