...
VERSION = "v1214"   # 🧠🖥 ENGINE ROOM arc round 5 — SEALED|LIVE BOUNDARY (which side of NOW am I scrubbing?). The timeline explained the sealed-vs-live split in WORDS (band caption) but the SEAM between the last sealed reel and the NOW zone was a plain 3px dark gap — identical to every inter-reel divider — so the single most important boundary (past|now) had no distinct marker + the sealed region had no zone label. FIX: (1) a faint "◀ SEALED" zone eyebrow over the recorded-reels region (absolutely positioned, doesn't disturb flex layout), (2) a distinct bright-mint SEAM at the sealed→NOW boundary (matching the live/green language) replacing the plain divider. With the green NOW block relabeled "the live edge" + the r2 retro playhead + "◀ RETRO" band, Konyo always knows: SEALED past left, live edge right. Empty-state verified (seam/eyebrow only render when reels exist). control_ui.html only. ×3 parity (42/80 → v1272)
