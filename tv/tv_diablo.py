...
VERSION = "v1322"   # 🔴 SESSIONS FLAGSHIP D24 - LIVE "RECORDING NOW" CARD (sessions-visual, Phase C). When a session is actively recording (on-air data-state=on) a live preview card appears at the TOP of the shelf (above the highlights strip) - the reel building in real time: ● REC pulse + "Recording now - this reel is building" + elapsed (mm:ss, right); reads so far (st.readCount) · current scene chip (_diabloChip e.g. "⚔ FARMING · Chaos Sanctuary") · phase (WATCHING/READING) · newest keep/grail (from st.liveRing -> 🏆 Death\u2019s Fathom). Mint reel-accent, red pulsing ● REC. LIVE + HONEST: reads the /api/status poll already in hand (no new fetch); paint(st) updates the card IN PLACE each poll (no innerHTML re-render -> the ● REC pulse never restarts, reads/scene tick live); only real live counts, newest-find omits when liveRing has no keep/grail; HIDDEN entirely off-air/sim (verified); respects on-air-compact (lives in the shelf overlay not the home dash) + no-scroll; reduced-motion -> ● REC solid (CSS @media). Verified: on-air card populated ("148 READS · ⚔ FARMING · Chaos Sanctuary · READING · 3:42 · 🏆 Deaths Fathom"), off-air hidden, JS 0 errors brace 0. NOTE: elapsed is client-tracked from first-observed-on-air (status payload carries no session-start ts) - reads "recording for ~X"; a true session-start engine field would be a one-line swap. control_ui.html only (67/1). x3 parity.
