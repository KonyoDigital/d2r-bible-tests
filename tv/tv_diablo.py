...
VERSION = "v1286"   # 📈 SESSIONS FLAGSHIP D12 - HISTORY TIMELINE / SPARKLINE (polish-ui-2, UI). COMPLETES the Sessions search/filter/sort/group/timeline cluster. A compact runs-per-day activity sparkline above the shelf: 14 day-bars from every runs t0 (client-side bucketing, no engine), bar height = runs that day, a day that produced a GRAIL gets a gold bar, today highlighted, hover names the day + counts ("Tue Jul 21 - 35 runs - 18 grails"), summary reads "93 runs - 24 grails" for the window. HONEST + light: real counts, NO smoothing/interpolation (today bar 26 cross-checked vs real session count 26). Deliberately independent of the live filters - a steady bird-eye overview above the filtered cards (can be made to compose later if Konyo wants). Dark-surface styled, read-only, hover-title only. demo 7/7 (:17980), JS/brace/tests green, no page errors; probe: 14 bars, 2 grail days gold, today marked, summary 93/24, today-count 26=26. NOTE: a clean standalone screenshot was defeated by the off-air short-stage transport overlap - a known layout artifact (VISUAL-LOCK candidate), not a timeline bug. control_ui.html only (44/1). x3 parity. The Sessions flagship arc (D1 finds -> D2 dossier -> D3 history -> D5 decision-story -> D6 recovery -> D7 seal/regret -> D8 search -> D9 filter -> D11 sort -> D10 grouping -> D12 timeline) is a complete cohesive product; D-arc paused here.
