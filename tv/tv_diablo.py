...
VERSION = "v1308"   # 🏦 E1c - THROWN-WITH-STATS COMPARISON (completes Konyos "muled-with-stats vs thrown-out, side by side"). Tossed items no longer vanish: aicToss now logs the thrown item WITH its checker read - name · verdict · score · the stats it read · a why (the affix breakdown, or "no valuable affixes") - to d2r_aicThrownLog (newest-first, capped 50); a bare clear (empty draft) logs nothing (honest, only real tosses). New "📊 Kept vs Tossed" comparison panel in the AI Item Checker card: two columns - 🏦 Muled (kept, checker-verified mules each with verdict/score + the E1b ✓✓ 2x-confirmed / ⚖ unclear badge) vs 🗑 Tossed (each verdict/score + why-tossed). Read-only review, nothing changes any item; hidden when both empty; a clear-tossed-log button when theres history. Impl: _aicThrownGet/Add/Clear store (window-exposed), _aicRenderCompare reads magicFinds (via _mfChecker) + the thrown log, called at end of renderAIItemChecker; aicToss captures draft + _aicVerdict before clearing; scoped 2-col CSS (stacks narrow). Verified: 16 bible scripts clean; node 8/8 (empty->hidden, newest-first, both columns, kept shows 2x confidence tying into E1b, tossed shows verdict+why, unchecked bare intakes EXCLUDED from kept, counts, clear); backward-compatible. E1 FOLLOW-UPS DONE (E1b+E1c = check twice for confidence + see kept-vs-thrown side by side). bible.html only. x3 parity.
