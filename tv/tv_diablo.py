...
VERSION = "v1300"   # 🟣 G4 TOUCHPOINT #3 - grail-promotion re-check (3rd + highest-value cheap seam; ALL 3 G4 SEAMS NOW COMPLETE, inert until keyed). Same /kai_verdict handler after touchpoint 2, one fenced block. Fires ONLY when the grail gate just PROMOTED a weak read up to grail - incoming verdict was toss/border and the gate bumped it to grail (a low-quality read of a unique/set name auto-becoming a Chronicle tick = the split-brain class that bit us with "Spirit"). Self-contained detect: _tier==grail AND body.verdict.tier in (toss,border). Confident grails (arrived grail, not promoted) + every non-grail final skip Grok. Rarest + highest value - a wrong promotion corrupts the Chronicle, worth one confirm. FLAG-NOT-OVERRIDE: disagreement attaches rec["kai"]["judge"]["g4"]={agree:false,verdict,note,ts,source:grok,kind:grail-recheck}; NEVER un-promotes (Konyo reviews at v1303). FULL FLAG MAP (v1303 reads all 3): seam1 register[g4] in kai_report.json; seam2 kai.judge[g4] in sessions.jsonl; seam3 kai.judge[g4] kind:grail-recheck. Toggle "🟣 N flagged" (stats.disagree) already sums all. Verified: py_compile OK; OFF byte-identical (no g4 key on a promotion, rec clean); ON+disagree both border->grail AND toss->grail flagged with tier STAYS grail; skips confident grail + non-grail final; one-grep removal intact (6 fence pairs: module/GET/POST/tp1/tp2/tp3). Live untested (morning). control_app.py only. x3 parity.
