...
VERSION = "v1280"   # 🔒 SESSIONS FLAGSHIP D7 ENGINE - seal-latency + regret-spotlight feed (team-lead). /api/sessions now carries per-session sealMs + regretItems. sealMs = closedAt - run-end (latest engineFrame ts), with a TRUTHFUL guard: a re-swept reels closedAt lands hours/days after the run (a reseal, NOT a seal latency), so sealMs is emitted ONLY when the gap is plausibly a real seal (>0 and <=30min) and left absent otherwise (the chip hides). Verified: all 27 current reels are re-swept (min gap 230min), so sealMs is honestly absent across history and will populate on the next FRESH run sealed minutes after ending - correct honest-absent, confirmed not a units bug. regretItems = [{name,frameId,ts}] for each name this run THREW that the judge later ruled KEEP (built from the same thrown/keeper loop the regrets count uses, now frame-located for the jump chip); dormant (None) on every current session because the journal has NO thrown_names yet - it lights up the instant a real regret occurs. Both fields degrade gracefully in the UI (chip omitted / spotlight falls back to the count). Verified on the real journal + 156/156 engine tests green. Serves after the next console restart. control_app.py only. x3 parity.
