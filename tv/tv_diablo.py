...
VERSION = "v1223"   # 🧠🖥 ENGINE ROOM arc round 14 — MATCHED NAME (raw → resolved, at a glance). The engine ALREADY resolves raw OCR into a clean name server-side: a read frame's beat carries b.names (clean, "Sullied Grand Charm of Blight") AND b.ocr_raw (the garble it was read from, "'Ii'"). So no fuzzy-match/DB access needed (safer, exactly as trustworthy as the engine's own resolution). FIX: the hero's "📖 the AI read this frame as" now shows the CLEAN name + a "⚡ cleaned up from <raw OCR>" line beneath it when raw≠resolved — Konyo SEES the resolution beside the stash frame. Solved a real wrinkle: a frame carries TWO beats at the same ts (raw OCR-lane + resolved deep-lane) and nav lands on either → merge siblings by frameId, surface the one that both named AND kept the raw. Only shows the ⚡ line on an actual cleanup (gameplay stays empty; KAI-caught footage shows raw unchanged). control_ui.html only, beat fields only. ×3 parity (51/80 → v1272)
