...
VERSION = "v1226"   # 🧠🖥 ENGINE ROOM arc round 17 — FIRST-OPEN ONBOARDING (a newcomer would leave without discovering 90% of the cockpit). The ENTIRE retro half (frames/images/drill-down/scorecard, rounds 6-16) hides behind ONE gesture a new user would never guess — dragging the timeline into the sealed reels. FIX: a one-time first-open orientation strip — "👋 first time? three moves — drag the timeline ◀ into the sealed reels to replay the past · click a frame or engine to see what the AI read on it · the key below decodes every symbol" + a "got it ✕" dismiss. Points out exactly the 3 moves that unlock the cockpit. Shows only on the very first open (localStorage tvd_er_onboarded, same pattern as tvd_card_pref), dismisses cleanly, never returns. Subtle gold strip; Esc still closes the cockpit with it up (no trap); the show is wrapped in the v1191 bulletproof-open try/catch so it can never break the open. control_ui.html only. ×3 parity (54/80 → v1272)
