...
VERSION = "v1351"   # 📺 TV-D ENGINE BAY (R2+R3) - the engines are living instruments now, on the honest wire (kills the dull; the nothing-hidden doctrine made visible). The rail engine cluster is a named instrument bay headed "AI ENGINES - THE SCANNER": all engines named + a green/dark wired-running lamp each, reading g3s status.engines authority directly. Verified honest across the full contract: liveEye green-pulse "reading - 2s ago"; secondEye dark "not connected - TV_VERIFY off" (wired:false, visible + reasoned); kai dark "no OCR bin - closer not plugged" (the exact wire Konyo wanted surfaced); router green-pulse "routed a read 4s ago - seen 128"; watchdog steady "armed - last seal clean", red-alert + what-tripped on a real violations verdict. Paint rule honored: green only on state==live, steady on armed, dark on wired:false/idle/down; note = the honest status line; disconnected engines VISIBLE + dark, never hidden. ROUTER + WATCHDOG (previously invisible) surfaced. METER-BLEED KILLED (root cause: on-air the stage min-height overflowed the fixed shell so the off-air dashboard cards ghosted behind the meters; fix = hide the off-air dash while live, body[data-state=on] .home-dash{display:none}, reclaiming stage room). STAGE PRESENCE: the live phase word breathes (slow glow-pulse, reduced-motion aware). Verified: invariant GREEN, JS 0, demo J2 alignment 0.00 + J5 + J6 pass, no-scroll (overflow 0 on-air). NOTE: v1349 zone-banner tier was NOT actually committed (a gate/revert race dropped it; saved in a patch for the Sessions-tab arc); v1349 shipped the engine-exposure backend only. NEXT: eyes.liveAgeMs pulse fidelity / kaiTs / sessionHealth.gate / mindStory / liveRing dials + the AI-reads receipt feed. control_ui.html only. x3.
