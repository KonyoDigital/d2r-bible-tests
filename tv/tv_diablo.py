...
VERSION = "v1281"   # 🔍 VAULT INTEGRITY CHECKER (Konyo, live: "an AI reader retro checking the muled items too - properly routed, rightly muled by judge, rightly thrown out, no duplicated.. like checking"). DETERMINISTIC-first retro pass (his pick) that re-derives the correct home for every muled/kept/thrown item and flags where the stored reality disagrees - ZERO AI cost. Five checks: MIS-ROUTE (item filed in the wrong locker vs its rule-derived mule), RW-IN-MAGIC (a forged runeword stranded in Magic&Rare - the CTA gold-dot class - belongs in RUNEWORDS), DUPLICATE (same item double-filed across the keep/magic/throw stores, which are meant to be mutually exclusive), WRONGLY-THROWN (a thrown-out read thats actually a grail unique / runeword / real keeper - grail-safety rescue), SHOULD-REVIEW (a muled item the rules would discard - surfaced, NOT auto-fixed, could be a real keep). Unambiguous findings carry a one-click Fix + a "Fix all safe" button; the judgement call is review-only. TRUTHFUL: every finding is a genuine disagreement between a stored item and its rule-derived home, nothing invented; fixes route through the same suggestMule/assign/owned/magicFinds/unknownReads the vault already uses, then persist(). A banner above the vault dock shows findings and stays HIDDEN when the vault is clean. Verified: extracted the shipped _vaultAudit + _vaultAuditApply and ran a crafted messy vault (misroute/rw-in-magic/dupe x2/wrongly-thrown/should-review) - all 5 fixable detected + fixed correctly (Shako->uni-armor, CTA->RUNEWORDS, Enigma rescued, dupes de-duplicated), review-only left alone, re-audit idempotent. This is the vault twin of the Sessions reader audit; the AI re-judge layer (G4) comes on top later. bible.html only (16/16 inline scripts clean). x3 parity.
