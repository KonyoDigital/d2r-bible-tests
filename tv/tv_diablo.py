...
VERSION = "v1345"   # 🎨 DESIGN-LOCK R2+R3 (Konyo-approved) - console PADDING unify + bible LS/LH scale + demo fix, bundled into one stamp so the bible.html stamp-touch cant clobber g3s in-flight ls/lh fold. CONSOLE PADDING (control_ui.html): killed the 7/8/9/10/11/13/16px pad wander -> 2-tier like the radius: PANEL CARDS on --hd-pad (12/14, folded .eh-organ/.eh-big/.rn-feed/.kai-gate/.sess-health joining .hd-col = 10 card sites one interior); INTERACTIVE ROWS on NEW --hd-pad-row (9/13, .hd-chip+.tf-row were already matched = zero-visual). FLAGGED compact-tier kept as-is (intentional density, folding would cramp): .kpi 6-across stat tile (8px narrow), .rnf-row/.rn-pool/.eye live-feed rows (tight so many fit). BIBLE LS/LH (bible.html): folded 462 em letter-spacing + line-height literals onto a new --ls-*/--lh-* scale (console role names: ls tight.02/snug.04/normal.06/wide.10/label.14/wider.20/widest.28 em; lh none1/tightest1.1/tight1.2/snug1.35/normal1.5/relaxed1.65/loose1.8) - approved deliberate normalization, near-dupes collapsed +-.05lh/.02em max, genuine one-offs (.24/.42/.35em, sub-1 lh) kept. Deferred: ~240 px letter-spacing (different unit - intentional micro-tuning, leave as-is) + ~100 JS-inline (regex-risky, follow-up). DEMO FIX: B8 shelf scene-fingerprint span wore .sh-verdict (which demo j7 requires to carry a seal glyph) -> gave it its own .shc-fp class (display:block+color folded on, render identical); demo_console 7/7 GREEN again. INVARIANT EXTENDED: now locks --fw + console --hd (incl radius-in + pad-row) + bible --ls/--lh scale (all defined-checks, negative-tested). Verified: demo 7/7, invariant GREEN, JS 0, 16 bible scripts compile, no-scroll intact. control_ui.html + bible.html + visual_lock_invariant.py. x3.
