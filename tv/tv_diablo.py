...
VERSION = "v1313"   # ✨ SESSIONS FLAGSHIP D20 - ANIMATED STAT COUNT-UPS (sessions-visual, Phase C). When the dossier opens the key numbers tick up from 0 to their value with an easeOutCubic over 520ms: the 4 stat tiles (reads · frames · named · found), the decision-funnel numbers, and coverage%. A premium micro-interaction, fast + tasteful. TRUTH-SAFE by construction: purely cosmetic - the FINAL animation frame writes the exact original text verbatim (el.textContent=finalText) so the number always lands on the real value; the animation never computes or alters truth; formatting (commas, %) preserved; non-numeric "—" + abbreviated 12.3K skipped and shown exact (no fake tick). MOTION DISCIPLINE: prefers-reduced-motion -> NO animation, exact finals instantly; guarded to run only on a real open (freshOpen check on ov.hidden || DOSSIER.n!==n) never on the 12s poll re-render so numbers dont re-animate while youre reading. Verified on the real dossier: 0 -> 877 -> settled EXACT (1480/4200/37/11 tiles, 90% coverage, funnel 1480/37/11/2); reduced-motion forced -> exact finals instantly no 0-flash; JS 0 errors brace 0. control_ui.html only (33/0). x3 parity.
