...
VERSION = "v1217"   # 🧠🖥 ENGINE ROOM arc round 8 — MISS SPOTLIGHT / jump-to-problem ("take me to the problems"). After r6/r7 Konyo can step frames + see each, but a reel has 100+ frames and only a handful are interesting. FIX: a flagged-frames spotlight bar at the top of the drill-down that classifies every frame the engine did something notable on — 🛡 gate HELD a read · 🧠 KAI retro-caught a live miss · 🧠🔬 super-analyze recovered · 🔴 read stalled — shows the shape ("⚑ 22 flagged · 19 held · 3 retro-caught"), whether THIS frame is one, and ⏮/⏭ buttons + [/] keys that jump to the next flagged frame (wrapping). Clean reel = WIN message ("✓ no flagged frames — engine ran clean"). Landing drops onto the full evidence + frame image. Refactored r6 cursor/render/fetch into shared _engineRoomGotoFrame(); jump references focusBeat.ts so first press always moves. All signal from cached beats (no new endpoint). control_ui.html only. ×3 parity (45/80 → v1272)
