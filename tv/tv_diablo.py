...
VERSION = "v1362"   # 🗺 U2 version/cache alignment - confirmed sound + 2 guard gaps closed (test-only, no live-path touch). The x3 ship stamp + cache-bust chain (_app_ver reads the parity stamp, /board no-cache, iframe v=boot->real stamp, amber-on-skew) verified solid. Added test_board_served_no_cache (the anti-stale mechanism was untested) + test_app_ver_equals_ship_version (the cache-bust value locked directly to the ship tag). 185/185 control, py OK, no behavior change - two silent-staleness paths now locked. test_control.py only. x3 parity.
