...
VERSION = "v1270"   # 🧠🗓 G2 TASK-FORCE AI PRIORITY-ROTATION ENGINE (Konyo: the daily pick / OPS queue / mission brief dead-ended on runewords 99/99 - make it read the chronicles, descend from runewords first as easiest/hit, and intelligently rotate to the next incomplete one). New _chronRotation() reads every chronicles real completion, treats a sealed one as DONE, descends past it, RANKS the incomplete ones (runewords are the priority tier - task first while any remain; once sealed, closest-to-done wins, tie-broken by fewest left) and attaches a TRUTHFUL farm move to each from the same verified boss/odds source the Forge uses. Five wire-ups: dailyCreateAi no longer wipes to "no runewords remain" (writes a real grail/set pick, only says all-sealed when truly complete); the Task-Force DAILY PICK row; the MISSION BRIEF resolution (best-EV grail/set target, not alphabetically-first) + deepened detail (best boss, 1:X per kill under MF/players, N left, route hint); the OPS QUEUE descends to real grail/set ops instead of "queue clear". For Konyos live state (runewords done, grail 246/403, sets 108/135) it ranks sets(80%) over grail(61%) -> Daily Pick "finish Tancreds Battlegear (1 piece left)", grail line "hunt Greyform (best at NM TZ Pindleskin, 1:1.1k)". TRUTHFUL (real counts + verified sources; omits the boss rather than inventing when a set has no matched source). New tests/v1270_taskforce_rotation.spec.ts (5) + existing green (_PICK_V bumped 2->3). bible.html + specs only (NOT control_ui.html). x3 parity.
