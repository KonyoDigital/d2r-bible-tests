...
VERSION = "v1273"   # 🔧 TASK-FORCE SET-MISSION ROUTING FIX (Konyo, live: clicked Do-this-now / OPS row on a set piece and it dead-ended on "item not found"). ROOT CAUSE: the v1270 chronicle-rotation engine tagged a SET rotation mission as kind:grail and both the Mission Brief Do-this-now (_sessionDoMission) and the OPS QUEUE Open button routed the set-PIECE name (Tancreds Hobnails) through openItemDetail, which looks the name up in ITEMS - but ITEMS only holds uniques/runewords, not set pieces, so it fell through to the "item not found" card. Grail uniques were fine (they ARE in ITEMS); only sets broke. FIX: set-rotation missions now carry kind:set (+ setName); _sessionDoMission set-branch lands on the F.Sets tab and snaps to Quick-wins (where the last piece + its best run + a tick button live) instead of the ITEMS lookup; the OPS QUEUE set row routes via goTab:fsets + fsetsSetFilter(near) instead of openItemDetail; the mission brief detail line + TZ verdict now recognise kind:set alongside grail (real set farm text). No more "item not found" from any set task. bible.html only. x3 parity.
