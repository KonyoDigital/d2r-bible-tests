...
VERSION = "v1361"   # 🔬 READS FORENSICS surface (Konyos X-ray, LIVE) + E2 regression guard + E3 re-close (E-series CLOSED). FORENSICS SURFACE (control_ui.html): the 🔬 READS FORENSICS drawer opens from the 🧾 AI READS feed (button on the tag, stopPropagation so it doesnt cycle) - full-screen, Esc/close/backdrop, session picker (default newest). FLAT read stream in THREE status sections (Konyos steer): ✓ GOT IT RIGHT (grounded/corrected/recovered) · ⛔ CORRECTLY REFUSED (blocked-fp) · ⚠ NEEDS YOUR EYE (unresolved). Each row: verbatim garble chip, the corrected FIX-transform ("SupFR POTIIIN -> Super Healing Potion ✓ via vocab edit 3"), the blocked REFUSAL-transform ("Diablos -> Ars AlDiabolos? ⛔ no base line, 1 edit away"), the 🧠 synthesis headline (Diablo language), witness chips (name/base/content/cross-frame), click->drill-down (verbatim frame + read-across-frames trail), hover-art (/hist/<frameId>.jpg). Honest-absent (raw-less / unsealed reel -> honest note; null-item reads in the refused/unresolved buckets). On g3s v1360 /api/forensics. E2 REGRESSION GUARD (test_control.py): freezes the 209/209 no-tally-without-panel-open invariant (grid_solo_ok False -> held, True -> fires, wallpaper geometry -> never sanctions) so the desktop false-tally bug cant silently return. E3 RE-CLOSE (control_app.py, Konyo-approved): _KAIVER_TARGET 4->5 + seal stamp kaiVer 4->5 so all 29 pre-E1 reels auto-re-close under E1+②+guard - the recovered grails (War Traveler x7 etc.) land in the REAL registers/vault/grail-tracking (were only in the X-ray projection before); additive/merge-max, wallpaper-safe (204 frames sampled, 0 not_d2r leaks). + the standing CONVENTION comment: seal-time logic change MUST bump kaiVer or old reels strand. Verified: 183/183 control (E2 guard + E3 resweep fixtures), 181/181 routes, invariant GREEN, JS 0, py OK. control_ui + control_app + test_control. x3 parity.
