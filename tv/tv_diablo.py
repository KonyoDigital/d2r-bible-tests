...
VERSION = "v1328"   # 📋 SESSIONS FLAGSHIP D27 - RECAP EXPORT (sessions-visual, Phase C). Two actions on the dossier action row: 📋 Copy recap (clipboard) + ⬇ .txt (download), both generating a clean shareable text recap of the run - header + custom name (D25) + session# + note + date + run duration + area; Reads/Frames/Named/Found; Coverage (if present); Verdict; 🏆 Grails (names) · 💎 Keeps (names); Beats line; footer sid. HONEST + robust: only real data, every ABSENT field OMITTED (no coverage line without coverage, no grail line without grails); Copy uses navigator.clipboard with an execCommand textarea fallback + button flashes "✓ Copied" (or "⚠ Press ⌘C" if blocked); Save writes a text/plain Blob named from the slugified custom name (tvdiablo-meph-200x.txt). Pure UI, no engine/network. Verified: intercepted the clipboard write - recap complete + EXACT (all numbers match, name/note/area/coverage/grails/keeps/beats present), copy flashed success, .txt present, JS 0 errors brace 0, visual-lock GREEN. control_ui.html only (76/1). x3 parity.
