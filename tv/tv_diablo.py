...
VERSION = "v1225"   # 🧠🖥 ENGINE ROOM arc round 16 — SIGNAL-FORWARD drill-down (empty rows recede so the signal leads). The evidence rows all carried equal visual weight, but on a gameplay/held frame most are just "—" (measured 5/8 empty on a real held frame: reader-net, read-in-flight, OCR, quorum, funnel) — equal weight to the ONE row that mattered (the gate decision + reason), so the panel read as a form full of blanks with the signal buried toward the fold. FIX: _erDetailRow detects an empty ("—") value and recedes it (40% opacity + tighter padding), so the rows that actually FIRED lead the eye — the gate decision + super stand at full weight while the 5 empty rows quietly step back. Reads as "here's what happened," not a blank form. Tiny/safe/pure presentation (a class + one CSS rule); every row's content unchanged + still there. control_ui.html only. ×3 parity (53/80 → v1272)
