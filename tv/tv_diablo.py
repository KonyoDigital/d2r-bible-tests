...
VERSION = "v1298"   # 🟣 G4 GROK ACCURACY TOGGLE - the control surface for the whole add-on (bible.html Tools tab, after the 🔄 Auto-route Sweep panel; collapsed by default, labeled POWER-USER ADD-ON). Reads GET /api/g4_status, posts POST /api/g4_toggle {on}. Honest 3-state readout: OFF ("the app runs 100% deterministic - the default") · "ON - needs your xAI key" (switch on, no key -> amber inert switch, zero Grok calls) · "ON - active" (purple, shows model + hourly cap + checks-so-far + 🟣 flagged count); module-absent -> "not installed". The note spells out opt-in, uncertain/important reads only (never every item), flags disagreements never overrides, needs XAI_API_KEY, lifts out clean. Companion change in g4_grok.py: status() now also returns `switch` (raw button intent) alongside `on` (switch AND key) + a switch_on() helper, so the UI can distinguish switched-on-but-keyless-inert from plain OFF (is_on() unchanged in meaning). The "ON - active" line surfaces the running stats.disagree as "🟣 N flagged" (live at-a-glance; the full v1303 review surface reads kai_report register entries where g4.agree===false). Verified: 16/16 bible scripts clean; toggle paint node test 5/5 (OFF / ON-needs-key amber-inert / ON-active model+flagged / module-absent; aria-checked + data-inert correct); g4_grok.py status transitions verified across all 5 real states; py_compile OK; one-grep removal intact (2 bible fence pairs + engine fences). Live HTTP untested (booting control_app touches D2R PID - unsafe; routes are tested fns, paint node-proven; first click = Konyos morning enable). bible.html + tv/g4_grok.py. x3 parity.
