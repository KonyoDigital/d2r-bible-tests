...
VERSION = "v1359"   # 🧩 CROSS-FRAME QUORUM (multi-witness sweep #2, Konyo-blessed corroboration - generalized to the router). The routing gate judged each frame in ISOLATION, but a tooltip lingers across frames and different brains catch it on different frames. Now: a tooltip frame held at quorum<2 PASSES when a same-label neighbor within the items on-screen lifetime adds a DISTINCT independent evidence class (union >=2). HONEST NUMBER: 9 recovered, NOT the 49 design-estimate - and the shrink IS the discipline: Guard 1 (full-gate cell-correctness) excluded 35 route-nulled dedup/near-dup frames (dups of a cluster head that already carries the verdict - promoting a routeless dup would be wrong); Guard 2 scoped it to TOOLTIP ONLY after an existing test caught a stash-runes frame borrowing a grid witness (grid/tabstrip non-independence - the phantom-ocr defense). So: 9 genuine tooltip frames across 29 reels, 0 new false, same >=2-independent-class discipline, post-hoc PROOF never a new route/funnel, stash quorum untouched. Mechanism generalizes (more yield in longer/busier sessions). Verified: 8 permanent fixture tests (distinct-class promotes, same-class does not, never crosses labels, outside-lifetime blocked, cell-correctness still vetoes, route-nulled-dedup not promoted, gameplay never, non-quorum untouched), 168/168 control, 181/181 routes, invariant GREEN. control_app.py + test_control.py. x3 parity.
