...
VERSION = "v1344"   # 🎨 DESIGN-LOCK R1 - CARD-RADIUS UNIFY (Konyo-approved look change, "go with the recommendations"). Killed the old 7/8/9/10/11/12px console radius spread; collapsed to a deliberate TWO-TIER scale, both single-sourced so they cant drift again: --hd-radius (the ~12px clamp) = CARD/panel corners across 9 containers (.hd-col .kpi .hh-card .hd-card .eh-organ/.eh-big .rn-feed .kai-gate .sess-health joining .m/.signal/.ticker already on it); NEW --hd-radius-in:8px = INNER rows/chips/pills across 5 short elements (.hd-chip .tf-row .rnf-row .eye .rn-pool) - kept tighter so a 24px feed-row doesnt over-round into a lozenge (the design judgment: 2-tier reads intentional vs one flat radius; flagged for Konyos eye, 30s to flatten if he wants literally one). INVARIANT EXTENDED: --hd-radius-in added to the locked --hd-* set. Verified: computed radii all 12px (cards) / 8px (inner), JS 0 errors, brace delta 0, visual-lock GREEN, no-scroll intact, 0 console errors. control_ui.html + visual_lock_invariant.py. x3 parity.
