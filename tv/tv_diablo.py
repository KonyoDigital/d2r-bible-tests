...
VERSION = "v1237"   # 👻 GHOST MODE arc round 8 - the COHESION / VISUAL pass (presentation-only, zero data changes). The debugger now reads as ONE instrument, two families, top-to-bottom as the concept: (1) STRUCTURAL TRUTH - CURRENT STATE moved to the FOOT below the whole rewindable Time Machine (rewind up top, whats true now at the base; one line reordered, IDs intact); (2) FAMILY ACCENT as information - the band left accent ENCODES rewindability: purple on THE FILM + THE JOURNEY (rewindable Time Machine), amber on CURRENT STATE (not rewound), engine spine stays neutral gold (always-live loop, not a scrubber) - read the accent and you know if scrubbing moves it; (3) HIERARCHY - CURRENT STATE recessed into a secondary amber-tinted companion card so the Time Machine reads as hero and now as its quiet companion; (4) GHOST-SHELL FRAMING - tightened the stepped-into-the-machine feel (purple time-machine rim + violet halo on the panel, starfield .5->.62), all scoped to body.ghost-on so zero-trace holds. Truthful labels intact (rewind vs current). Pure add-on, GHOST-off = zero trace. control_ui.html only. GHOST MODE 8/50+. x3 parity
