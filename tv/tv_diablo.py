...
VERSION = "v1271"   # 📼🗂 SESSIONS FLAGSHIP D3 - HISTORY as a board region + first-run/empty states + z-fix. Promoted the shelf from a Theatre-trapped overlay into a first-class off-air HISTORY region on the home dash: a COLLAPSIBLE "HISTORY · N runs · tap to explore" (default collapsed to a slim ~46px one-liner; persists; tap to expand the run strip - cover/session/date/reads/top-grail/seal; tap a card -> the D2 dossier straight from home with the Theatre closed; "see all" opens the full shelf). FIRST-RUN/EMPTY: both the region + full shelf show a teaching hero (Press ON, your first session seals a reel here) instead of a bare box. Z-FIX: the shelf/transport overlap was tall cards clipping at the transport line - added padding-bottom so finds+fingerprint scroll fully clear. NO-SCROLL respected (collapsed-default adds only ~59px; the dash short-viewport overflow is pre-existing, eased further by the GHOST retirement next). D2 dossier + D1 finds + B8 fingerprint reused/intact; Theatre escape discipline held. control_ui.html only (128/3). Floor + demo 7/7 (J2 0.00px). SESSIONS FLAGSHIP D3. x3 parity
