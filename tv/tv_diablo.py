...
VERSION = "v1333"   # 🗺 classFrame.native - B4 chapters now LIVE on real reels + a stash sub-scene bug FIXED. Added native to each classFrame in _theatre_sessions: the reels classFrame carried only {f,ts}, so source each frames AREA from the nearest deep read (±5s) + attach native=_diablo_scene_label(frame.scene, that area). Honest-absent (null if no scene), additive (existing classFrame fields untouched). BONUS BUG FIXED (the real classFrames exposed it): tab-classified stash-gems/stash-runes/stash-materials scenes were wrongly sent to "unclear" by _diablo_scene_labels exact-match on "stash" - fixed sc.startswith("stash") -> STASH (menu); also corrects the routing-row / live / fingerprint paths for any tab-classified stash scene; bare-stash unchanged. Verified on the REAL journal: 81 sessions carry classFrames, EVERY classFrame now has native; scene->label map confirmed (gameplay->FARMING, stash/stash-gems/stash-runes/stash-materials->STASH, tooltip->unclear = item-inspect not a location). py_compile OK, B4 15/15 (incl a new stash-gems case), B8 + sweep(sunders 4) + g4-flags + status-native green -> the 116 suite HOLDS (helper change only ADDED correct stash-* handling). SESSION_FIELD_CONTRACT.md updated (classFrames now {scene,thumb,frameId,ts,native}). Result: sessions-visuals B4 chapter ribbon lights up on Konyos real reels with zero further UI work - each montage frame carries its game-true label 🏛/⚔/🛡. control_app.py + contract doc. x3 parity.
