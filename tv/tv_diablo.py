...
VERSION = "v1375"   # EV-RANK engine intelligence - the flagship hunt-next smarts (honest EV, no fabrication). _ev_hours(dropChance, killsPerHr, confidence=0.5) = expected hours-to-find matching the Calculator formula EXACTLY (runs=log(1-c)/log(1-dropChance); hours=runs/kph) so it cant drift; honest-absent None on invalid odds. _ev_rank ranks missing grails ascending by expected-hours (fastest first); invalid/unknown -> unranked (never a fabricated rank). POST /api/evrank stateless: the CLIENT posts each missing grail best source + real odds/kph (no odds-drift), the engine ranks the hunt-priority. Verified: formula-match to Calculator (1e-9), rarer-ranks-later, higher-kph-sooner, invalid->unranked, empty->empty, live POST ranks correctly. 7/7 EV + 226/226 control + 181/181 routes + invariant GREEN. Contract handed to sessions-visual for the flagship target (rank 0) + ordered hunt list; unranked shows as no-known-farm. Phase 2 learns-pace flagged. control_app + test_control. x3.
