...
VERSION = "v1263"   # 🛡🔬 CONSOLE F4 - THE ACCURACY-GATE HERO BADGE (surfaces the now-honest gate). Pulled sessionHealth.gate.{proven,held} out of the buried FUNNELS sub-line into a proud KAI GATE badge under the 3 eyes in THE KAI READER cluster. Two honest states: GUARDING (held>0, gold rail) "N% proven · N proven · N held · held N uncertain reads back - guarding your accuracy"; CLEAN (held=0, mint rail) "100% proven · every read cleared the two-witness gate". Tag = proven-rate %. Reads the real sh.gate (_newest_gate_count, gatePass from the newest sealed reels routing); both null -> badge HIDDEN (no fake numbers off-air). "held N uncertain" is the honest framing of "caught N misreads" - held = reads the gate refused to certify, not an overclaim. LIGHT (hdKaiGate rides the existing poll via hdRefresh, no new timer/fetch). Left FUNNELS organ + ENGINE HEALTH model untouched. 0.00px head-tab invariant held. control_ui.html only (58/1). Now that the gate is honest (v1259), the console shows Konyo the brain protecting his accuracy. x3 parity
