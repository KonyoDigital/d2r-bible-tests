...
VERSION = "v1224"   # 🧠🖥 ENGINE ROOM arc round 15 — GATE WHY in plain language (debug WHY the engine decided). The gate decision was expressed only in terse internal codes ("HELD · quorum<2", "no label", "not in DB") — cryptic for debugging. FIX: the drill-down's gate decision now spells out the why beneath the code — quorum<2 → "only one evidence source agreed — the gate needs at least two"; no label → "no item label was detected on this frame"; not in DB → "the read isn't a known item name in the database"; PROVEN → "N evidence sources agreed · confidence X". Konyo can now debug the decision without knowing the reason codes. Kept the terse _GATE_WHY map untouched (shared by the space-tight FUNNELS organ + reel drawer); added a separate _GATE_WHY_LONG used only in the roomy drill-down. gateReason+routeConf+routeSources already on the beat (no data-surface change). control_ui.html only. ×3 parity (52/80 → v1272)
