...
VERSION = "v1243"   # 👻🎬 GHOST MODE arc round 12b - FILM STORYTELLING AUTO-OPENS on scrub (symmetric with the JOURNEY, which already auto-narrates since r9 = one coherent instrument). In _engineRoomRenderRetro: scrubbing THE FILM now AUTO-OPENS the inline forensics with no click - land on any frame and the AIs-mind view (frame image, read raw->resolved, owner/verdict) opens INLINE on the ghosted console and tracks the cursor frame-by-frame (deep OCR/raw enriched as they land), head "THIS FRAME - what the AI saw and decided". Snapping to NOW (LIVE) closes it, so it is bound to actively-viewing-the-past, not permanently in the way. Still INLINE - never an overlay, never routes (inline != routing; Konyos rule holds). Truthful: real frames/reads only, honest gaps, honest not-archived note. ZERO-TRACE held: flip-off restores er-detail + scrubbers to #engine-room, real .shell outerHTML BYTE-IDENTICAL (21197==21197). One-toggle + docked + inline + zero-trace intact. control_ui.html only (6ins/5del). GHOST MODE 12b/50+. x3 parity
