...
VERSION = "v1310"   # 🔍 VAULT INTEGRITY CHECKER DEEPENING - now aware of the whole system (G3 + E1 + AI Checker). Two new finding classes on top of the v1281 five: 🔬 CHECKER-TOSS-KEPT (review-only) - an E1 muled item whose stored AI-Checker verdict is TOSS but its sitting kept in Magic & Rare, surfaced with the score ("keep only if you meant to"), a judgement call never auto-changed; 🤖 G3-VAULT-CONFLICT (auto-fixable) - a name the G3 sweep auto-filled into a proper tracker (d2r_g3Filled) that ALSO still sits in Magic & Rare, one-click Fix drops the redundant vault copy (its tracked elsewhere now) and names the exact tracker - the safe fix touches only the vault copy, NEVER the tracker. PROVENANCE UNIFICATION (the cohesion ask): every finding old+new now carries a provenance tag - ✋ manual · 🤖 G3 (d2r_g3Filled) · 🔬 checked (_mfChecker) - shown inline, so a fix is informed by how the item got there. Deterministic-first kept (Fix / Fix-all-safe for unambiguous incl g3-vault-conflict; review-only for judgement incl checker-toss-kept; TRUTHFUL - a clean checker-KEEP keeper never flags). Verified: 16 bible scripts clean; node 9/9 on crafted vault (checker-toss-kept review-only, g3-vault-conflict fixable+names tracker, provenance tags correct, clean keeper not flagged, apply removes vault copy WITHOUT touching the G3 tracker, checker-toss-kept not auto-fixable); backward-compatible (no g3Filled/no checker records -> exactly the v1281 audit). "One intelligent system, verified" - the vault checker cross-references G3, E1, and the AI Checker. bible.html only. x3 parity.
