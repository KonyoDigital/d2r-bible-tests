...
VERSION = "v1354"   # 🧾 RECEIPTS[] STREAM - the org backbone: every AI read now has a canonical, routable identity. The last big TV-D backend piece (Konyo: reference IDs so reads get labeled + routed). status.receipts = read-only DISPLAY projection off the journal tail (NO grail writes), mtime-cached like eyes/gate, newest-first, bounded <=30, empty off-air. Each receipt = {id (DETERMINISTIC engine:frameKey:seq - same row = same id every poll = self-dedupe), engine, kind, ts, refs:{itemName?/sessionId?/frameId?/area?/scene? - only present keys}, diablo:{label} (game-true WHERE: "FARMING - The Pit Level 1", "ROUTED runes x4", "WATCHDOG: ...") | null, route:{type,target} | null (item->itemName / session->sessionId / flag->rule)}. Grounding stays CLIENT-side (server DB-agnostic; client resolves itemName -> dossier if grounded else the Checker, never a fake dossier). Lane->receipt: deep->liveEye/read (one per item), verify->secondEye, kai->kai/judge (name from kai.judge.name), intake->router/route, watchdog->watchdog/flag; provisional/ocr rows skipped. Verified: py_compile OK, 16/16 receipts test (id-stability/dedup, lane mapping, bounded, honest-absent, server omits grounded), 156/156 control suite (real Handler end-to-end), invariant GREEN. Contract handed to sessions-visual for the bottom feed. control_app.py only. x3 parity.
