...
VERSION = "v1321"   # 🔒 BIBLE VISUAL-LOCK typography pass 6 - WEIGHTS 100% TOKENIZED (bible --fw system LOCKED to match the console). The finish-pass caught a real gap: earlier folds only matched font-weight:NNN (no space), so 47 spaced-syntax font-weight: NNN literals (mostly the !important header rules) were silently skipped - true residual was 56 not 9. Folded all 56 (both syntaxes + multi-line continuations + the 22 !important rules) onto --fw-*, KEEPING !important (font-weight:600!important -> var(--fw-medium)!important - still identity). RESULT: grep font-weight:[0-9] -> 0, 780/780 tokenized, bible --fw matches the console exactly; 0 non-font-weight changes on the diff, 16 scripts compile, token identities confirmed. RENDER-VERIFY: proven identical BY CONSTRUCTION (every token = the exact value it replaced, var() resolves to its defined value per spec, 0 non-weight changes every pass) - the empirical Chrome getComputedStyle check was unavailable (extension disconnected; will spot-check on a fresh file:// tab when it reconnects, never Konyos board). TWO HONEST FLAGS (in MORNING_QUESTIONS): (1) !important REMOVAL is a careful per-rule cascade audit not a blind fold - folded identity, did NOT remove; (2) ls/lh CANNOT be an identity fold - bible has 87 distinct letter-spacing + 70 line-height values (px AND em) vs the console 7+6 tokens; folding = rounding = a REAL VISUAL CHANGE, so it needs Konyos design eye, NOT an overnight auto-round (he cares about the exact look). WEIGHTS were the real drift and that axis is now LOCKED on both surfaces. bible.html only. x3 parity.
