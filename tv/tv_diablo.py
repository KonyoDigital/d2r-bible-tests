...
VERSION = "v1335"   # 📈 SESSIONS FLAGSHIP D29 - SINCE-LAST-SESSION DELTAS (sessions-visual; the .dsl CSS completing the strip - the D29 JS landed bundled in v1334). A "vs last run" chip strip under the dossier stats comparing this run to the one immediately before it by time: 👁 +360 · ⚡ +162 · 📖 +5 · 🏆 +2 · 📊 +11% (reuses the D14 metric derivation; each higher-is-better ▲ mint / ▼ red). HONEST: omits the whole strip on the FIRST recorded run (no prior) + any metric where either side is absent or unchanged. This round = the 6-line .dsl chip CSS (without it the strip rendered as unstyled text; the JS _dossierSinceLast + assembly were swept into v1334 by crossed cadence). Verified: session 7 (prior=6) -> "VS LAST RUN · 👁 +360 · ⚡ +162 · 📖 +5 · 🏆 +2 · 📊 +11%" every delta exact (reads 1120->1480, hr 386->548, found 6->11, grails 0->2, cov 79->90), all green (beat last on every axis); session 6 (first run) -> no strip (honest-absent). JS 0 errors, visual-lock GREEN. control_ui.html only (6/0). x3 parity.
