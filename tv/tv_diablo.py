...
VERSION = "v1371"   # SESSIONS R6 (cohesion, closes the hunt-hub arc) + autoroute tally honesty (bundled). SESSIONS R6 (control_ui, CSS-only): the tab now reads as three coherent colour-coded groups not gold cards floating under coloured banners - YOUR HUNT gold (Task Force hero+hierarchy, gold header+antique bar), DAILY MISSIONS mint (Forge Quests header/bar recoloured mint), YOUR RECORD info-blue (KPI cluster headerless-by-design + LAST SESSION + HISTORY blue). Semantic override flagged for veto: the LAST SESSION reel-digest was a lone mint accent stranded in the blue RECORD zone (already mismatched, gold header/mint card) -> recoloured info-blue to join RECORD; one-line revert if Konyo wants reel=mint kept. AUTOROUTE FIX (control_app, g3 round 9): a stackable with NO tally (intake=0) reported held=max(reads,1) = frame SIGHTINGS as quantity ("5 Hellfire Torch" from 5 sightings, never counted); now untallied stackable -> 1 ("seen, uncounted"), tallied unchanged (0 in the data have reads>intake). Sighting != quantity. Low-harm (autoroute sweep is review-gated + apply is merge-max) but honest - closes the tally-honesty audit; the display-stat fabrication classes (rate + tally) are now all clean. (Data note: Hellfire Torch mis-classed as material - its a unique charm - flagged separately, not fixed blind.) Verified: invariant GREEN, JS 0, no-scroll, 215/215 control, 181/181 routes. control_ui + control_app + test_control. x3.
