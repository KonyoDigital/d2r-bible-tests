...
VERSION = "v1288"   # 🎨 SESSIONS FLAGSHIP VISUAL PASS R1 - TYPE SYSTEM + SPACING UNIFICATION (sessions-visual agent; Konyo: "typography and alignment and structuring should be symmetric more for sessions.. flagship style .. this is the main page and its not looking like it"). The off-air MOTHERBOARD was built across ~40 rounds so every region had drifted ad-hoc values fighting each other: header letter-spacing declared .12em but forced .16em by an !important tail (two sources fighting); KPI hero numerals used a hand-rolled clamp - the ONLY numerals not on the --fs-* scale; six different gutters (12/10/8/10/10/7); card padding drift (.hd-col 11x14 vs .hd-engines 10x14); chip padding in an !important layer fighting the base. R1 FIX (type system + gutter/padding unification, zero layout-height risk): one dashboard spacing-token set in :root (--hd-gap 12 / --hd-gap-in 10 / --hd-gap-row 8 / --hd-pad-y 12 / --hd-pad-x 14 / --hd-head-mb 9 / --hd-head-ls .14em) = one rhythm one source; .hd-h headers resolve weight/spacing/margin from the shared vocabulary (dropped from the !important tail, settled .14em); KPI numerals tokenized to --fs-lg (on-scale, dominant numeral); every region gutter/padding references the tokens; removed the .hd-engines override + folded chip padding out of the !important tail. Verified: 1 inline script 0 errors, brace delta 0, both dash halves rendered on a throwaway static port (NOT :17772) with even gutters / consistent numerals / one header language. NEXT: R2 structure (matched card heights for side-by-side pairs, titled regions) + R3 column rhythm + the short-viewport transport overlap. control_ui.html only (27/16). x3 parity.
