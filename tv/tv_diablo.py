...
VERSION = "v1264"   # 🏦🔧 VAULT FIX - the CTA gold-dot bug (Konyo, live). Call to Arms (and any runeword) showed a stray gold diamond dot in the MAGIC and RARE locker instead of a proper tile in RUNEWORDS. ROOT CAUSE (bible.html aicJudgeApply keep path): when the AI item-judge recognized a forged runeword it forced tier=keep and dumped the runeword name into magicFinds (the Magic-and-Rare KEEPER track); renderVault renders every magicFinds key as a tile, and a runeword has no base art so it fell through to the gem-fallback gold diamond = the gold dot on the mule; and it was never added to owned, so its correct RUNEWORDS assignment got pruned every render, leaving only the stray dot. FIX (two edits in the vault JS block): (1) aicJudgeApply keep branch now adds a runeword to owned + routes it via suggestMule to the RUNEWORDS locker and keeps it OUT of magicFinds (non-runewords unchanged); (2) a one-time renderVault self-heal migration sweeps magicFinds for any runeword key, moves it to owned + its proper mule, deletes it from magicFinds and persists - cleaning Konyos existing bad CTA data and generalizing to every runeword (Enigma, Spirit, Insight...). Truthful (the item now lives in exactly ONE correct locker), light. The legit mule-assignment badge + normal Magic-and-Rare keepers untouched. Verified: v405 runeword-mule 4/4, v203 mule-vault + v345 magic-rare-art 13/13, and a repro spec confirmed CTA leaves magicFinds -> no gold-dot tile -> proper RUNEWORDS tile. bible.html only. x3 parity
