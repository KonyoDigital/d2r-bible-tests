...
VERSION = "v1297"   # 🟣 G4 TOUCHPOINT #1 - uncertain chronicle auto-route verify (first cheap Grok seam, inert until keyed). Hooks at SEAL time right after _kai_compile_register (the between-sessions path - NEVER a hot/live path), one fenced # GROK ADD-ON (G4) - touchpoint 1 block. Walks the register and calls _g4_verify ONLY for entries where tier==border (the low-confidence tier = uncertain auto-routes; confident grail/keep skipped), capped 10/seal on top of the module hourly budget - never bulk, a handful of calls per session at most and only when ON. DISAGREEMENT = FLAG, never override: on a real Grok disagree it attaches register_entry["g4"]={agree:false, verdict, note<=12w, ts, source:grok} and LEAVES the deterministic tier untouched; the flag rides into the sealed kai_report.json register for the v1303 "🟣 Grok caught this" review surface (which reads g4.agree===false). Agreements attach nothing (zero footprint). The flag does NOT ride into the Chronicle-inbox propose (stays name/tier only) - it lives in the report for the review queue. Verified: py_compile OK; OFF path byte-identical (0 entries touched, register deep-equal unchanged, no print/mutation); ON+disagree stubbed (border gets the flag, tier stays border, grail untouched, shape correct); one-grep removal intact (4 balanced fence pairs). Live path untested tonight (no key; morning-verified per MORNING_QUESTIONS #2). Uses tier==border as the register uncertainty signal (register carries tier not numeric conf; border IS the low-confidence tier). control_app.py only (one fenced block). x3 parity.
