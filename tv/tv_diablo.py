...
VERSION = "v1282"   # 🔎 SESSIONS FLAGSHIP D8 - SHELF SEARCH (polish-ui-2, UI) + D7 real-data honest-absent RE-VERIFY. D8: a live search box in the session-shelf header that filters the run cards AS YOU TYPE by item name / area / grail / session number / date / verdict (each card carries a data-search index) - plus quick-filter chips (grail / clean / flagged, toggle + active state), a live "N of M" count and an honest "no runs match" empty state. The query PERSISTS across the 12s poll re-renders. CRITICAL containment fix: keystrokes in the search box no longer fire the theatres single-letter shortcuts (typing "stash" used to trigger s = toggle-shelf); Esc clears then blurs. D7 RE-VERIFY on real data (:17980): the seal-latency chip + regret spotlight both correctly render NOTHING on history (sealMs all rejected as reseal timestamps; zero thrown_names), the dossier still fully renders (D6 badge intact), no errors - they light up on a fresh run / real regret. demo 7/7 (:17980), JS/brace/tests green, no page errors, D8: grail->10 of 107, spirit->6 runs, no-match state, chip filter + keydown containment all PASS. control_ui.html only (59/3). x3 parity.
