...
VERSION = "v1325"   # 🗺 B4 DIABLO-LANGUAGE - game-true scene labels (engine; Konyos console-accuracy arc; the code landed bundled in v1324, recorded properly here). Investigation: the reader emits raw scene (town|stash|inventory|loot|gameplay|transition) + area per deep read; v1253 R1 already carries the nearest scene/tab/area onto name-less routing frames - but the raw scene was never turned into the label a PLAYER would say (a transition frame read "transition" not "ENTERING <area>"; town-vs-farming safe-vs-drops was never distinguished - gameplay was the catch-all, town depended on a model guess). BUILT the deterministic layer: _diablo_scene_label(scene,area) + _TOWN_AREAS (the 5 act-town safe zones), pure/module-level, turns (scene,area) into: transition/loading -> ENTERING <area> (or "ENTERING (loading)" honest when the frame is too dark to read an area); open panel stash/inventory/loot -> STASH/INVENTORY/LOOT <area> (panel wins over town context); town by _TOWN_AREAS OR scene=town -> TOWN <area> (safe, deterministic where a guess isnt); gameplay in a non-town area -> FARMING <area> (drops); no scene+no area -> "unclear" (never invents a location). Wired ADDITIVELY into _kai_reconciles routing rows as a `native` field {kind:entering|town|farming|menu|unclear, label, area} alongside the existing scene/tab/area - sessions-visual renders native.label on the live "now" banner + the session timeline ("🏛 ENTERING The Pit" / "⚔ FARMING · Chaos Sanctuary" / "🛡 TOWN · Harrogath" instead of "gameplay/near-black"). Verified: py_compile OK, 15/15 helper cases (ENTERING portal->area + dark->loading, TOWN-vs-FARMING split, panel-wins-over-town, case-insensitive town, honest unclear; the test caught + fixed a real panel-vs-town ordering bug). HONEST BOUNDARY: this LABELS what the read faithfully said; upgrading a dark loading frame the reader called gameplay->ENTERING needs a reader-prompt (tv_diablo.py) transition-detection round (complementary B5/B7). control_app.py (+ comment version-correct). x3 parity.
