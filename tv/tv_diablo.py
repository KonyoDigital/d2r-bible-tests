...
VERSION = "v1260"   # 🖥🔧 CONSOLE group C - smoke-test visual bugs fixed (from polish-ui-2 own synthetic-item pass; the pipeline passed all 6 stages, these were the found bugs). C1 CAPTURE PILL no longer wraps ("Locked · Diablo II" was wrapping "II" at <=1728 rail): WATCHING row is now a column (label over pill) + nowrap; bonus shorter rail exposes the Inbox button above fold. C2 KAI EYE STATUS clamped 3->2 lines (full on hover). C3 FORGE chip reads forge-ready from fs.now.length (same source as FORGE QUESTS + Task Force) so they never disagree (was fs.counts.now showing 0 while quests showed 2). C4 FUNNELS tab count verified-correct (keys off object shape; the smoke-test synthetic was a string) - no change. Held the 0.00px head-tab invariant. control_ui.html only (16/2). CONSOLE group C. x3 parity
