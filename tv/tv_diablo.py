...
VERSION = "v1272"   # 👻🗑 GHOST MODE RETIRED (Konyo: broken on click + redundant with the Sessions flagship - do i even need it? no). Removed the WHOLE debugger overlay: the header ghost switch + CSS, body.ghost-on / #ghost-shell / gsh-* (clone/dock/teardown/scrubbers), the grail-journey (_erGj/_grailAtT), the ENTIRE Engine-Room subsystem (_engineRoom*/_er*/.er-*/#engine-room), _erOwnerVerdict (A5 - its whole concern was live-leak INSIDE the ER during scrub, moot now the ER is gone), and the hdRefresh _engineRoomRefresh call. -1355 lines (6/1361). Sessions never called any of it (verified). PRESERVED: shared shell-fade/rise + reduced-motion + endurance rules for their non-ghost selectors, engine-HEALTH .eh-* dashboard, the tab-rail media queries. ZERO remaining ghost/ER refs, 0 console errors, clean header reflow, demo 7/7 (J2 0.00px re-verified with the switch gone), home/Sessions D1-D3/KAI READER/live pulse/KAI GATE all intact. The grail journey (the one cool piece) comes back FRESH in the Sessions tab from d2r_foundLog per the roadmap. control_ui.html only. Simpler console, broken toggle gone. x3 parity
