...
VERSION = "v1202"   # 🩹 FUNNEL round 6 — intake-lease CLOCK-SKEW (leaks a busy-tab lock): _intake_lease_claim/_status compared the lease deadline `until` (wall-clock time.time()*1000) directly in 3 expiry checks. A backward NTP/sleep-wake jump between claim and check makes `now` read smaller → an already-expired lease (past its 120s TTL) reads still-held for the jump size, leaking a "busy" tab lock for minutes. Complication (as anticipated): `until` IS surfaced cross-process — /intake_claim ships it to board JS, /api/status ships it to the theatre — so it can't just become monotonic. FIX (same split as v1201's _t0f/_t0f_mono): keep `until` wall-clock for the returned/displayed field; add internal-only `untilMono` driving the actual expiry comparisons (never returned — status snapshot builds {owner,until,since} only). +7 REAL behavioral tests incl the regression (control 103→110). ×3 parity (30/80 → v1252)
