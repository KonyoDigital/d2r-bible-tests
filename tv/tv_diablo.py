...
VERSION = "v1309"   # 🎬 SESSIONS FLAGSHIP D18 - CINEMATIC SHELF CARD REDESIGN (sessions-visual, Phase C). Each run card now reads like a film-reel still / collectible card (within the Diablo-broadcast identity, all on the token system, no new palette): cover-art hero (the runs reel thumb w=440, full-bleed + top/bottom scrim, gentle hover zoom); a broadcast lower-third over it - session title in Cinzel serif (top-left) + a verdict seal chip (🛡 clean / 🚨 N flagged / ◌ open, top-right); the headline find at the heros base (the runs top grail, gold-lit = the money moment); a clean 4-cell stat grid (reads · found · 🏆 grails · coverage%); then verdict/story chips + scene fingerprint. Rarity accent: gold border+glow on a grail run (.grail), mint ring for the current reel, desaturated for retired ghosts. MACHINERY PRESERVED (card redesign, not shelf surgery): every filter/sort/search data-attr intact (data-n/grail/verdict/t0/t1/reads/found/search) + tap-to-dossier works; scoped strictly to #th-shelfov .sh-card (home history cards untouched). Verified on the real shelf w/ 4 mixed runs (grail/flagged/clean/ghost): gold accent on grail run, seals correct, headline grail shown, stat grid exact, all data-attrs present, tap->dossier, JS 0 errors brace 0. DESIGN CALL: dropped the inline finds-strip in favor of the hero headline-grail + 🏆-count (cleaner/Maxroll-like; full item list one tap away in the dossier; searchable index still has every item - search unaffected) - logged for Konyos veto. A few now-dead local vars flagged for the D31 cohesion pass. control_ui.html only (75/22). x3 parity.
