...
VERSION = "v1366"   # 🔬 FORENSICS HONESTY SPLIT (② from the unresolved-103 hunt) - the X-ray stops scaring. The scary "unresolved 103" was NEVER 103 missed items: proven 0 grail misses + screen-text/noise. BACKWARD-COMPATIBLE (no break for the shipped v1361 surface): status STAYS "unresolved"; added unresolvedKind ("non-item" | "unreadable-item") sub-field + summary counts grailMisses/screenText/unreadable. Verified vs 29 reels: unresolved 103 -> grailMisses 0 · screenText 37 · unreadable 66 (37+66=103). grailMisses:0 PROVEN by construction (an unresolved read resolved no item, so no grail the AI IDENTIFIED was dropped). Conservative detector (transition banners · short-UI · pure-noise · fuzzy cube/consumable prompts -> non-item; ambiguous garble -> unreadable-item, never over-claiming). No grounder change - pure reclassification labeling, zero recovery/count risk. (Honest note: a transition banner near-matching a grail sig lands blocked-fp = correctly-refused, not unresolved - the existing block detector, honest.) Verified: 5/5 ② + forensics 8/8 + 202/202 control + 181/181 routes + invariant GREEN. Contract delta (additive) handed to sessions-visual for the "0 grail misses" reassurance line. control_app + test_control. x3.
