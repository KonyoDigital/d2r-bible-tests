...
VERSION = "v1265"   # 📼🏆 SESSIONS FLAGSHIP D1 (backend) - WHAT I FOUND: expose the register finds per session so the Sessions tab can finally show the grails KAI witnessed each run. _theatre_sessions now reads register.items alongside the count and adds "finds" (capped 16, card fields {name,tier,loc,frameId,ts}, sorted grail-tier-first then newest) + "topFind" {name,tier} to /api/sessions. TRUTHFUL: set only when a sealed kai register with items is present; no register (unswept/old) -> both null, never fabricated; swept-empty -> registered:0 no finds. REAL-DATA proof: the Enigma reel s_1784580368648 (7-item register) now carries registered:7, topFind Hellfire Torch/grail, and finds with the 3 grails sorted ahead of the 4 rares, each with frameId for jump-to-frame. Floor 536 (+4). control_app.py + tests (NOT control_ui.html). Sessions flagship spine begins. x3 parity
