...
VERSION = "v1299"   # 🟣 G4 TOUCHPOINT #2 - borderline Item-Checker keep/toss verify (2nd cheap Grok seam, inert until keyed). Hooks in the /kai_verdict POST handler right after the judge rec is built (the receipt path where every aicJudge keep/toss lands with its score), one fenced GROK ADD-ON block. Band anchored to the aics REAL cutoffs (_aicVerdict: score>=14 keep, >=7 border, else toss) so it fires ONLY on the near-cutoff calls a single point would flip: a keep scored 14-16 (barely cleared) or a toss scored 5-6 (just under the border line). Confident keeps>=17, clear tosses<=4, border, and grail all skip Grok - no overlap with seam 1 (border) or seam 3 (grail). FLAG-NOT-OVERRIDE: a real disagreement attaches rec["kai"]["judge"]["g4"]={agree:false,verdict,note,ts,source:grok} and leaves the tier untouched; the flag rides into the journal sessions.jsonl kai-judge rows. v1303 review surface now reads TWO sources for g4.agree===false (seam1 register in kai_report.json + seam2 kai.judge in sessions.jsonl); the toggle cards live "🟣 N flagged" (stats.disagree) already reflects every touchpoint. Verified: py_compile OK; band predicate 12 cases PASS (keep 14/15/16 fire, 13/17/20 dont; toss 5/6 fire, 0/3 dont; border/grail never); OFF byte-identical (verify None -> no g4 key, rec clean); ON+disagree flags with tier UNCHANGED; ON+agree + confident-keep no flag; one-grep removal intact (5 fence pairs). Live untested (morning). control_app.py only. x3 parity.
