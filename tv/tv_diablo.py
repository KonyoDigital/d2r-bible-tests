...
VERSION = "v1291"   # 🔄 G3 UNIFIED AUTO-ROUTE/TALLY - the sunder-tally back-fill, review-gated (Konyo: "the AI read my sunder charms dozens of times.. tally those.. properly tallied and routed individually and surgically"). ENGINE (control_app.py): new read-only GET /api/autoroute-sweep + _autoroute_aggregate/_classify/_sweep_cached (memoized). Aggregates the INTAKE lane (not just register - which is exactly WHY the trackers looked empty despite dozens of reads: register misses intake tallies), MAX-of-snapshot not SUM, essence display-names folded. Every non-stackable ships as a `candidate` for the browser to classify against the LIVE ITEM_REGISTRY (closes the static-vocab undercount). Real-journal output: sunders 4/6 {Crack 10 Rotting 9 Bone 6 Flame 2 - Black Cleft/Cold Rupture NEVER seen, omitted}, runes 32/407, gems 33/695, materials 20/237, statues 5/5, 74 candidates. UI (bible.html): "🔄 Auto-route Sweep (KAI)" panel in Tools - Scan fetches + classifies candidates against live vocab (unique/set/runeword guarded by RW_BASES else unclear), computes the MERGE-MAX diff vs current trackers, shows only CHANGED rows grouped by tracker; Apply (disabled till a change exists, uiConfirm-gated) MERGES MAX (never lowers/removes), persists, writes provenance d2r_g3Filled (by:g3) so Konyo sees "🤖 KAI filled"; the "⚠️ couldnt classify - review manually" list (RotW bases/class items/rolled magic-rare -> the Item-Checker->vault path per Konyos architecture) is shown and NEVER applied. TRUTHFUL, nothing writes before the click. Verified: py_compile OK; endpoint JSON-serializable on real data; 16/16 bible scripts clean; MERGE-MAX proven by extracting the REAL functions + running 13/13 in node (Ral x50 stays 50, Rotting x12 stays, Crack seeds 0->10, Black Cleft never appears, found SoJ + made Enigma not re-added, live-vocab recovers Harlequin Crest + Bul-Kathos Wedding Band). Live HTTP boot skipped (touches D2R PID state - unsafe unprompted; the 8-line route wraps already-tested fns). control_app.py + bible.html. x3 parity.
