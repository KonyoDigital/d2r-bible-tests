...
VERSION = "v1210"   # 🧠🖥 ENGINE ROOM arc round 1 — READ-CHAIN STRIP in the per-frame drill-down (the north star: "SEE what the AI sees, who owns each read, why"). A fixed 5-node pipeline 🔴live→🔵second→🧠KAI→🧠🔬super→🚦gate: LIT node = that layer fired this frame · GOLD-RING "OWNS" = the layer owning the final read (from the Phase-D reconciler's authoritative owner). Per-layer color identity. Live-node fire aligned to the SAME item-read inference as the owner row (no contradiction — rune tally reads honest all-dark, item read lights live+owns, KAI-caught miss = live-dark+KAI-owns, super recovery lights live+KAI+super w/ SUPER owning). Companion: widened detail label col (no 2-line wrap at fullscreen). control_ui.html only, reads existing beat/liveRing fields, no new endpoints. ×3 parity (38/80 → v1252 → +20 ER → v1272)
