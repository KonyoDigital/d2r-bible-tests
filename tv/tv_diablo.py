...
VERSION = "v1267"   # 🏆📼 SESSIONS FLAGSHIP D1 (UI) - WHAT I FOUND cards (the flagship moment). The real grails KAI witnessed each session now render as premium gold-glowing cards on the shelf card (cap 6) + the home last-session digest (cap 4): HD art (thArtImg) + name in its KAI-tier colour + grail mark + subtle loc, under a "What I found" lead, topFind first, "+N more" from registered when capped. CLICK a find -> jumps the Theatre to the EXACT frame KAI saw it (_thJumpToFind: thOpen+thLoadSession+thGo to the beat matching frameId, ts-nearest fallback; handles both compact N_ts and footage reel/f_ms shapes). VERIFIED: clicked grail Annihilus in session 37 -> Theatre landed on f_1784580484660 (beat 53/59). TRUTHFUL: renders only when the reel has real finds -> honest-absent otherwise (verified the home digest showed nothing for a no-finds reel, not a fake 0); registered null falls back, never fabricates. Real art system (thArtImg/_HDART, added to thArtRepaint). LIGHT (12s poll, lazy art). Coloured by KAI tier (grail gold/keep mint) since the console has no item-rarity DB - true D2 rarity colour would need a backend rarity field per find (flagged). control_ui.html only (85/2). SESSIONS FLAGSHIP D1 complete - the main tab now answers what did I find. x3 parity
