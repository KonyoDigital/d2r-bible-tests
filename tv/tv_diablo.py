...
VERSION = "v1274"   # 📊 SESSIONS FLAGSHIP D4 - FARMING-PRODUCTIVITY KPIs (polish-ui-2). A Maxroll-grade instrument cluster at the top of the OFF-AIR dash: 6 stat tiles (big tabular-mono numerals, sentence-case labels) - Sessions today, Reads/hr (mint accent), Grails found (gold accent), Tallies banked, Frames/run, Named/run. ALL client-side aggregates off HD_ALL (the /api/sessions poll) - no new fetch or timer, rides the 12s poll like D3 (LIGHT). TRUTHFUL: real aggregates verified against raw /api/sessions (grails tile 24 == raw grail-find sum 24); reads/hr = total reads / sum(t1-t0) session-hours (reading rate during ACTUAL session time, not wall-clock), guarded (needs >0.02h and reads>0 else dash), no divide-by-zero; honest empty state = all six tiles muted "-" on a fresh account, never a bright fake zero; auto-compact numerals (1,284 / 12.9K). Off-air only (hidden on-air with the other telemetry cards so the live view stays compact). Two flags carried to D-rounds: (1) only "sessions today" is today-scoped, the other five are lifetime aggregates (a today<->lifetime toggle is a candidate round); (2) the row adds ~64px to a dash that already overflows on short viewports - a real one-viewport fix is a dash-density pass. demo 7/7 (J2/J4/J7), JS/brace/tests green, no page errors. control_ui.html only (53/1). x3 parity.
