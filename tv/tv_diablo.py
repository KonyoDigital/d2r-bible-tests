...
VERSION = "v1277"   # 🔬 SESSIONS FLAGSHIP D6 - SUPER-RECOVERY BADGE + MISSED-TEXT DRILL (polish-ui-2, UI) + D5 real-data RE-VERIFY. D6: a brain-lens badge on the dossier decision section - "recovered 7/9 missed reads - 78%" (mint win-tone when recovered>0) that DEGRADES gracefully - with no recovery ledger it shows the existing kaiMissed count ("N missed-text frames"), and "no missed text - a full read" at zero, never faking; plus a MISSED TEXT drill section, one tappable row per missed frame -> jump-to-frame (reuses _thJumpToFind), honest "unreadable text" when none, omitted until the engine provides the list. D5 RE-VERIFY on a throwaway :17980 against the new /api/sessions (Konyos :17772 never touched): coverage meter populates with REAL data and honestly shows the low old-reel coverage (session 1: 15% - 5/33 - 28 gaps, amber, NOT nudged upward), montage populates with real thumbs (naturalWidth 2940), funnel honest-empty on 0-read reels. Bundled polish: a scene-label resolver so the richer report vocab renders clean - stash-gems/materials/runes -> GEMS/MATERIALS/RUNES (coloured as stash), tooltip -> ITEM, never raw. ENGINE FOLLOW-UP (v1278, team-lead): per-session superRecovery:{recovered,missed} + missedFrames:[{frameId,ts,label?}] from the reel reports missed ledger to light the R/M% + drill (badge already live via kaiMissed fallback). demo 7/7 (J2/J4/J7 via :17980), JS/brace/tests green, no page errors, D6 injected-data PASS. control_ui.html only (62/5). x3 parity.
