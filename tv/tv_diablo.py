...
VERSION = "v1213"   # 🧠🖥 ENGINE ROOM arc round 4 — ALWAYS-VISIBLE KEY (cockpit now self-documenting). Rounds 1-3 made ownership/retro-position/health legible, but the glyph language was only decoded INSIDE the drill-down — on the always-visible spine/timeline you still had to already know 🔴=live 🔵=second 🧠=KAI, green=healthy/amber=strained. FIX: a compact always-visible KEY footer decoding both systems in one quiet line — READ CHAIN 🔴live·🔵second·🧠KAI·🧠🔬super→🚦gate | HEALTH ●healthy·●strained·●down·●idle (health dots use the EXACT pulse colors, so one legend decodes both the corner-dots AND the r3 pills). Styled subtle (dim, --fs-2xs, faint top-border) so a power user skips it + a returning user reads it. Static markup → present in all 3 states (live/off-air/retro), aria-hidden. control_ui.html only. ×3 parity (41/80 → v1272)
