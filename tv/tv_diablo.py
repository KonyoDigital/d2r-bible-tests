...
VERSION = "v1253"   # 🖥📏 CONSOLE UPGRADE round 5 - THE RAIL FIT (finishes the home restructure). The right rail was overflowing (content-volume ~1333px). Compacted TRUTHFULLY (no real signal deleted): (d) the capture lock-on pill moved UP into the KAI READER cluster under the eyes (above the fold, where "what the eyes point at" belongs); (a) folded the duplicate rows (Last-thought / reads-bar / mind-foot duplicate the header ON-AIR indicator + the AGENT MIND ticker - hidden, kept in DOM for compat; kept Signal + capture pill), ~90px; (b) SESSION HEALTH 186px 4-line card -> tight inline strip (tabs · lease · re-fires + OK verdict pill), ~half height; (c) Theatre+Shelf secondary controls trimmed to single-line, primary ON AIR stays full/prominent; KAI READ FEED resting note one line. RESULT: rail 1333px -> ~1120px, fits no-scroll at 2560x1440 (Konyo fullscreen); at 1728 all important content (KAI READER eyes+pill+feed + Session Health) above the fold. Kept the v1249 KAI READER prominent (did NOT shrink the eyes, per guardrail). Head-tab 0.00px invariant held (shell padding untouched). LIGHT (CSS + DOM moves). control_ui.html only (28/14). CONSOLE UPGRADE 5 - HOME DONE. Next: the DIABLO-LANGUAGE accuracy arc. x3 parity
