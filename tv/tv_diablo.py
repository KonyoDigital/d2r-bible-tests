...
VERSION = "v1295"   # 🟣 G4 GROK - removable scaffold (Konyo: cheap fingers in a couple of places, ON/OFF, "implement it to be taken out eventually"). INVESTIGATION: no existing runtime Grok in the app (every "Grok" in the code is a review-credit comment; grok-mcp is a separate project) - G4 is the apps FIRST live Grok path, clean slate. The verdict ladder gives natural only-fire-on-uncertain gates (tiers grail/keep/border/toss, numeric routeConf/confidence, the /kai_verdict grail gate). 3 CHEAP SEAMS chosen: (1) uncertain chronicle auto-route (fire only tier=border OR confidence<2); (2) borderline Item-Checker keep/toss (only the narrow score band around the cutoff); (3) grail-promotion re-check (only when the grail gate promotes a generated/rare-combo name - the split-brain class). All uncertain/important-only, never bulk. SCAFFOLD: new tv/g4_grok.py = the ENTIRE feature, self-contained (stdlib only): is_on() = switch AND key (switch = env TV_G4_GROK=1 OR tv/g4_grok.state file; key = XAI_API_KEY/G4_XAI_KEY from env only = cousin-safe), DEFAULT OFF; g4_verify(context) = the one entry point, returns None INSTANTLY when off/unkeyed/over-budget/error (credit guard G4_GROK_HOURLY_MAX=40). control_app.py: ONE marked block (# GROK ADD-ON (G4) .. # END) - optional import (absent file = feature simply absent, never an error) + _g4_verify/_g4_status shims + 2 fenced routes GET /api/g4_status, POST /api/g4_toggle. NO engine touchpoint calls _g4_verify yet (scaffold only). REMOVAL SEAM (zero scars): delete g4_grok.py + the marked blocks; grep "GROK ADD-ON|g4_grok|_g4_verify|/api/g4" finds every trace. .gitignore excludes g4_grok.state (never inherit an ON state). Verified: py_compile OK both files; OFF-by-default proven (clean env -> is_on False, g4_verify None, ZERO network); toggle-on without key stays OFF; feature-absent path clean; BYTE-IDENTICAL to today (nothing calls it, endpoints additive+inert). control_app.py + tv/g4_grok.py + .gitignore. x3 parity.
