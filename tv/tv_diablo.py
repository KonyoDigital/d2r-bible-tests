...
VERSION = "v1216"   # 🧠🖥 ENGINE ROOM arc round 7 — THE FRAME IMAGE (the north star: SEE what the AI saw). The drill-down showed everything the AI THOUGHT about a frame (read chain/owner/gate/OCR/quorum) but not the frame ITSELF. FIX: the actual captured D2R frame is now the HERO at the top of the RAW PROCESS drill-down, above the read-chain + verdict — Konyo sees the exact frame (map/character/orbs/item labels) next to what each engine made of it. Because r6 re-renders per frame-step, the image UPDATES as he steps ◀▶ — watch the actual frames go by with the reads on each. Click → full-res in a new tab. Uses the EXISTING /hist/<frameId>.jpg?w=560 endpoint (frame images already served; no new endpoint). Degrades gracefully (unarchived frame → hides image + "📸 not archived", same onerror pattern as tally thumbs). control_ui.html only. FUTURE data-surface noted: read-region bbox overlay needs per-read pixel coords (not in journal today). ×3 parity (44/80 → v1272)
