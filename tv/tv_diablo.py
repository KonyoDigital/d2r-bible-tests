...
VERSION = "v1304"   # 🟣 G4 credit-aware rate-limit + selective per-seam config (never burn credits). tv/g4_grok.py: a DAILY ceiling G4_GROK_DAILY_MAX (default 300) alongside the hourly G4_GROK_HOURLY_MAX (40), both from one 24h ring - a call fires only if under BOTH. Per-seam selective config G4_GROK_SEAMS (default all 3): set it to run only what he wants, e.g. G4_GROK_SEAMS=grail-recheck = only the highest-value cheapest seam; aliases accepted (chronicle/keep-toss/grail). g4_verify consults context.kind and no-ops (NO network) for a disabled seam, so selection needs zero touchpoint changes. Budget surfaced in status() -> budget:{hourlyUsed,hourlyMax,dailyUsed,dailyMax} + seams[], and the toggle cards ON-active line reads e.g. "Grok double-checks only the uncertain calls (grail-recheck). Model grok-4-latest · this hour 3/40 · today 12/300 · 🟣 2 flagged." Self-contained (no touchpoint edits, removal seam untouched). + a tv/G4_GROK_REMOVAL.md doc listing every piece to delete. Verified: py_compile OK; default seams all 3, G4_GROK_SEAMS=grail -> only grail-recheck, aliases resolve, hourly cap 39ok/40blocked, daily cap 300 blocked even at 0 hourly, seam-gate returns None with NO network for a disabled seam; UI node 5/5 (active line shows hour+today usage, active seam, flagged count), 16 bible scripts clean; OFF still byte-identical. tv/g4_grok.py + bible.html. x3 parity.
