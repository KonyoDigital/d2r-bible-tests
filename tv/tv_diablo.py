...
VERSION = "v1197"   # 🩹 FUNNEL round 4 — routed-mark requires ok (sequel to v1185): _kai_build_routing set funnel_by_fid[fid]="kai-funnel" for ANY kai-funnel receipt with no ok check. But kai-funnel receipts CAN be ok:false (v1185 honest-miss-on-rejection + the never-zero guardHeld block). So a FAILED attempt got marked "routed" → _kai_reconcile narrated it as the accepted read ("its tally count is the accepted read") and _kai_stage3_select permanently skipped re-selecting the frame — silently undoing v1185's honesty one layer up (audit-trail bug; the real tally path gates on _intake_is_real, unaffected). FIX: gate the routed-mark on ik.get("ok"). judge_fids left alone (quorum-vote question, own ping-pong retry). +3 tests (control 93→96). ×3 parity (25/80 → v1252)
