...
VERSION = "v1376"   # 🌉 GRAIL-FARM BRIDGE gated (g3s round 13) - the flagship hunt-priority pipeline end-to-end: bible odds -> bridge -> /api/evrank -> NEXT GRAIL hero. bible.html _writeGrailFarm() reuses the F-Uniques Calculator (effChance/hoursFor at current MF/players) and exports each MISSING grail best-source to localStorage d2r_grailFarm [{name, source, dropChance=1/N, killsPerHr}], missing-only, no-source omitted, writes only on change, fork-aware (added to _LP_FORKED beside d2r_forgeSummary). Refresh hooks: boot (deferred), toggleOwned (found/unfound), persist (MF/players change). control_app _ev_hours now CEILs runs to match the bible runsFor exactly, so the flagship hero and F-Uniques show ONE number for the same grail. The console reads d2r_grailFarm (same as d2r_forgeSummary), POSTs {items} to /api/evrank, renders ranked[0] as the hero + the ordered hunt list (unranked = no known farm, honest). Verified: node harness 7/7 (missing-only, min-hours source, dropChance=1/chance, found-excluded, no-source-omitted, POSTs+ranks), bible inline JS compiles, control 226/226, routes 181/181, agent 199/199, x3 parity. bible.html + control_app.py + test_control.py.
