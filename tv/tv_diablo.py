...
VERSION = "v1222"   # 🧠🖥 ENGINE ROOM arc round 13 — KEYBOARD DISCOVERABILITY (advertise the power controls). Rounds 6+8 wired ←→ (frame-step) and [] (jump-to-flagged), but they lived ONLY in hover-title tooltips — a new/returning user could never guess them. FIX: visible keycaps at each keyboard-driven control — the frame nav (◀ N/M ▶) carries a subtle "← →" keycap pair, the flag-jump (⏮/⏭) carries "[ ]", styled as little keyboard keys (raised bottom border), dim so they inform without shouting, each with a hover title spelling it out. Now the shortcut is advertised right where you'd use it — no docs. Purely presentational (keys already wired r6/r8, still fire). control_ui.html only. FUTURE candidate flagged: matched-name (raw OCR → DB name) as its own round with the name-DB confirmed reachable. ×3 parity (50/80 → v1272)
