...
VERSION = "v1248"   # 🖥🔄 CONSOLE UPGRADE round 2 - THE TAKEOVER (Konyo: "the console is already open"). Fixed the double-click friction: TV DIABLO.app runs control_app.py --open, which tried to BIND :17772; when the supervisors always-up HEADLESS console already held it, the bind raised OSError and the old code REFUSED (notification "already open") and exited WITHOUT a window - so a double-click gave nothing. Now on bind-fail with --open and NO real window present, it ATTACHES a window to the running server (window-only takeover) instead of refusing; only a genuine live window keeps the v781 one-window refuse. Window-presence is tracked by a self-healing pid lock (.tvd_window.pid, gitignored; a dead pid reads absent so a crashed window never wedges the takeover). Also GUARDED the engine-driver/kai-closer start on not _WINDOW_ONLY so a takeover/attach never spins up a SECOND driver (the port-owning primary owns it) - fixes a latent double-driver in the pre-existing --window-only path too. The launcher git-pulls before run, so the takeover lands the moment Konyo double-clicks after this ships. control_app.py + tests (NOT control_ui.html). Floor 325. CONSOLE UPGRADE 2. x3 parity
