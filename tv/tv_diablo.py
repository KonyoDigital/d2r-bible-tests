...
VERSION = "v1319"   # 🎨 BIBLE VISUAL-LOCK typography pass 4 - CSS-SIDE WEIGHT TOKENIZATION COMPLETE. Folded every remaining single-line CSS weight literal (the non-header component bodies): stat values, table cells (td.rare-1/.best-cell), buttons (.sim-btn/.shard-act/.statue-check), h2/h3, ranks/odds/hrs, .mission-*, .top-drop-*, .mw-*, boss-detail & tool-card bodies, vault/TV·D bodies - 386 weight literals across 386 single-line rules onto --fw-*. MILESTONE: CSS side fully tokenized - count conserved 508 -> 122 raw + 611 tokenized (122+611=733). The 122 remaining are NOT CSS single-line: 113 JS-string inline weights (pass 5) + 9 CSS multi-line edge cases (hand-check, not blind-fold). Token distribution (all identity): 248 semibold(700)/171 medium(600)/150 bold(800)/23 regular(400)/12 normal(500)/7 black(900). PROVABLY SAFE: every changed line a font-weight identity swap (0 non-font-weight changes on the diff), each token equals the literal -> byte-identical render, 16 inline scripts compile, scoped to single-line rules (JS-string + multi-line untouched). No visual inconsistencies, no broken rules, no !important fights in the CSS bodies. Residual 122 (733->728->697->508->122). bible.html only. x3 parity.
