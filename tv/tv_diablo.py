...
VERSION = "v1238"   # 👻🏆 GHOST MODE arc round 9 - JOURNEY TICK STORYTELLING (pure-read from d2r_foundLog, real names + real dates, zero fabrication). THE JOURNEY now narrates your actual hunt, not just a climbing number: (1) the marker NAMES the grail(s) that landed at its date ("Jun 15 landed Blackhorns Face, Blackleach Blade +2 more" / "Jun 5 found Wolfhowl"), in the purple Time-Machine voice; (2) busy days grouped honestly - newest-first, up to 2 names then "+N more" (a huge day rendered +26 more), real counts; (3) HOVER preview - moving across the bar pops a purple tooltip naming the find nearest the cursor without moving the committed marker, so ticks are inspectable at 1px; (4) honest gaps - before the first find reads "no grails found yet at this point - drag right to watch them land", never invents a find; (5) un-tick caveat unchanged. foundLog key is the display name (same-origin), verified against Konyos ACTUAL found-log (hundreds of real grails narrated across the season). Truthful, pure add-on, GHOST-off = zero trace (tooltip lives only in the hidden overlay, never on the shell; outerHTML byte-identical). control_ui.html only. GHOST MODE 9/50+. x3 parity
