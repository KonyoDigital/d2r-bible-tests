...
VERSION = "v1284"   # 🔃 SESSIONS FLAGSHIP D11 - SHELF SORT CONTROL (polish-ui-2, UI). A compact sort selector in the shelf search row: Newest / Longest / Most found / Most reads. COMPOSES with the search + filter bar - it reorders the whole card set while the filters hide the rest, so you always see the filtered result in the chosen order. Reads sort keys off the card data-attrs (data-t0/t1/reads/found) - no re-derivation, pure UI, no engine. Longest = duration (t1-t0) desc; Most found = finds desc; Most reads = reads desc; ties break by newest; Newest (t0 desc) is default. Sort state persists across the 12s poll; the selects arrow keys navigate options only (keydown contained, never drives the theatre transport). Now the shelf has the full search + filter + sort trio composing together. demo 7/7 (:17980), JS/brace/tests green, no page errors; probe: newest=t0 desc, most-reads desc (32), most-found desc (11), longest=duration desc, composes with grail filter. control_ui.html only (34/0 additive). x3 parity.
