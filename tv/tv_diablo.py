...
VERSION = "v1219"   # 🧠🖥 ENGINE ROOM arc round 10 (ARC HALFWAY) — THE LIVE READ FEED (the "live time" half of the north star). The retro side is deep (nav/image/spotlight/scorecard) but the LIVE on-air side was thin — the spine showed live health + the readout a summary, but Konyo couldn't WATCH the reads flow. FIX: when ON AIR, a mint panel below the readout shows the last few reads as they land (newest first) — each a card with the actual frame thumbnail (/hist/), who owned it (🔴live·🔵second·🧠KAI·🧠🔬super), the verdict (🏆grail/✓keep/◐border/🗑toss), and the why. Refreshes every live poll → reads visibly flow in real time. Symmetric to r9 (retro→gold scorecard, live→mint feed); degrades to "watching — waiting for the first read…". State-exclusive: ON AIR only, hidden idle + hidden the instant you scrub retro. Uses status.liveRing (the reconciler's read ring, r1 fields f/owner/verdict/why; no new endpoint). control_ui.html only. ×3 parity (47/80 → v1272)
