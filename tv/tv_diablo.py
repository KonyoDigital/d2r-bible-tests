...
VERSION = "v1358"   # 🔍 E4 UI (closes sealed-vs-live honesty) + E1 two-witness grail grounding (Grok audit, evidence-backed). E4 UI HALF (control_ui.html): the theatre scrub can no longer show a live GUESS as a sealed VERDICT - consumed beat.engineFrame (was 0 refs), owner now keyed on engineFrame PRESENCE + sealed===true (live->red/second->blue/kai->brain/super->brain-sci/ocr/funnel), NOT owner-truthiness; honest-absent = a beat-derived owner shown VISIBLY PROVISIONAL (dashed/muted, "- provisional" tag + "sealed reel wins in retro" tip), never mistakable for sealed, NEVER falls back to st.liveRing; frame-level 🛡 sealed / ◌ provisional tell. 6/6 UI tests. E1 TWO-WITNESS (control_app.py + test_control.py): garbled grail GROUND-LABELS (a dropped War Traveler reads "WAA TRAVELIR / BATYLE BOOTS", missed 7 frames/6 reels) now register - _kai_ground_lines fires on tooltip context OR ground-label context, the latter gated by _kai_groundlabel_ctx (terse <=3 tokens/line + a distinctive base-type token from _kai_base_sig, 264 tokens self-parsed from bibles lf-base-codes lexicon - NOT a hand-static table). MULTI-WITNESS pattern (Konyo-blessed): name token + a real base witness on a terse frame = trust; one noisy signal alone = block. Tooltip path byte-unchanged. Verified 9/9 E1 (War Traveler grounds; chat "Diablos"->Ars AlDiabolos BLOCKED no base witness; bare name blocked; prose blocked; Harlequin/Enigma tooltip still ground) + 29-reel regression: recovers EXACTLY the 7 War Traveler drops, only War Traveler, 0 new false grounds. 160/160 control (+4 permanent E1 fixtures) + 181/181 routes + invariant GREEN. NOTE: base-witness is "a distinctive base type present" (0-FP verified, self-maintaining) not strict per-unique table (a follow-up if Konyo wants). Enigma/Shako await Konyos frame ids. control_ui.html + control_app.py + test_control.py. x3.
