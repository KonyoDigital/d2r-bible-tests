...
VERSION = "v1246"   # 👻🖼 GHOST MODE arc round 15 - FIX: the frame screenshots are back (Konyo: "where are the pictures? there were screenshots before"). Root cause was NOT a broken image - the frame IMG rendered and loaded fine (naturalWidth 2940, no 403). The inspect zone lives at the FOOT of the docked Time Machine, and when it AUTO-opens on scrub (v1243) it landed at window-y ~1219, BELOW the ~1050 viewport fold - so the picture was there but off-screen and Konyo never saw it (pre-v1243 inspect only opened via the button, which scrolled into view; the auto-open never did). FIX (layout/behavior only, no data change): (1) on scrub-auto-open (first open only, not every tick = no jank) scroll the inspect zone INTO VIEW (block nearest) so the frame the AI saw is visible the moment you scrub; (2) the frame IMG now loads eager (was lazy) - it is the hero of the inspect, never wait for it; (3) while inspecting the dock grows (max-height 56vh->82vh via .gsh-inspecting) so image + read fit together, shrinking back on snap-to-NOW/close. The 403 in logs is a red herring (an unarchived deep-lane sibling frame, honest onerror fallback), not the base frame. ZERO-TRACE held (outerHTML 21197==21197). Truthful: real frames only, honest "not archived" fallback. One-toggle + docked + inline + 3-panel structure + transition + keyboard all intact. control_ui.html only (15ins/2del). GHOST MODE 15/50+. x3 parity
