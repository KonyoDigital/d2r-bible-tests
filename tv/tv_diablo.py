...
VERSION = "v1326"   # ⭐ SESSIONS FLAGSHIP D26 - PIN / FAVORITE SESSIONS (sessions-visual, Phase C). A ⭐ toggle on every shelf card (hero top-right, paired with the verdict seal) + on the dossier ("☆ Pin this run" / "★ Pinned"). Pinned runs FLOAT to the top of the shelf (pinned-first WITHIN whatever sort/search/filter is active), get a "📌 Pinned" group header (any sort view) + a gold .pinned accent (left rail + border). A "⭐ pinned" filter chip composes with search + grail/clean/flagged/day. Persisted to localStorage keyed by the stable sessionId (same as D25 names). Clicking ⭐ toggles the pin and never opens the dossier (handled before the card-tap in the click delegation). Pure UI + localStorage, no engine. Verified on the real shelf (4 runs): newest [5,4,3,2] -> pin session 2 (oldest) -> floats to top ["2★",5,4,3], .pinned accent + "📌 Pinned" header, localStorage {sid2:1}; ⭐ pinned filter shows only [2]; dossier "★ Pinned"; JS 0 errors brace 0; visual_lock_invariant GREEN (all D26 CSS/JS uses var(--fw-*)). control_ui.html only (48/19). x3 parity.
