...
VERSION = "v1336"   # 🎬 SESSIONS FLAGSHIP D30 - COVER = BEST-FIND FRAME (sessions-visual). The cover art on the shelf card hero + the dossier cover is now the ACTUAL frame where the runs best find was captured (the money moment), instead of the generic reel thumb. Best find = top grail (else top find); its frameId -> /hist/<frameId>.jpg (the same convention the tally/beat thumbnails use). Falls back to the reel thumb, then the 🎞 placeholder. Shared _coverSrc(sm) drives both surfaces. HONEST: only uses the find frame when the find actually carries a frameId; otherwise the reel thumb (current behavior, no regression). So a run with a frame-anchored grail shows that grails drop frame as its cover; everything else unchanged. Verified: grail with frameId frameABC -> card hero bg /hist/frameABC.jpg?w=440 + dossier cover /hist/frameABC.jpg?w=420; grail without frameId (has thumb) -> falls back to /hist/reelthumb.jpg. JS 0 errors, visual-lock GREEN. control_ui.html only (16/4). x3 parity.
