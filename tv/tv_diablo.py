...
VERSION = "v1301"   # 📊 SESSIONS FLAGSHIP D14 - SESSION COMPARISON (sessions-visual, Phase C). A "⚖ Compare" button on the shelf controls opens a full-screen compare overlay (above the dossier): two session pickers default to the last two real runs (A=prior/gold, B=newest/mint), re-render live on change. 8 metric rows A-metric-B with a ▲/▼ delta on B vs A: reads / reads-hr / frames / found / grails / named / coverage / duration. Farming metrics higher-is-better (▲ mint improved / ▼ red regressed, winner side glows); duration neutral (gold, no winner). Honest "—" for any field a reel lacks; no delta when a side is absent/equal. Plus a scene-mix diff (each runs real scene-% fingerprint side by side, reuses _sceneFingerprint). PURE UI off the /api/sessions card data already in hand (no new fetch/engine). On the new type system (--fs/--fw/--ls/--lh tokens, gold/mint matching Task-Force/Last-Session); overlay scrolls internally so home no-scroll is untouched. Verified on the REAL code path w/ injected sessions (throwaway port): defaults A=prior/B=newest, deltas accurate (reads 1240->1580 ▲340, coverage 81->90 ▲9, reads-hr 443->494 ▲51), direction/color both ways, picker re-render, back closes, <2 runs honest empty state; JS 0 errors brace 0. NOTE: duration uses the apps existing minutes format (192m00s) for consistency; h:m is an app-wide helper decision (deferred, morning). control_ui.html only (144/0 pure addition). x3 parity.
