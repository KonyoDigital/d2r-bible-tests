...
VERSION = "v1262"   # 🛡💍 AUDIT FIX A4 - grail tooltip name grounding, which CLOSES the audit-fix group A. Real OCR was leet-garbling grail names (Harlequin Crest read as H4RLEQVIN CR, digit-for-letter), and the tokenizer dropped them for not being purely alphabetic, so the item landed in missed not register. Added a leet-aware grounder: de-leet the OCR then match a distinctive signature token against the strict grail lexicon (edit-1 at 6-9, edit-2 at 10+), guarded three ways (item-tooltip context only, skip stat/flavor lines, stopword-block) so gameplay can never mint a false name. Grounded frames are promoted to named and registered as a factual sighting (review-gated). VERIFIED on the real frames via the OCR binary: the Harlequin tooltip grounds to Harlequin Crest, a frame whose title was never OCR-captured stays honestly unnamed, and a full 142-frame reel sweep produced exactly 2 grounds (both legit) with 0 false positives. Floor 532. control_app.py + tests. GROUP A CLOSED - A1 panel guard, A2 gate honesty, A3 re-seal, A4 grail names, A5 sealed-wins, A6 via re-seal, all shipped v1258-v1262. x3 parity
