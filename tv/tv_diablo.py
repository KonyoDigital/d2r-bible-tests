...
VERSION = "v1292"   # 🔄 G3 THREE-OUTCOME ROUTING (Konyos architecture ruling made real) - the old "unclassifiable" dead-end is gone. bible.html _arComputeDiff now splits every witnessed item three ways after the tracked-collection pass: (1) CHRONICLE/TRACKED -> auto-route (merge-max apply, unchanged); (2) NON-CHRONICLE real item (RotW class bases Dread Whorl/Eagle Gorget/Storm Whorl/Viper Grip/Grim Eye/Ghoul Hood/Bone Razor/Beast Noose/Plague Wing, white bases, rolled magic/rare) -> a "🔬 -> Item Checker & vault" section with an "open Item Checker" button (routes to the manual keep-or-toss + mule flow, per Konyo: everything not in chronicle is checker-checked and vault-muled/thrown); (3) genuinely unreadable -> a tiny "❓ unclear" list (0 on real data). Plus a quiet "↩ ignored N consumables/gold" footnote. RotW bases are no longer a missing-dictionary BLOCKER - theyre expected bucket-2 items (a base dict would only prettify labels later). control_app.py endpoint stays outcome-agnostic (returns tallies + raw candidates; the browser does the 3-way routing since it holds ITEM_REGISTRY) - only a doc comment added. CLEAN SEAM for G4: bucket-1 auto-route + bucket-2 hand-off are plain functions with zero Grok dependency, so the removable verify layer bolts on later without touching this. Verified: py_compile OK; 16/16 bible scripts clean; node test on REAL extracted functions + real server output 23/23 pass (merge-max + the 3-bucket routing; real-data bucket sizes itemChecker 36 / ignored 18 / unclear 0). control_app.py + bible.html. x3 parity.
