...
VERSION = "v1306"   # 🏆 SESSIONS FLAGSHIP D17 - BEST-RUN / STREAK HIGHLIGHTS (sessions-visual, Phase C). A highlights strip at the TOP of the shelf (read-only overview before the run grid), up to 5 cards off the run set (reuses _cmpList): 🏆 Best run (most grails, tiebreak reads) · ⚡ Most reads · 📈 Top reads/hr · 📊 Best coverage · 🔥 Streak (consecutive calendar days farming ending at the latest run day, "N days · M runs"). HONEST: cards OMIT when data cant support them (no grails -> no Best-run card; no coverage data -> no coverage card) - real bests only, no fabrication. Single-run cards are tap-to-dossier (click -> that sessions detail); Streak non-clickable. When one run is standout on multiple axes both cards point to it (truthful - it genuinely was both; different measures). Pure UI off /api/sessions already in hand (no new fetch), new type system, gold-tinted responsive auto-fit grid, shelf scrolls internally (home no-scroll untouched). Verified on the real shelf w/ 5 injected runs + a 3-day streak: Best S4 (3 grails/1480), most-reads S4, top-hr 592, best-cov S5 84%, streak 3 days/5 runs all exact, tap-to-dossier wired, JS 0 errors brace 0. control_ui.html only (47/1). x3 parity.
