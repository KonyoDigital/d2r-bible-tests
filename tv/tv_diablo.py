...
VERSION = "v1211"   # 🧠🖥 ENGINE ROOM arc round 2 — LABELED PLAYHEAD FLAG ("you are here in time" was unclear: the retro cursor was a thin 2px unlabeled line lost among 66 barcode ticks + stall bands). A gold labeled handle ("reel N · H:MM:SS") now rides the top of the cursor over the reels' empty top band + a bolder playhead line — the reel/instant is pinned to the timeline POSITION, not just read from the caption below. Correctness catch: single-sourced the flag from the SAME resolved reel.n + best.ts the spine band + readout use (in _engineRoomRenderRetro), so flag/band/readout/LEDGER always agree even when scrubbing onto a still-resolving reel. control_ui.html only, existing fields. ×3 parity (39/80 → v1272)
