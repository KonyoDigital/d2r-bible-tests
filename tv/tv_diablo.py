...
VERSION = "v1307"   # 🏦 E1b - FUNNEL-2x ACCURACY RECONCILE (Konyos "funneled a couple times through the checker"). When you mule an item the AI Item Checker ALREADY verified, the 2nd read is RECONCILED with the first instead of overwriting - a 2x pass raises confidence, honestly. Both reads AGREE (same verdict+stats) -> agree:true checks:2 -> card "✓✓ 2x confirmed". Verdicts DIFFER -> flagged UNCLEAR, both kept (verdictReads:[keep,toss]) -> "⚖ 2x unclear". Stats DIFFER -> mods in BOTH reads are confident, a mod in only one is listed DISPUTED ("⚖ only seen in one read") - never silently dropped; union preserved for display. OPTIONAL - reconcile only triggers on a re-check; a single pass behaves exactly as before. Mod match case/space-insensitive so trivial OCR diffs dont false-flag. Extends the E1 verdict-attach (v1287): _aicReconcile(prev,cur) (intersection=confident, symmetric-diff=disputed, verdict agree/disagree; window-exposed, node-testable); aicMule reconciles when a prior checked record exists + honest 2x status; _mfChecker passes through checks/reconciled/agree/disputed/verdictReads; vault card shows the confidence + disputed-stats line. Verified: 16 bible scripts clean; _aicReconcile node 7/7 (full agreement checks=2, verdict-disagree, stat-disagree union preserves all 3, 3rd read checks=3, normalized match no false dispute); backward-compatible (old records default checks 1, unchanged rendering). bible.html only. x3 parity.
