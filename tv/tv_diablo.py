...
VERSION = "v1207"   # 🩹 FUNNEL round 8 — closer's OWN OCR worker orphanable on any per-frame exception: _kai_closer_loop spawns an ocr_mac --worker subprocess (wp) once per reel it closes; wp.stdin.close()/wp.terminate() was a plain sequential statement AFTER the ~120-line per-frame loop, NOT in a try/finally. Any uncaught exception in that loop (across _kai_frame_cls/_kai_tab_strip_refine/stash_eye.analyze_frame/_kai_grid_vote_label/etc) jumps to the closer's outer per-reel except (sleep 10 + move on) — skipping wp.terminate() → wp orphaned, AND it COMPOUNDS: the next reel spawns another. Same worker-leak vein as v1206, different subsystem (retro-OCR). FIX: wrap the per-frame for-loop in try/finally, wp cleanup in finally (its own try/except). git diff -b = 11 real lines (comment+try+finally), verified pure re-indent, no logic drift. +2 tests (control 119→121). ×3 parity (35/80 → v1252)
