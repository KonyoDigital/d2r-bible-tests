...
VERSION = "v1278"   # 🔬 SESSIONS FLAGSHIP D6 ENGINE - super-recovery + missed-text-drill feed (team-lead; lights up the R/M% badge + drill polish-ui-2 built with a kaiMissed fallback). /api/sessions now carries per-session superRecovery + missedFrames, from the SAME cached reel report (no extra read). A "recovery" = a deeper pass rescued item text the first pass missed: kai.caughtNames (retro sweep), super.deepNames (deep re-read), second.correctedNames (second-eye correction). The report `missed` ledger is the text that stayed UNREAD (final gaps), disjoint from recoveries, so total-missed M = recovered + still-unread and recovered/M is the TRUE rescue rate. superRecovery = {recovered, missed(=M)}. missedFrames = up to 24 drill rows: a recovered frame carries its rescued name as `label`, a still-unread frame omits `label` (UI shows the honest "unreadable text"), each {frameId, ts} jump-wired, sorted by ts. Verified on the real journal: 65/107 sessions carry the badge, 23 have real recoveries >0 - e.g. a session showing recovered 2/3 where the deep pass clawed back "Spirit" (a runeword the first pass missed). Honest-absent when a reel had no missed text. 156/156 engine tests green. Serves after the next console restart. control_app.py only. x3 parity.
