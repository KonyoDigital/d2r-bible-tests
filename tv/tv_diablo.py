...
VERSION = "v1266"   # 🗣📊 DIABLO-LANGUAGE B8 - SESSION SCENE FINGERPRINT. A "what this run WAS" line from each sessions real sceneReads/tabReads (v1254), rendered on the HOME last-session digest + the SHELF card via _sceneFingerprint, e.g. "farming 63% · stash 18% · Gems x5 · town 13% · portal 8%" - each segment colour-led. SYNCED: uses the SAME hardcoded _DIABLO_SCENES/_DIABLO_TABS dictionary + palette as the v1255 live chips + retro trace = one source of truth. TRUTHFUL: % from real read counts; returns empty when a reel has no sceneReads (27/88 sealed reels carry them) -> caller omits the line, never faked; legacy reels keep the old kaiClasses line as fallback so no card loses its at-a-glance summary. Two honest design calls: (1) % of reads not event-counts (the scene tally has no ordering, so 8 transition-reads could be 8 reads of ONE loading screen - "% of reads" is honest, "8 portals" would overclaim); (2) stash detail prefers the dominant tally tab (Gems/Runes/Materials xN). LIGHT (derives off the 12s sessions poll). control_ui.html only (56/3). DIABLO-LANGUAGE 8. x3 parity
