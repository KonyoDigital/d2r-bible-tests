...
VERSION = "v1305"   # 🟣 G4 COHESION - self-describing flags + the removable-Grok layer COMPLETE (also certifies v1305 REMOVAL TEST). Cohesion fix (3 lines): each touchpoints disagreement flag now carries its own kind (chronicle / keep-toss / grail-recheck), so the "🟣 Grok caught this" surface is self-describing and never depends on collect-time defaults - all 3 flags now consistent with seam 3s original shape. FULL-ARC VERIFY (all green): py_compile OK (control_app.py + g4_grok.py); 16 bible scripts compile; G3 regression 23/23 (unaffected); self-describing kinds flow through _g4_collect_flags -> [chronicle, grail-recheck, keep-toss]; fence balance intact (control_app 6 blocks, bible 2 pairs); REMOVAL TEST STILL CLEAN after the cohesion edits (stripped py_compile OK, 0 traces). THE WHOLE G4 LAYER, DONE (Konyo-spec end to end): scaffold (v1295) · toggle (v1298) · 3 cheap seams (v1297/1299/1300, flag-never-override, OFF byte-identical) · review surface (v1303) · credit+config caps+per-seam (v1304) · removal test+doc (v1305) · cohesion (this). Every trace behind # GROK ADD-ON (G4) fences + the one module file; grep finds all; the removal script deletes it with ZERO scars; OFF = byte-identical to a no-G4 build. Standing morning items: Konyos XAI_API_KEY + model (default grok-4-latest); live agree/disagree path untested tonight (no key) - first exercise = his morning enable. control_app.py only (3 lines). x3 parity.
