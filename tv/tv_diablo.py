...
VERSION = "v1355"   # 📺 TV-D ENGINE-BAY DENSITY (R6, Konyo-steered) - all 7 engine lamps in one glance, nothing-hidden made literal. Was 1-of-7 visible on-air (the console controls ate ~340px before the bay started). Fix (Konyo picked engines-first + keep-full-detail + two-per-row): (1) the AI ENGINES bay now sits directly under the primary On Air / END SESSION button; the SECONDARY controls (advanced, Theatre) moved BELOW the bay into a new .rail-secondary block - engines lead, prime real estate. (2) Router|Watchdog and Agent Mind|Master Brain pair TWO-PER-ROW; the 3 eyes (Live/Second/Kai) stay full-width; full 2-line status kept (no truncation, no hover - detail visible at a glance as Konyo wanted). (3) tightened: banner 1-line, eye pad 8->6, gap 5->4, paired status fs-2xs. Engine content ~520px of rail; on Konyos fullscreen (~900px rail) all 7 fit with room. Verified: invariant GREEN, JS 0, demo J5 THREE EYES + J6 signal pass (advanced details still present + closed after the move). NOTE for Konyo: banner dropped "- the scanner" to win the 1-line fit (restore if missed); if his real screen still cuts 1-2, a button-compact reclaim is a fast follow-up. control_ui.html only. x3 parity.
