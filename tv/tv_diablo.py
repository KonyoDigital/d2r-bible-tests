...
VERSION = "v1303"   # 🗺 SESSIONS FLAGSHIP D16 - AREA HEATMAP (sessions-visual, Phase C). A "🗺 Areas" button on the shelf controls (next to ⚖ Compare) opens a heatmap overlay: where you farm most, as ranked heat-bars off the session areas[] across all real runs. Ranked #1..12 by run FREQUENCY (the honest weight); bar width prop frequency, bar OPACITY heat-scales (hotter=more often) so it reads as a heatmap. Per area: name · N runs · % of runs · total reads (summed) · 🏆 grails. Header "🗺 Where you farm · across M runs · N areas"; honest empty state. HONEST-DATA CALL: sceneReads keys are scene-TYPES (town/farming/stash) not zones, and finds carry item loc (stash/floor/equipped) not a game-area, so grails genuinely cant be pinned to a zone - frequency is the honest weight and the 🏆 counts are labelled precisely ("grails found in the runs that visited an area, not pinned to the zone") with a footnote; NO fabricated per-zone attribution. Pure UI off areas[] already in hand (reuses _cmpList; no new fetch/engine), new type system, gold heat palette, overlay scrolls internally (home no-scroll untouched). Verified on the real code w/ 6 injected runs: aggregation exact (Chaos Sanctuary 4/67%/4650/🏆4, Meph 2/33%/2570/🏆2, tiebreak by reads, heat gradient descends), back closes, empty honest, JS 0 errors brace 0. control_ui.html only (83/0). x3 parity.
