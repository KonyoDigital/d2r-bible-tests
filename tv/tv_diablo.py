...
VERSION = "v1374"   # AREA-INFERENCE RIBBON CONSISTENCY - completes "ENTERING The Pit" across BOTH retro surfaces via ONE tested law. The classFrames chapter ribbon now uses the same forward area-inference as the reconciler (a transition frame names the zone being ENTERED, forward, never the nearest which could be the PREVIOUS zone). Refactored to one law: extracted _forward_area_from(ts, area_ts, window) as a pure module helper; the reconcilers _forward_area and the ribbons _cf_forward_area are now thin wrappers (E4 one-tested-law pattern) - the two closures had their own copy of the loop, now they cant drift. Honest: forward-only (<=8s), a nearest-PREVIOUS zone never wins, no forward zone -> loading unchanged. The shared law is now directly unit-testable. Verified: 4/4 area-inference incl the new _forward_area_from shared-law test (forward-not-previous, previous-only->absent, beyond-window->absent, no-ts->absent), 219/219 control, 181/181 routes, invariant GREEN. control_app + test_control. x3.
