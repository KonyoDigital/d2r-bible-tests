...
VERSION = "v1220"   # 🧠🖥 ENGINE ROOM arc round 11 — DENSITY/HIERARCHY pass (make 10 rounds of features read as ONE instrument). The drill-down had become a tall vertical stack — flag bar, then the frame image ALONE with a big empty gutter beside it, then the read-chain full-width, then ~9 evidence rows; it read as stacked additions with dead space. FIX: reflowed the top into a two-column HERO — frame image (left) + the CONCLUSION (right: read-chain + "owns the final read" + verdict) side by side, filling the empty gutter; the raw evidence rows (reader-net/read-in-flight/OCR/quorum/gate/funnel/super/raw) read below full-width. Now it breathes + reads top-down: "here's the frame → here's what the engines concluded → here's the evidence." Pure layout — no logic/ID/data changes, every r1-r10 feature intact; two-column collapses to stacked on narrow via flex-wrap. control_ui.html only. ×3 parity (48/80 → v1272)
