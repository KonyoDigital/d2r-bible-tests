...
VERSION = "v1363"   # 🗺📋 B10 area->act Diablo truth + Sessions R2 hunt-hub. B10 (control_app): _diablo_scene_label - the ONE Diablo-language fn feeding the live native banner + classFrames + forensics synthesis + B8 fingerprint - now prefixes the ACT when the area is mapped ("FARMING · Act 1 · Dark Wood", "TOWN · Act 5 · Harrogath") via a canonical ~130-entry _AREA_ACT map (vanilla RotW area names, reels confirm vanilla); UNMAPPED area -> label unchanged (never a fabricated act), unclear stays unclear - honest edge states. Pure/deterministic, additive act field, ZERO routing/grounding/count impact - pure labeling, every surface speaks richer Diablo at once. SESSIONS R2 (control_ui): the hunt-hub zone tier + re-home - 3 hunt zones (YOUR HUNT gold / DAILY MISSIONS mint / YOUR RECORD blue), the TV-D engine cards (Health/Engines/Intake) display:none on the Sessions dash (Konyos split - they live on TV-D), and the Task Force (grail 243/403) now LEADS via CSS order (KPI - wall demoted). Verified: 191/191 control (+6 B10), invariant GREEN, JS 0, no-scroll, demo J2/J5/J6. control_ui + control_app + test_control. x3.
