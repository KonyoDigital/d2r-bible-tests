...
VERSION = "v1193"   # 🩹 FUNNEL round 3 — register tier BEST-wins (not first-wins): _kai_compile_register's _consider used "first non-blank tier wins" — walking sess_rows chronologically, the FIRST kai-judge verdict froze an item's tier forever, so a later more-authoritative super-analyze re-read (which _kai_reconcile explicitly ranks ABOVE a first-pass) couldn't upgrade a stale 'border' to 'grail'. Tier is a QUALITY verdict, not a timestamp. FIX: _KAI_TIER_RANK (grail>keep>border) — a later better verdict upgrades, a proven grail is never buried under a weaker later guess. The tier-domain analog of never-zero/max-verified-total-wins. firstSeenTs/frameId keep "earliest wins" (correct there). +5 tests, new coverage for a previously-untested fn (control 83→88). ×3 parity (21/80 → v1252)
