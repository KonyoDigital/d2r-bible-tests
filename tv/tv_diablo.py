...
VERSION = "v1360"   # 🔬 READS FORENSICS backend (Konyos "give me the X-ray" - the same forensic view g3 uses, so he can pinpoint + surgically fix). PURE READ-ONLY PROJECTION (no hot-path writes): _kai_forensics_project() re-runs the SAME pure grounder/near-miss helpers over the stored raw OCR, deterministically reconstructing every read RETROACTIVELY on all existing reels. Exposed at /api/forensics?sid=<sid> (or newest reel, reel-mtime cached) + a lean forensicsSummary counts badge on /api/status. Per read: status (grounded | resolved-corrected | recovered-2witness | recovered-crossframe | blocked-fp | unresolved), corrected:bool (TRUE = a garble the AI FIXED, render as a fix not a clean read - Konyos "see it got it right"), synthesis (the connect-the-dots reasoning in Diablo language via _diablo_scene_label), frames[].raw (the OCR garble VERBATIM), correction{raw,resolved,via,edit} / block{raw,nearest,edit,blockedBy}, witnesses, resolvedFrom. Grouped by item+status. Verified vs the 29 REAL reels, 0 fabricated: War Traveler x7 -> recovered-2witness (travelir+base), "anclents"/"Diablos" -> blocked-fp (no-tooltip-context, correctly refused), Death Torc/Hard Leather -> resolved-corrected (clean-match-nearby); totals clean 61 / corrected 5 / recovered 7 / blocked 7 / unresolved 103. 8 fixture tests + 176/176 control + 181/181 routes + invariant GREEN, bad sid -> honest {ok:false}, honest-absent on raw-less reels. Contract handed to sessions-visual for the 🔬 surface. control_app.py + test_control.py. x3 parity.
