...
VERSION = "v1334"   # 🏆 SESSIONS FLAGSHIP D28 - GRAIL-PROGRESS-THIS-SESSION (sessions-visual, Phase C; Chronicle tie-in). A "🏆 Grail progress" section on the dossier (when the run found grails): +N to the hunt = the runs GENUINELY-NEW grails counted via firstSeenTs inside the session window (a re-find doesnt inflate) + the grail names; a progress bar + "243 / 403 Holy Grail · 160 to go" from the current ledger (d2r_forgeSummary - same source as the home-dash grail chip). HONEST CALL on "241->243": the engine exposes NO per-session grail snapshot, so a literal before/after for an arbitrary past run would be a reconstruction that breaks once later runs also find grails - so it shows the runs REAL contribution (+N new) + the CURRENT total, never a fabricated arrow (same truth standard as D16/D22; if the engine ever emits grailBefore/After the arrow is a one-line add). Honest-absent: no grails -> section omits; forgeSummary absent -> just "+N this run · names" (no total bar). Verified: 2 grails found but only 1 with firstSeenTs this session -> "+1 to the hunt · Deaths Fathom, Nightwings Veil · 243/403 · 160 to go" (bar 60%) - the +1 not +2 correctly reflects the one genuinely-new grail. JS 0 errors, visual-lock GREEN. control_ui.html only (31/0). x3 parity.
