...
VERSION = "v1368"   # 🔍 SCENE-FINGERPRINT portals honesty + register merge-max lock (batched). PORTALS FIX: _session_scene_fingerprint.portals now counts distinct portal/loading EVENTS (maximal runs of entering-reads, same law as townTrips) not the raw read count - a portals loading screen spans several frames = several entering reads = over-count (measured: 8649_18321 4->1, 0319_92862 4->2, 10+ real sessions up to 4x). So the dossier stops saying "took 4 portals" when Konyo took 1. Same field, honest + consistent, ZERO grail/routing impact - pure display honesty; all 27 journal sessions now portals==distinct-runs. Confirmed the rest honest (of-real-reads, honest-absent, farmingPct-of-reads). MERGE-MAX LOCK (round 5, riding along): TestRegisterMergeMax freezes the register invariant the E3 re-seal relies on - monotonic-under-reclose (v2 superset v1), best-tier-never-downgrades (a later border cant bury a proven grail), never-zero holds. Proves grail count can only GROW on the auto-resweep, never lose a find; no automatic un-tick path exists (only explicit user d2r_grailUnfound). Verified: 4/4 portals + 3/3 merge-max + 209/209 control + 181/181 routes + invariant GREEN. control_app + test_control. x3.
