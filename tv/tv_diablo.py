...
VERSION = "v1349"   # 🔌 ENGINE-EXPOSURE (#1+#2, the "nothing hidden" data contract) + 🎨 zone-banner tier. Two lanes, both gate-ready, bundled. ENGINE-EXPOSURE (control_app.py, pure additive read-surface, no behavior change - for the TV-D control panel dials): (1) _WATCHDOG_LAST deepened -> {sid, violations, rules[], ts, verdict} so the UI shows WHAT tripped + when, not just a count (null pre-seal = honest-absent); (2) eyes{} unified - _eyes_pulse gains liveTs, status_payload adds fresh per-poll liveAgeMs (kept OUT of the mtime cache = live, null when no read) so all THREE eyes share one shape {liveTs, liveAgeMs, verifyTs, kaiTs, kaiMissed}. 10/10 green (watchdog null->clean->violations, eyes shape, liveAgeMs freshness + cache-purity). Data contract handed to sessions-visual (eyes/watchdog/driver/sessionHealth.gate/mindStory/liveRing, all real-0/null/[]/absent when the engine hasnt produced = dead engine renders a dark dial, never a fake glow). ZONE-BANNER TIER (control_ui.html, Konyo-steered): new .zone-banner component (fw-bold, ls-widest, accent-coloured label + 4px accent bar + icon + subtitle, all locked tokens) sits ABOVE the card-title tier so zones finally announce themselves - the fix for "cant tell sections apart". 3 zones: YOUR HUNT (gold) / READOUTS (info blue) / THE SCANNER (mint). no-scroll watched (fs-sm eyebrow, overflow 0). Verified: invariant GREEN, JS 0, py_compile OK. control_app.py + control_ui.html. x3 parity.
