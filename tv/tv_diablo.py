...
VERSION = "v1343"   # 🔒 VISUAL-LOCK STRUCTURE PASS (Phase G structure pillar; same discipline as the weight pass - fold identical values onto tokens zero-visual, extend the invariant, FLAG genuine drift for a design call not silent redesign). Ran a full structural inventory of every console region. ALIGNED (all zero-visual, the literal already equalled the token): (1) new token --hd-radius: clamp(10px,1vw,14px) = THE console-card radius - the cockpit cards (.m/.signal/.ticker) already shared this exact clamp, now single-sourced so their corners cant drift apart (computed 12px @vw1200, byte-identical); (2) folded the 4 exact padding:12px 14px literals (.cmp-scene-col/.dgp/.sh-live/.sh-timeline) onto var(--hd-pad-y) var(--hd-pad-x) - the token IS 12/14 so pixel-identical (--hd-pad now single-sources 5 sites). INVARIANT EXTENDED: visual_lock_invariant.py now also asserts the consoles --hd-* structure rhythm is DEFINED in :root (gap · gap-in · gap-row · pad-y · pad-x · head-mb · head-ls · radius) - value-agnostic (px/clamp/var all valid), locking the tokens exist as the single source (cant be silently deleted); mirrors the --fw defined-check; negative-tested (removing --hd-radius fails loudly); still GREEN. FLAGGED for Konyos design eye (genuine visual/identity changes - NOT altered, logged in MORNING_QUESTIONS alongside ls/lh): card-radius spread (5 radii side-by-side 9/10/11/12, rail 7->14), card-padding wander (8/9/11/13/14/16 vs token 12/14), section-header 5 families (accent-bar drift + stat-label tracking .04/.10/.20em same role) - each = one deliberate design pass, not a silent fold. Verified: invariant GREEN (incl new structure check), JS 0 errors, on-air render byte-identical (radii/padding computed-equal), no-scroll intact, 0 console errors. control_ui.html + visual_lock_invariant.py. x3 parity.
