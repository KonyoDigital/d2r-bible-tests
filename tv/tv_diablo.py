...
VERSION = "v1240"   # 👻⏪ GHOST MODE arc round 11 - THE TIME MACHINE DOCKED AROUND THE REPLICA (Konyo: one toggle, no "enter the time machine" button, the console just turns into a time machine in place - no routing). Killed the enter-the-time-machine gateway. Flip GHOST ON -> the ghosted console replica floats on top WITH the FILM + JOURNEY scrubbers + CURRENT STATE + r9 narration docked right at the foot, visible immediately, framing the replica - no second tap to reach the time-travel. SAFE PATH: RELOCATE the live wired scrubber nodes (er-film/er-gj/er-reelcard/er-livefeed/er-current) into the dock (same elements, not clones), recording each nodes exact home (parent + nextSibling) so flip-off restores them precisely. Deep forensics (engine spine, read-chain, owner/verdict, frame image, drill-down) stay in #engine-room behind an OPTIONAL "inspect this moment" - never the required path, never routes the view. Refactor: _engineRoomOpen -> _engineRoomEnsure (build+wire+init without showing) so ghost-on builds then docks the scrubbers; _engineRoomRefresh updates while ghost is on so the docked FILM stays live (67 reels docked, journey narration live, docked film-scrub drives retro without opening the drill-in). ZERO-TRACE held: flip OFF restores scrubbers to #engine-room, removes #ghost-shell, real .shell outerHTML BYTE-IDENTICAL (21197==21197), 0 orphaned data-gh-id, re-open after a full cycle repopulates cleanly. Pure add-on, truthful, control_ui.html only. GHOST MODE 11/50+. x3 parity
