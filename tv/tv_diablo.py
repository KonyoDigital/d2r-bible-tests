...
VERSION = "v1346"   # 🎨 DESIGN-LOCK R3 (Konyo-approved) - console SECTION-HEADER unify. Made the gold-accent-bar family THE canonical card-title chrome: .hd-h dashboard headers + the rail KAI-READER + SESSION-HEALTH pseudo-headers now share ONE spec via new --hd-head-size (fs-xs, the middle - .hd-h came down ~1.5px, SESSION HEALTH up ~1.5px, all computed-equal 14px): weight --fw-semibold (rail was bold), uppercase explicit on all (.hd-h no longer relies on pre-uppercased content), margin --hd-head-mb 9px everywhere (rail was a cramped 5/3px), pad 0 0 6px 10px everywhere. Kept the 2px accent-bar + hairline underline (already shared) + each headers semantic COLOR (gold/gold-dim/mint = live engine - color is identity not drift). Dropped 2 redundant overrides. "THE KAI READER" now reads as the same card-title as "DAILY TASK FORCE", only the mint tells them apart. STAT-TILE LABEL TRACKING collapsed .04/.10/.20em -> ONE --ls-wide (.10em) across all 5 label surfaces (.m .l stage meters were .20em, .tkv-l was .04em; .kpi-l/.eye .en/.rnf-lastlab already .10). FLAGGED for Konyo: stage meters MODE/READS/AREA/SCENE tightened .20->.10em (less broadcast-wide) - 1-line revert if he wants them as their own wide tier. INVARIANT: --hd-head-size added to the locked --hd set. Verified: demo 7/7, invariant GREEN, JS 0, no-scroll intact. control_ui.html + visual_lock_invariant.py. x3 parity.
