...
VERSION = "v1339"   # 🧠 F3 AGENT MIND GLOW-UP (sessions-visual, Phase F). Elevated the live thought-stream ticker from "raw kind word + text" (it printed cap/skip/boot as the marker - technical noise) to per-thought ICON + honest rarity/kind ACCENT + a reduced-motion-aware landing pop. PER-THOUGHT ICON: mapped the 8 REAL engine kinds (cap/skip/boot/settle/read/transition/ocr/heartbeat -> 📷/⏭/⚙/👁/📖/🌀/⚡/💓); when the engines own thought already opens with an emoji (🎯/⚡/👋/💓/⏳/⚠/👁‍🗨 - the scanners more specific choice) THAT leads + is stripped from the text -> never a double-icon. RARITY/KIND ACCENT (honest-by-construction, derived from the thoughts REAL words never fabricated): 🏆/grail -> bright gold+glow (money-moment pops); failure words (⚠/fail/timed out/denied/throttl/error) -> muted amber; good reads (👋/🎯/new text/second look/reads live + all settle) -> mint; scene/transition -> cool blue; ocr -> violet; rest dim. Grail vs warn tuned apart so a drop is unmistakable vs a hiccup. LANDING POP: newest thoughts icon scale-pops as it lands (existing lineIn slide kept) + a prefers-reduced-motion:reduce guard kills both (the ticker had none - D20/D21 discipline). On-air-focused (only the live stream), locked type system (no font-weight literals added), no-scroll (tickers existing height, zero layout cost). Verified: mocked on-air across every kind -> 🏆 gold+glow, ⚠ amber, scene blue, ocr violet, skip/heartbeat dim, newest icon pop, no double-emoji (ZWJ 👁‍🗨 handled), leading-emoji regex extracts, plain text -> kind icon fallback; off-air unchanged; JS 0 errors, visual-lock GREEN. control_ui.html only (64/3). x3 parity.
