...
VERSION = "v1215"   # 🧠🖥 ENGINE ROOM arc round 6 — FRAME NAVIGATION (watch the AI's reads EVOLVE). Biggest capability gap: you could drill into ONE frame but had to re-scrub the dense 66-reel axis to see an adjacent one (imprecise, can't reliably step 1 frame). FIX: a ◀ N/M ▶ control in the RAW PROCESS head + ←/→ arrow keys that step prev/next FRAME within the current reel — re-rendering read-chain, owner/verdict, gate, OCR, quorum, spine organs, playhead flag, readout, spine band, all time-synced to that frame. Reads the reel's CACHED beats (ER.reelCache, no fetch), lazily pulling each frame's deep forensic blob on landing. Konyo can now hold → and watch the engine's decisions change frame-by-frame. Sets focusSid/focusTs → clean retro-frame mode + cursor repositions; arrow handler guarded to drill-down-open + doesn't break Esc; buttons disable at reel first/last; "106/112" shows position. control_ui.html only. ×3 parity (43/80 → v1272)
