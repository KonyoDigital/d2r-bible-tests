...
VERSION = "v1293"   # 🎨 VISUAL LOCK typography (brought forward) - ONE weight source across the console (sessions-visual agent). Extends the R1 type system from the Sessions dash to the WHOLE control_ui.html. Sizes were already on --fs-*; weights were 207 scattered literals - now ALL folded onto --fw-* (164 font: shorthand weights + 40 standalone font-weight + 3 inline theatre-header styles; 500->normal 600->medium 700->semibold 800->bold), ZERO residual literal weights left in CSS. Also removed a DEAD !important rule (.panel-h/h3.section/.console-h letter-spacing targeting selectors that dont exist in control_ui.html - those class names live in bible.html, a different file) = one less phantom override. Deliberately deferred: 3 JS-built inline weights (out of scope for a CSS round), font-weight 400/900 (no matching token), and letter-spacing/line-height literals (mostly OFF-scale on purpose - display .3/.42/.24em, lh 1.4/1.55/1.2; folding only exact matches would FRAGMENT it - the right move is a dedicated Phase-G round v1341-1345 that first decides whether to expand the --ls-*/--lh-* token set, then folds app-wide by one rule). Identity swap (var(--fw-bold)===800): JS 0 errors, brace delta 0, console renders IDENTICALLY on throwaway port, weights resolve correctly (ON AIR 800, phase 700, .hd-h 700), R3 overlap fix still holds. control_ui.html only (207/205). x3 parity.
