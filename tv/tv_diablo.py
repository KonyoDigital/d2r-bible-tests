...
VERSION = "v1350"   # 🔌 ENGINES{} CONNECTED-STATUS - the "disconnected wire is visible, never silent" safeguard, real (the nothing-hidden doctrine in the backend). Pure liveness projection off the v1349 truths, no new bookkeeping except one router heartbeat. status.engines = {liveEye, secondEye, kai, router, watchdog}, each {label, wired:bool, state:"live"|"idle"|"down"|"armed", lastBeatMs:int|null, note}. wired:false = a DISCONNECTED WIRE the UI paints VISIBLE (⚫), never silently absent. Per-engine truth: liveEye always wired (down=on-air+agent-alive+no-read-5min=wedged); secondEye evidence-based wired (⚫ until a real verify beat = TV_VERIFY off/not-connected); kai wired=tvd-kai-closer thread alive (⚫ "no OCR bin - closer not plugged" when bin/ocr_mac missing or TV_KAI=0 - exactly the wire Konyo wanted surfaced); router wired=tvd-engine-driver thread alive (down on engineDeadHard, _DRV_BEAT live=routed a read <=30s); watchdog event-driven always armed (lastBeatMs=last seal, note=last verdict). Helpers _engines_status() + _engine_thread_alive(). Verified: py_compile OK, 13/13 engines projection test (cold/live/wedged-down/dead-hard/post-seal/shape), 156/156 test_control suite (boots real Handler, end-to-end payload), invariant GREEN. Contract handed to sessions-visual (paint 🟢 only on state=="live", ⚫ on wired:false/idle/down). The receipts[] stream stays HELD for Konyos schema-nod. control_app.py only. x3 parity.
