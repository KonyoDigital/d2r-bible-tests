...
VERSION = "v1372"   # AREA-INFERENCE - "ENTERING The Pit" (Konyos explicitly-requested feature, honest-absent). At the reconciler: a transition/loading frame is a dark screen with no area of its own, so it now borrows the zone being ENTERED from the NEXT deep read that names one (forward-looking, <=8s - loading precedes the zone by a few seconds). Result: "ENTERING The Pit" instead of "ENTERING (loading)"; added The Pit/Hole to the B10 act map so it renders full Diablo language "ENTERING Act 1 · The Pit". Honest-by-construction: borrows only a REAL forward zone; no forward zone -> "ENTERING (loading)" unchanged; a zone >8s away NOT borrowed (window respected). Retro-only by nature (needs the future read; live cant infer what hasnt happened) - lands on sealed engineFrames -> theatre + forensics. Zero routing/grounding/count impact - pure labeling. Thin current evidence (fires once on the 45-session journal) but forward value - every portal->zone now names the zone; cheap + correct + Konyo-requested won over a speculative bigger pivot. Verified: 3/3 area-inference + 6/6 area-act + 218/218 control + 181/181 routes + invariant GREEN. control_app + test_control. x3.
