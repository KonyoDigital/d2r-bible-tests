...
VERSION = "v1356"   # 🧾 TV-D FINALE - the AI-READS RECEIPT FEED (Konyos org-backbone vision, on-screen). The bottom ticker now STREAMS the AIs reads on g3s receipts[] contract: per-engine Diablo-language rows composed kind + refs.itemName + diablo.label - "read Griffons Eye - FARMING - The Pit Level 1", "verified Shako - confirmed x1", "routed - ROUTED runes x4", "judged Ral Rune - KEEP", "flagged - WATCHDOG: mlvl-block". Per-engine left-accent from the bay palette. Keyed on receipt.id (self-dedupe), newest-first, bounded <=14 shown + scroll, NO page growth. CLICK->ROUTE (client-side grounding via the board iframe): item -> _aicIsGrailName ? F-Uniques dossier (setActiveItem) : the Checker; session -> Sessions dossier; flag -> Tools - never a fake dossier. HOVER-ART: framed receipt floats its captured frame near the cursor via /hist/<frameId>.jpg (the consoles established URL path - confirmed correct; CSP-safe, cursor-following, reduced-motion aware). HONEST: empty off-air -> resting line, streams on-air. The tag CYCLES reads (default) -> thoughts -> story -> log (raw thoughts preserved as a mode). TICKER RENAMED to "AI READS" (accurate for the receipt content + de-dupes "Agent Mind" which the bay keeps - Konyos rename intent satisfied; 1-word to "THOUGHTS" if he prefers). Verified: invariant GREEN, JS 0, demo J5+J6 pass, no-scroll (bounded feed). THE TV-D CONTROL PANEL IS COMPLETE: engines up top (all 7, honest lamps), their reads streaming below (routable, frames on hover). control_ui.html only. x3 parity.
