...
VERSION = "v1233"   # 👻⏪ GHOST MODE arc round 4 — THE GRAIL JOURNEY (full-range months-scale rewind axis). A dedicated draggable Time Machine axis from the earliest found-log stamp -> now, below the frame timeline (two truthful scales: frames=seconds/reels, journey=months). All 351 ticks are REAL finds at real dates (visible finding pace; dense clusters = grinding sessions). Drag the marker and grail climbs monotonically from real data: 29 by May15 -> 299 by Jun16 -> 351 today; readout "N/403 uniques found by <date> · real, from your found-log". Kept SEPARATE from the r3 frame-scrub grail-at-T (both coexist) so the reel/second frame-debug UX stays intact. Truthful per matrix: real ticks/counts, no fabricated points, absent if no found-log. Pure read of localStorage, console untouched, zero-trace. control_ui.html only. GHOST MODE 4/50+. ×3 parity
