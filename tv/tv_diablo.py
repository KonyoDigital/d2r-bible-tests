...
VERSION = "v1377"   # 🏛 SESSIONS FLAGSHIP R3 - the foundational per-context split (Konyo blessed MY HUNT). The Sessions nav is now CONSOLE-NATIVE: it no longer opens the bible iframe - it sets data-view=sessions and shows the flagship hunt hub as the console home (new showSessions(), no shell-open); the broadcast stage steps aside so the hunt leads. TV-D clears data-view -> the cockpit returns (stage + record); toggle confirmed both ways. The split fix is refined from a blanket hide to per-context: body:not([data-view=sessions]) .zone-hunt/#hd-taskforce/.zone-mission/#hd-forge display:none. A FULL GRAIL BIBLE link in the hub opens the bible pane for deep data. Demo J1 updated (PANE_TABS drops session - Sessions no longer a bible pane); demos 7/7. Verified: invariant GREEN, JS parse 0, brace delta 0, view toggle DOM-confirmed (sessions: hunt shown/stage hidden/bible link/no shell-open · tvd: cockpit restored). R4 next = the chronicle build (NEXT GRAIL hero + grail meter + three movements) in the Sessions view. control_ui.html + demo_console.mjs.
