...
VERSION = "v1201"   # 🩹 FUNNEL round 5 — closer-loop CLOCK-SKEW (serial → biggest blast radius): _kai_closer_loop's two receipt-wait loops (funnel-fire ~120s, super-analyze ~40s) are pure time.time()-delta pacing loops. A backward NTP/sleep-wake jump mid-wait makes time.time()-anchor negative → the loop polls until real wall-time claws back past the window PLUS the jump = multi-minute stall — and the closer is SERIAL, so it blocks the ENTIRE rest of that reel's post-seal pass (other tabs' gap-funnels, judge ping-pong, kai_report write). FIX: monotonic anchors for both deadlines. Verified per-spot: _t0f is DUAL-PURPOSE (also compared vs a persisted wall-clock journal field) → kept wall-clock + added separate _t0f_mono; _t0w pacing-only (clean swap); _t0s untouched. Flagged _intake_lease TTL as the same class (next round). +4 tests (control 99→103). ×3 parity (29/80 → v1252)
