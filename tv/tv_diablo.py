...
VERSION = "v1232"   # 👻⏪ GHOST MODE arc round 3 + overlap fix — THE TRUTHFUL REWIND BEGINS: grail-by-date. Scrub the Time Machine into the past and a purple strip shows your REAL grail count as of that moment ("⏪ THE GRAIL · as of <date> · N/403 uniques found by this moment · from your found-log · real & exact for the found-once-kept case"), N = count of d2r_foundLog entries stamped <= cursor ts (verified monotonic 239 by Jun1 -> 351 by Aug1). Truthful per the matrix: retro-only (hidden at NOW), absent if no found-log (never faked). PLUS the live v1231 header bug Konyo caught FIXED: the 👻 GHOST MODE switch was overlapping the Tools tab + bleeding into the clock at narrow widths (header overflow) -> now the switch has its OWN divider-separated slot (compact icon-only when tight, full label when roomy) and the tab rail compacts/scrolls in its own lane -> NO overlap 1000-2560px. Pure add-on, GHOST-off = clean live console, zero-trace intact. control_ui.html only. GHOST MODE 3/50+. ×3 parity
