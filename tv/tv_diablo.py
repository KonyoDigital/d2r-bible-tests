...
VERSION = "v1314"   # 🎨 BIBLE VISUAL-LOCK typography pass 1 - --fw token foundation + 5 flagship headers (extends Konyos flagship-look to the board). INVESTIGATION: bible.html already has --fs-* (5 size tokens, used 694x - size system solid) but ZERO --fw-/--ls-/--lh- tokens; the drift is entirely font-weight - 733 raw font-weight:NNN literals (700x290, 600x223, 800x169, 400x31, 500x13, 900x7) + ~30 !important header rules. Exactly the gap sessions-visual closed on the console. THIS PASS (conservative, flagship-first, identity swaps only): established the --fw-* set in :root mirroring the console (regular 400 / normal 500 / medium 600 / semibold 700 / bold 800 / black 900 - all identity values); folded the 5 core shared header/title classes that render the flagship headers across EVERY tab (.sec-h-t section headers · .boss-name card headers · .gic-name item-card names · .tvf-title tab-view titles · .va-title integrity banner) -> var(--fw-semibold)/var(--fw-medium). PROVABLY SAFE: var(--fw-semibold) computes to exactly 700 (the value it replaced) = byte-for-byte identical render. Verified: tokens carry identity values, count conserved (733 -> 728 raw + 5 tokenized), 16 inline scripts clean, diff exactly 6 edits (1 token def + 5 folds, 6/6). DEFERRED (follow-up rounds): the 728 remaining weight literals (fold in themed batches toward 0 residual like the console), --ls-*/--lh-* tokens (bible has 0 today), the !important header rules (audit separately). bible.html only. x3 parity.
