...
VERSION = "v1209"   # 🩹 FUNNEL round 10 — sibling torn-write: gate_pingpong.json (per-reel retry-tries state — its own docstring promises "a reel never retries forever") was written with bare open(path,"w")+json.dump — the EXACT torn-write shape _kai_write_report_atomic was built to fix for kai_report.json (v948.17 Grok P0-3), never applied to this sibling. A crash mid-write truncates it; the read side swallows a bad parse into _pp_tries={} (forgetting every frame's try count) → an already-pinned "honest miss, tries maxed" frame gets re-queued + re-tried past its cap after a crash (API-cost/retry-budget concern, not grail-count). FIX: route this write through the existing _kai_write_report_atomic (tmp+os.replace) — call-site swap, no new code. (3 other angles swept clean: no other latest-session attribution · dedupe correct · register session-filtered · coveragePct math bounded [0,100].) +1 test. ×3 parity (37/80 → v1252)
