...
VERSION = "v1208"   # 🩹 FUNNEL round 9 — receipt→WRONG-REEL boundary: /intake_result journaled a straggler receipt under whatever sessionId appeared LAST in the journal (the route exists because the board POSTs to the :17771 bridge which dies at session end, so control's longer-lived HTTP server catches post-end receipts). Gap: in Konyo's back-to-back farming, session A's slow intake POST can land AFTER session B has already logged a row → the A straggler gets tagged onto B, polluting B's sess_rows + making _kai_stage3_gap_funnels' receipted set treat a tab B never photographed as covered (suppresses a real gap-funnel). FIX: extract sid straight off frameId (reel_<sid>/<n>_<ms>, session already encoded in frame identity — timing-immune) when reel-shaped; fall back to the exact original journal-scan for non-reel board calls (bible.html vault/tally, unaffected). +3 REAL end-to-end tests (isolated tempdir). ×3 parity (36/80 → v1252)
