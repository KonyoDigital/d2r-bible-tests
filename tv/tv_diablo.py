...
VERSION = "v1338"   # 📊 F2 READS SPARKLINE / SESSION-VITALS RIBBON (sessions-visual, Phase F - the cockpit joins the flagship polish). A live-vitals ribbon on the RIGHT of the Agent Mind ticker (the always-visible bottom cockpit bar), shown ONLY during a scan (on-air): a mint reads-activity SPARKLINE (reads-per-poll deltas over the last ~40 samples, area fill + non-scaling stroke - the scans rhythm, bursts vs idle, at a glance) + ⚡ reads/hr · 👁 reads · ⏱ elapsed (mono tabular, complementing the stage meters current-scene/reads with the TREND + RATE they lack). LIVE + HONEST: fed by paint() each poll (reads buffer capped 40); reads/hr shows "—" until ~5s elapsed so a tiny window cant produce a wild extrapolation; HIDDEN entirely off-air/sim (CSS-gated by [data-state]) so the resting console stays slim/no-scroll - the ribbon lives in the tickers existing height, zero layout cost. Mint on-air palette, var(--fw-*) (invariant-safe), dataviz principles (area fill, emphasized trend, tabular numerals). Verified: mocked /api/status on-air w/ climbing readCount -> ribbon renders, reads climb (547), reads/hr computes (5169) once elapsed meaningful, elapsed ticks (6:21), sparkline populates; off-air -> hidden. JS 0 errors, visual-lock GREEN. control_ui.html only (48/0). x3 parity.
