...
VERSION = "v1357"   # 🔍 E4 ENGINE HALF (Grok audit) - sealed-vs-liveRing honesty, engine side. The retro _engmap build now routes through _kai_engine_frame_effective - the previously test-only sealed-wins law is now the ONE production law (one-statement swap). It enforces the kaiVer>=3 gate at serve time, so a sub-Phase-C reel yields NO authoritative frames (honest-absent) -> the UI shows provisional, never a weak guess as sealed; and it kills the inline-vs-tested-law drift. Verified: 8/8 E4 test - kaiVer-4 (all reels Konyo actually has) -> _engmap IDENTICAL to the old inline path (owner/verdict/why/layers/scene/tab/area preserved = ZERO behavior change), kaiVer-2/1 -> no authoritative frames (honest-absent), kaiVer-3 boundary authoritative, missing-kaiVer safe, no-key empty; 156/156 test_control; 181/181 test_routes (the law tests still green, now exercised in prod too); invariant GREEN; py_compile OK. UI half (sessions-visual): consume beat.engineFrame, key owner/verdict on PRESENCE + sealed===true (not owner-truthiness), never fall back to st.liveRing on a scrubbed retro frame, honest-absent when no engineFrame - in flight. control_app.py only. x3 parity.
