...
VERSION = "v1285"   # 📅 SESSIONS FLAGSHIP D10 - SHELF DAY-GROUPING (polish-ui-2, UI). Lightweight full-width date headers over the shelf cards - Today / Yesterday / This week / Older - each with an honest per-group run count, sectioning by each cards t0. COMPOSES with search / filter / sort: groups the filtered+sorted VISIBLE set and the per-group counts re-sum live (all->107 across groups, grail filter->10 re-summing to 10). GRACEFUL COLLAPSE under a non-date sort (Longest / Most found / Most reads) - the day headers disappear to a flat sorted list (day sections would scatter off-chronological); switch back to Newest and they return. Headers redraw on every filter/sort/poll change; pure UI, no engine. Now the shelf has search + filter + sort + day-grouping all composing. demo 7/7 (:17980), JS/brace/tests green, no page errors; probe: Today 30 / Yesterday 43 / This week 34, counts sum = visible (107=107), grail 10=10, non-date sort collapses, Newest restores. control_ui.html only (32/0 additive). x3 parity.
