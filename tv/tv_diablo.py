...
VERSION = "v1332"   # 🔴🗺 D24 LIVE-BANNER NATIVE (sessions-visual; Diablo-language complaint-fix now on the LIVE view too). The "recording now" banners scene chip now prefers the live st.native.label (g3s /api/status native, v1329): 🌀 ENTERING The Pit / ⚔ FARMING · Chaos Sanctuary / 🏛 TOWN · Harrogath, coloured by kind (dl-transition/loot/town). null (dark frame / no live scene) -> falls back to the scene chip (_diabloChip) -> hidden; NEVER shows "unclear" live (per the contract). Verified on-air with native=entering: banner reads "● REC · Recording now · 2:30 · 342 READS · 🌀 ENTERING The Pit · READING" (chip dl-chip dl-transition); null-native falls back to the scene chip (pre-D24 path, unchanged). A portal/loading frame now reads "🌀 ENTERING The Pit" live, not "gameplay/near-black" - the complaint-fix is now retro AND live. JS 0 errors, visual-lock GREEN. control_ui.html only (10/1). x3 parity.
