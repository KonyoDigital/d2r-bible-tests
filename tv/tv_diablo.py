...
VERSION = "v1323"   # 🔒✏ SESSIONS D25 (session notes/naming) + the VISUAL-LOCK invariant test GREEN on both surfaces (weight system LOCKED + test-enforced). D25 (sessions-visual): name a run + add a note right on the dossier - the title becomes an inline editor ("✎ name" -> "Meph 200x" -> Enter), named runs show the custom name + a "SESSION N" eyebrow (number stays discoverable) + a one-line note; persisted to localStorage keyed by the STABLE sessionId (follows the run across re-render/poll/reload); the custom name overrides "Session N" on the shelf card + feeds the search index; unnamed keeps the auto-title. Robust: the 12s poll re-render skips while an edit input is open (typing never wiped); Enter/blur save, Esc cancel, empty clears. Pure UI + localStorage (browser-local - a personal note; cross-machine sync would need an engine field, flagged). VISUAL-LOCK TEST: g3 wrote visual_lock_invariant.py (stdlib, CI-runnable: asserts 0 raw font-weight:NNN + the --fw-* token set on BOTH bible.html + control_ui.html, fails loudly with file:line) - it CAUGHT 3 real control_ui stragglers the console pass missed (1 spaced-syntax .hdc-len 400 + 2 JS-string 700s); team-lead folded them identity-safe (added --fw-regular:400 to control_ui :root; 3 folds), test now EXIT 0 = 0 raw both surfaces, --fw intact. + LOCKED_TYPE_SYSTEM.md (the frozen --fw contract + usage rule + ls/lh "needs Konyos design call" note + history) + BUGS.md VISUAL-LOCK entry. THE APP WEIGHT TYPE SYSTEM IS LOCKED ON BOTH SURFACES + drift-tested. Pre-push wiring deferred to Konyos morning (a red gate mid-run could block the autonomous push chain). control_ui.html (D25 +67/-5, +3 straggler folds) + visual_lock_invariant.py + LOCKED_TYPE_SYSTEM.md + BUGS.md. x3 parity.
