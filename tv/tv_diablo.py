...
VERSION = "v1269"   # 📼🔍 SESSIONS FLAGSHIP D2 - the SESSION DETAIL DOSSIER. Tapping a session card now opens a full-page DOSSIER destination before the Theatre - a list of cards became a place you explore. _sessionDossier(n) from the /api/sessions summary: HERO (cover + Session N + ran-time), 4 headline stat tiles (reads/frames/named/found) + a verdict SEAL (clean / N flagged / unsealed), the FULL WHAT-I-FOUND finds list (D1 cards, click->jump-to-frame), the B8 scene fingerprint full-size, and a VERDICT & STORY chip row (swept/tallies/judged/regrets + class fingerprint + areas). Primary CTA: Open in Theatre; back/Esc to the shelf. Full-viewport page (not the cramped 256px off-air stage). Escape discipline held (dossier topmost in thEscUnwind: Esc->shelf->theatre). TRUTHFUL (honest dash when a field is absent; no coverage field so the 4th tile is found/registered). control_ui.html only (128/4). Floor 156, demo 7/7 (J4 Esc discipline). SESSIONS FLAGSHIP D2. x3 parity
