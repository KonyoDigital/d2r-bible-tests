...
VERSION = "v1231"   # 👻 GHOST MODE arc round 2 — UNIFIED: ONE debugger switch, ENGINE ROOM ABSORBED (Konyo: "engine room integrated to the time machine · console/gaming not mixing with debugging · the debugger switch is enough"). The separate 🧠 Engine-Room rail button is REMOVED from the live console (grep-confirmed gone) — LIVE console = play/scan only, ZERO debug UI. The ONE 👻 GHOST MODE switch (ghost-switch/_ghostToggle) is the sole debugger entry: flip ON → live console RECEDES + dims/desaturates into "the past" (body.ghost-on) AND the debugger opens on top = the WHOLE Engine-Room 20-round toolset (spine/timeline/read-chain/frame-image/drill-down/spotlight/scorecard/live-feed), reframed "👻 GHOST MODE · the debugger · you are viewing the past", the receded live console showing through a starfield veil. Flip OFF (switch/✕/Esc, all via _engineRoomClose which now clears ghost) → ZERO TRACE. Pure add-on, ER tools intact inside. control_ui.html only. GHOST MODE 2/50+. ×3 parity
