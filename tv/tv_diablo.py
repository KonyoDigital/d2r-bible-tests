...
VERSION = "v1340"   # 📸 F5 INTAKE HERO CARD (sessions-visual, Phase F). When a real photo tally lands mid-scan, a premium broadcast confirmation flashes over the stage - 📸 camera + the tab in Cinzel + the count ("📸 RUNES +14"). The intake lanes own violet identity (#b9a5ff, matching the theatre intake ticks), a camera-FLASH + icon-pop on entry, auto-dismiss ~2.6s. Absolute over the stage, pointer-transparent, on-air only - sits beside the existing keep/grail stage-pulse without colliding (that top-center rounded, this centered hero). HONEST-BY-CONSTRUCTION: reads the live intakeRing (/api/status - same feed the LIVE INTAKE chips use, no new fetch); fires ONLY on a NEW real intake (ok!==false AND counted>0) - an errored/zero shot never flashes; BACKLOG-GUARDED: the first poll SEEDS the key WITHOUT firing so a page load / reconnect with tallies already in the ring never replays them, only a genuinely new tally landing triggers it; on-air only, key resets off-air so nothing lingers; reduced-motion guard kills flash+pop+entrance (card still appears, static). Verified: mocked a growing ring - poll 1 (gems already present) did NOT fire, poll 2 (new runes +14) fired "📸 RUNES +14"; violet card + flash centered while the LIVE INTAKE chips update; off-air with a tally in the ring -> hidden, no fire. JS 0 errors, visual-lock GREEN, 0 console errors, --fw-bold (no literals). control_ui.html only (58/0). x3 parity.
