...
VERSION = "v1268"   # 🔗 CHRONICLE-SYNC ladder<->non-ladder (Konyo: the chronicles are the same, sync them; ladder has extra ladder-only runewords; vault+mules stay separate). Investigation corrected the assumptions: on Mac the made-runeword chronicle (rwMade/rwUnmade/grailUnfound) was ALREADY shared (in _WP_FORKED not _LP_FORKED); only d2r_foundLog (dated grail chronicle) + d2r_setPieces + d2r_grailImportReport were still forked. FIX: moved those 3 to SHARED (removed from _LP_FORKED), re-added to the _WP_FORKED concat so the WINDOWS cousin keeps its OWN isolated chronicle (no cross-machine grail bleed). rwVerify KEPT forked (its non-ladder FAIL verdicts Mania/Hysteria are not ladder truth - sharing would re-introduce the block-banner bug it prevents; intentional safe carve-out). One-time GUARDED IDEMPOTENT merge migration (flag d2r_chronSyncMerged_v1): unions foundLog item keys + setPieces from the L. copies into the shared keys (non-ladder stamp kept on collision; both are real dates so nothing invented), adopts grailImportReport if empty, removes the L. copies - NO DATA LOSS, re-run is a no-op. Vault/mules/inventories STILL forked (separate accounts). Ladder-only runewords still render ladder-only on non-ladder (render untouched). Now switching Main<->Ladder shows the SAME grail/set/runeword chronicle. 42 targeted tests green + new tests/v949_chronicle_sync.spec.ts (4). bible.html only (57/2). x3 parity.
