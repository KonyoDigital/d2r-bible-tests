...
VERSION = "v1353"   # 📺 TV-D ENGINE BAY R5 - the last honest-dial wires; every engine + the gate now reads TRUE off g3s contract, honest-absent when it has not produced. SECOND EYE status appends the act-age from eyes.verifyTs ("double-checks in gaps - last check 9s ago"); appended ONLY when verifyTs is real, no age text when 0. KAI status appends eyes.kaiTs + kaiMissed ("driving - closed 45s ago - 2 missed"); honest-null otherwise. KAI GATE dial (sessionHealth.gate) CONFIRMED already honest - hdKaiGate reads gate.{proven,held}, shows "41 proven - 3 held" + guarding when present, stays HIDDEN when both null / key absent (pre-seal); verified both ways, no change needed. That completes the honest-dial wiring: the whole engine bay reads off real fields, dark+reasoned when disconnected, no age/number when it has not acted. Verified: invariant GREEN, JS 0, demo J5 THREE EYES + J6 signal pass. Still open (non-blocking): Konyos Agent-Mind-appears-twice steer (bay status row vs bottom thought-stream ticker). NEXT: the AI-reads receipt feed once Konyo nods g3 reference-ID schema. control_ui.html only. x3 parity.
