...
VERSION = "v1331"   # 🗺 SESSIONS - SURFACE B8 FINGERPRINT + B4 NATIVE LABELS (sessions-visual; the visible Diablo-language win). (1) B8 FINGERPRINT LINE - "🗺 The run · in Diablo terms" on the DOSSIER + a compact line on the SHELF CARD from sm.sceneFingerprint: "⚔ 89% farming (of reads) · 🏛 2 town trips · 🌀 2 portals · 📍 mostly Chaos Sanctuary". HONEST per the contract: farming% labelled "(of reads)" (read-share NOT wall-time); null -> falls back to the old sceneReads% line -> nothing (honest-absent on thin reels). LIVE on real data (sceneFingerprint is in the /api/sessions summary) - portals are FINALLY counted, so "gameplay on a loading screen" becomes "🌀 N portals". (2) B4 NATIVE in the D22 CHAPTERS - the chapter ribbon prefers frame.native.label (kind->icon + dl- colour): 🌀 ENTERING The Pit -> ⚔ FARMING · Chaos Sanctuary ×2 -> 🏛 TOWN · Harrogath -> 🎒 STASH; collapses by native.label, falls back to raw scene when native absent (no regression). (3) HELD the D24 live-banner native (pending g3s /api/status round - already gated v1329, wiring next). Verified: dossier+card show the B8 line, a reel without B8 falls back, native chapters render exactly; JS 0 errors, visual-lock GREEN. DATA NOTE (routed to g3): native is on the per-frame routing rows, NOT the summary classFrames - so B4 chapters are tested-correct but DORMANT on real data until g3 adds native to each classFrame (a small engine round); B8 is the guaranteed-live win meanwhile. control_ui.html only (38/10). x3 parity.
