...
VERSION = "v1239"   # 👻🖥 GHOST MODE arc round 10 - THE GHOST-SHELL REPLICA (Konyo redirect: "a shell on top of the shell", the face is your console not the ER cockpit). Flip ON -> cloneNode(true) the live .shell into #ghost-shell: the WHOLE console (tabs, STANDBY stage, meters, Daily Task Force, Engine Health, rail, Agent Mind) rendered ghosted on top - desaturated + translucent, purple time-machine rim + violet halo + gentle float + a "viewing the past" ribbon; the REAL shell recedes (scale .9, dim) behind it and shows through the starfield = the shell-on-top-of-shell read. The clone is DISPLAY-ONLY: on-handlers stripped, IDs namespaced to data-gh-id, ghost-switch removed from the copy, inert + aria-hidden (verified 0 IDs / 0 handlers / 0 dup-id collisions on the clone). The Time Machine (FILM + JOURNEY + CURRENT + r9 narration + ER deep tools) is now a DRILL-IN opened FROM the ghost shell via "enter the time machine" (z 930), no longer the headline; closing it returns to the ghost shell (no longer exits ghost). ZERO-TRACE sacred: flip OFF (_ghostExit) removes #ghost-shell entirely, real .shell outerHTML BYTE-IDENTICAL before==after (21197==21197), 0 orphaned data-gh-id, body class "", recede CSS-only scoped to the real shell. Pure add-on, truthful, control_ui.html only. SCOPE NOTE: kept full Time Machine as the drill-in this round (scrubbers are ID-wired inside #engine-room; docking them around the replica is the next refinement, film-storytelling preserved at r10_film_storytelling.patch). GHOST MODE 10/50+. x3 parity
