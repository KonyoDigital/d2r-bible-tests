...
VERSION = "v1275"   # 🎬 SESSIONS FLAGSHIP D5 - THE AI DECISION STORY (polish-ui-2, UI). Two new dossier sections. (1) ROUTING FUNNEL: "eye N read -> tag N named -> book N registered -> grail N grails" as a one-glance narrative, DERIVED client-side from the summary already sent (reads/named/registered/grail-tier finds) - works TODAY, no engine dep, grail stage gold-accented (verified 16 read -> 11 named -> 3 registered -> 1 grail on a real session). (2) COVERAGE METER: a bar + "84% coverage - 128 of 153 frames read - 2 gaps"; TRUTHFUL - consumes an engine coverage field and is OMITTED entirely until provided (NOT derived from reads, which are not 1:1 with frames). (3) classFrames MONTAGE: a horizontal strip of scene-labeled frame thumbnails (PORTAL/TOWN/STASH/FARMING), each click -> jump-to-frame; consumes an engine classFrames field, omitted until provided. Both engine-fed sections verified on injected synthetic data (84% bar, scene labels, jump wired) and honest-absent when the field is missing (section simply hidden, no broken UI). ENGINE FOLLOW-UP (v1276, team-lead lane): add per-session coverage:{read,total,gaps?} + classFrames:[{scene,thumb,frameId,ts}] to /api/sessions to light up the meter + montage. demo 7/7 (J2/J4/J7), JS/brace/tests green, no page errors. control_ui.html only (79/1). x3 parity.
