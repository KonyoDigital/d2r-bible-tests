...
VERSION = "v1252"   # 🖥🧹 CONSOLE UPGRADE round 4 - HOME RESTRUCTURE (Konyo: home needs fixing, AGENT MIND sticky/overlapping, no scroll, group by logic + typography). (#1 AGENT MIND overlap FIXED) it was not a z-index bug - the ticker was a full 84-124px full-width bar eating the home vertical budget, crushing the dash into a ~200px internal-scroll box; now at rest it collapses to a slim ~45px bar (streams thoughts only when LIVE, full height on air), no longer crowding the dash. (#3 LOGICAL GROUPING) DAILY TASK FORCE + LAST SESSION (both "today session activity") paired side-by-side; FORGE QUESTS + LIVE INTAKE paired. (#4 TYPOGRAPHY) tighter, consistent card headers = clean hierarchy + reclaimed space. (#2 NO-SCROLL) compacted resting stage hero + tighter padding/gaps: the whole left dash fits with NO scroll at 1920+ (was ~550px scroll). CAUGHT+REVERTED a regression: tightening shell PADDING broke the 0.00px head-tab invariant (home shell pad must MATCH board pane pad for cross-frame tab align) - reverted pad, kept gap, J2 back to 0.00px, left a do-NOT-change note. HONEST FLAG: the RIGHT RAIL still overflows below 2560 (content-volume, ~1330px) - r5 rail-fit to follow. LIGHT (CSS + 1 class change). Did NOT touch ENGINE HEALTH model / anti-lag / fault lamp / parked GHOST MODE / Grok v1251. control_ui.html only (17/8). CONSOLE UPGRADE 4. x3 parity
