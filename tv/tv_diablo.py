...
VERSION = "v1242"   # 👻🎬 GHOST MODE arc round 12 - FILM STORYTELLING resumed as the on-demand inline inspect (rebuilt fresh on v1241s refactor, not force-applied). The inline "inspect this moment" deep view now speaks in the film-storytelling voice: header reframed to "THIS FRAME - what the AI saw and decided", the frame-side sibling of the r9 journey narration = one coherent Time Machine. ON-DEMAND, never in the way (Konyos rule): the deep view does NOT auto-open on scrub - scrub the docked film with nothing open and it stays closed; hit inspect and the AI-mind view opens inline (frame image, read raw->resolved, read-chain, owner/verdict); then WHILE open, scrubbing the film follows it frame-by-frame (OCR/raw enriched as they land). Truthful: only real captured frames + real reads, honest "no item named" gaps, "cleaned up from <raw OCR>" line, no synthesized frames (unarchived frame jpg -> img onerror -> honest "this frame was not archived"). ZERO-TRACE held: flip-off restores er-detail + scrubbers to #engine-room, real .shell outerHTML BYTE-IDENTICAL (21197==21197). One-toggle + docked scrubbers + inline + zero-trace all intact. Pure add-on, control_ui.html only. GHOST MODE 12/50+. x3 parity
