...
VERSION = "v1261"   # 🛡🔬 AUDIT FIX A5 - THE SEALED-WINS GUARD (kills live-leak-into-retro). _erOwnerVerdict now keys on FRAME-PRESENCE not owner-truthiness: was "if b.engineFrame && b.engineFrame.owner" (fell through to liveRing when owner=None), now "if b.engineFrame" -> a sealed frame present is authoritative, live NEVER consulted for it. A sealed owner=None frame (the honest majority: gameplay/transition/miss) now returns an authoritative "no owner (sealed)" carrying the sealed verdict/why, instead of surfacing a LIVE GUESS as the gold/authoritative retro verdict during scrub. Matches the canonical _kai_engine_frame_effective (frame-in-sealed_fs). liveRing is now reached ONLY when there is NO sealed frame (the unsealed now-edge) = correct. Did NOT touch the kaiVer<3 threshold (A2/A3 lane). Unit-tested the exact leak scenario. Floor 525. control_ui.html only (13/3). AUDIT FIX A5. Group A audit fixes now 5/6 (A4 grail-names in flight). x3 parity
