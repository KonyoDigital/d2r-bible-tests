...
VERSION = "v1311"   # 🎬 SESSIONS FLAGSHIP D19 - BEAT CHIPS (sessions-visual, Phase C). A "⚡ The beats" section on the DOSSIER: the runs story as an iconographic color-coded chip ribbon in narrative order - 🔥 entered · ⚔ farmed · 🏛 town · 📸 N tallied · 🏆 <grail name> (one per grail) · 💎 N keeps · 🔬 N judged · 💔 N regrets · 🛡 sealed / 🚨 flagged / ◌ unsealed. Colors reuse the Diablo scene/tier palette (portal orange, town blue, farm/keep mint, intake purple, grail gold, regret red, seal gold). HONEST by construction: derived purely from the session summary (sceneReads + finds + tally/judge/regret/seal counts) - NO fabricated timestamps; its "what the run contained" in a rough narrative arc (enter->farm->finds->seal), every chip OMITS when its data is absent. Real chronological beats would need the frame timeline (loads only while watching) - noted as a future option, not faked. Placed on the DOSSIER (not the D18 card, which is freshly compact), under the seal row above the decision funnel (beats=narrative, funnel=routing math). Pure UI off /api/sessions in hand, type system, overlay scrolls internally (home no-scroll untouched). Verified on a rich dossier: all 10 beats correct + in order (🔥->⚔->🏛->📸3->🏆 Deaths Fathom->🏆 Nightwings Veil->💎2->🔬4->💔1->🛡 sealed), JS 0 errors brace 0. control_ui.html only (41/0). x3 parity.
