...
VERSION = "v1287"   # 🏦📊 E1 - VAULT STATS: attach the AI-Item-Checker VERDICT to muled keepers (Konyo: "the STATS are attached to the items that are MULED, compared to thrown out"). The checker already stored + displayed the read AFFIXES on a muled magic/rare keeper; the missing stat was the JUDGEMENT. Now aicMule() computes the checkers verdict (keep/border/toss + score) via _aicVerdict and attaches it to the magicFinds entry (checked:true, verdict, score, checkedAt) so a checker-VERIFIED mule carries its full read - affixes + judgement + when - not just a name. New _mfChecker(name) reads it back (null for bare intakes / legacy string entries / missing = honest provenance). _magicFindCardHtml now shows a "🔬 AI Checker - KEEP/BORDERLINE/TOSS - score N - checked <date>" badge on the muled items detail card + a "🔬 checked" tag in the subtitle, visibly distinguishing checker-verified mules from bare intakes. TRUTHFUL (only the verdict the checker actually computed from the affixes on the item), additive, backward-compatible (old entries lack the fields -> _mfChecker null -> no badge). Verified: 16/16 inline scripts clean; _mfChecker returns the record for a checked mule and null for bare/legacy/missing. FOLLOW-UP (later): the funnel-2x accuracy reconcile + a thrown-with-stats comparison log. bible.html only. x3 parity.
