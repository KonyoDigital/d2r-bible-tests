...
VERSION = "v1294"   # 🔬 G3 bucket-2 -> AI ITEM CHECKER hand-off (completes Konyos chronicle/else architecture end to end). The "🔬 Item Checker & vault" list was a jump button; now each non-chronicle item is a LIVE hand-off into the checker draft. Per-item: every bucket-2 chip is a clickable 🔬<name> button -> _arSendToChecker seeds the checkers single-item draft via _aicSetDraft (d2r_aicDraft), expands+renders+jumps to the checker card; Konyo then judges -> Mule (vault) or Toss. Seed parsing: a rolled item (affix words of/chaotic/sullied/shimmering/ivory/jewel...) -> the checkers name field; a plain base (Dread Whorl, War Sword) -> the base field (drives art+bench); quality defaults rare. Bulk "🔬 send all -> checker": seeds a persisted queue (d2r_aicQueue), loads the first, shows a "▶ load next (N)" control - one item at a time, queue survives the jump. Did NOT touch aicMule/aicToss (no invasive coupling - clean; send-all advances on the users load-next click by design). Verified: 16/16 bible scripts clean; node test on REAL extracted hand-off fns 12/12 (white base->base slot, rolled->name slot, _arSendToChecker populates draft, send-all seeds queue+loads first, load-next walks, empty clears); regression merge-max+3-bucket still 23/23 (_arComputeDiff unchanged). Caveats: d2r_aicQueue is transient scratch (not forked - not account state); auto-advance on Mule/Toss deliberately not hooked (would couple into another feature). bible.html only (61 ins). x3 parity.
