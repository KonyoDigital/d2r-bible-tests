...
VERSION = "v1290"   # 🎨 SESSIONS FLAGSHIP VISUAL PASS R3 - OFF-AIR HERO OVERLAP FIX + no-scroll-safe caption (sessions-visual agent). Completes the R1 type -> R2 structure -> R3 overlap arc. DIAGNOSIS: the off-air caption/meters overlap was NOT short-viewport-only - it was PERMANENT. .center (absolute, flex-centered, static vw padding guessing at the meters height) and .meters (absolute, bottom-pinned) were independent layers blind to each others size, so whenever the caption wrapped to 2 lines - which the DEFAULT standby copy already did - the centered content spilled straight OVER the MODE/READS/AREA/SCENE meter tiles (44px overlap measured). The team had papered over this for ~10 versions by shrinking fonts/stage. FIX A (off-air only, deterministic): the stage becomes flex-column - .center (flex:1) owns the space ABOVE the now-in-flow .meters, the marquee stays absolute with the centers top-padding reserving its band -> marquee > hero > meters can NEVER overlap at any viewport/caption length. ON-AIR / SIM fully untouched (meters stay the absolute broadcast lower-third over the film). B1 (folded in for no-scroll safety): the DEFAULT standby caption shortened to ONE line, Konyos voice kept - "Press ON to wake the hidden scanner and take the board live." -> "Press ON - wake the hidden scanner, go live." (HTML default + JS standby branch matched) so the default stage is 258px (256 floor +2, HEIGHT-NEUTRAL, no-scroll doctrine preserved); the +37px only remains for genuinely multi-line transient captions (e.g. D2R-window-missing, stage 362px, still never overlaps - bounded correctness exception). Verified live (throwaway port): default 1 line / all 3 gaps positive / no overlap; forced 4-line still no overlap; JS 0 errors brace 0. KPI band stays untitled (no fragile absolute label into the gap just cleared - an untitled hero stat band is a legit flagship pattern). control_ui.html only (+21/-3 since v1288). x3 parity.
