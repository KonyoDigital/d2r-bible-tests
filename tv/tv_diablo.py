...
VERSION = "v1235"   # 👻🔨 GHOST MODE arc round 6 - THE FORGE LAYER (Option A: truthful current-state, never a faked past). d2r_forgeSummary is a snapshot derived from the un-timestamped stash, so it genuinely CANNOT rewind (no per-moment forge history); a rewound Forge would lie. So: a THE FORGE strip ("what you can make right now") with an amber CURRENT / NOT REWOUND badge that sets it apart from the rewindable FILM/JOURNEY layers - shows N craftable now + M one-step-out (+ item names, honest "nothing craftable" when empty) + a plain note that the stash is not time-stamped so THE FORGE cannot rewind (craftability AS OF NOW, real and current, not a rewound past). It STAYS PUT when scrubbing film/journey (verified before==after) - never pretends to rewind. Three layers now read clearly: FILM (rewind), JOURNEY (rewind), FORGE (current, honest). Option B (partial intake-only timestamped rewind) flagged NOT built - needs Konyo go on an additive record-path snapshot (rule #1). Truthful, pure add-on, GHOST-off = zero trace. control_ui.html only. GHOST MODE 6/50+. x3 parity
