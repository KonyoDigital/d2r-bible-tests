...
VERSION = "v1236"   # 👻🗃 GHOST MODE arc round 7 - THE TALLIES + VAULT LAYER (completes matrix coverage). Tally/vault data lives in the board (bible iframe), read live read-only via _runeCount/_gemCount/_materialCount and the owned Set size. All are current-only maps with no timeline, so they genuinely cannot rewind. Grouped FORGE (r6) + TALLIES + VAULT into ONE "CURRENT STATE - not rewound" section (one amber badge, same visual language as r6, apart from the rewindable FILM/JOURNEY): FORGE (N now, M one-step-out), TALLIES (runes/gems/materials live from board), VAULT (items held via owned.size), plus an honest note that stash and vault are not time-stamped so these cannot rewind (holdings AS OF NOW, real and current, not a rewound past). STAYS PUT on scrub (verified) - never fakes a past; unreachable rows simply do not render. Debugger now cleanly two families: TIME MACHINE (rewind: FILM + JOURNEY) vs CURRENT STATE (not rewound: FORGE + TALLIES + VAULT). Truthful, pure add-on, GHOST-off = zero trace. control_ui.html only. GHOST MODE 7/50+. x3 parity
