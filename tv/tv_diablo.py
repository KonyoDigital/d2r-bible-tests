...
VERSION = "v1318"   # 🎞 SESSIONS FLAGSHIP D22 - FILMSTRIP CHAPTER MARKERS (sessions-visual, Phase C). A "🎞 The reel · chapters" jump-to-chapter ribbon above the classFrames montage on the dossier: the ordered classFrames collapse into scene CHAPTERS (consecutive same-scene frames = one chapter, e.g. "⚔ FARMING ×4"), color-coded by scene (🔥 portal · ⚔ farming · 🏛 town · 🎒 stash) with sequence arrows; each chapter clickable -> jumps to its first frame = the runs structure at a glance. Plus FIND-MARKERS on the montage tiles: a 🏆/💎 badge appears on a frame ONLY when a real finds frameId EXACTLY matches that frame - truthful, never guessed (in test only Deaths Fathom f5 marked; Nightwings had no frameId -> no marker). Refactored scene-labeling into a shared _sceneLabel helper (chapters + montage both use it). HONEST LIMIT: a full read/find timeline ALONG the strip would need engine ts-aligned beat data; classFrames are scene-representative frames not a continuous timeline - so the honest version: real scene chapters (transitions we truly have) + find-markers on exact frame matches; chapters omit when <2 (single-scene run). Pure UI off classFrames + finds in hand, type system (reuses the dl- scene palette), filmstrip scrolls in its own container (home no-scroll untouched). Verified on an 8-frame session: 🔥PORTAL->⚔FARMING×4->🏛TOWN->🎒GEMS->⚔FARMING (collapse correct, 4 arrows), 🏆 marker only on f5, 8 tiles, JS 0 errors brace 0. control_ui.html only (54/11). x3 parity.
