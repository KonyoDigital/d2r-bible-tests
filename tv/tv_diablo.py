...
VERSION = "v1296"   # 🎨 VISUAL LOCK typography - letter-spacing + line-height token completion (sessions-visual). The console type system is now COMPLETE and single-sourced: sizes + weights (v1293) + NOW letter-spacing + line-height, all on role-named tokens. Added/expanded: 7 --ls-* roles (tight .02 / snug .04 NEW / normal .06 / wide .10 / label .14 NEW / wider .20 / widest .28; reseated --ls-wide .12->.10 + --ls-wider .18->.20 to fit the real bands - safe, 1 existing ref) and 6 --lh-* roles (none 1 / tightest 1.1 / tight 1.2 / snug 1.35 / normal 1.5 / relaxed 1.65; 0 existing refs). Aliased --hd-head-ls -> var(--ls-label) so the header token chains to the scale. Folded 254 literals (149 ls + 2 lh props + 99 lh-in-font-shorthand + 4 inline theatre-header styles); near-dup collapse capped <=.04em ls / <=.05 lh (visually identical), every collapse documented inline in :root. One-offs kept literal + annotated /* intentional one-off */ (.42em widest kicker, .96 display lh, 1.9/1.72/1.85 credits+overlay reading blocks, solid 1). Verified: JS 0 errors, brace 0, tokens resolve (--ls-label .14em, --lh-snug 1.35), console IDENTICAL on throwaway port, NO-SCROLL intact (line-height folds added ZERO height, off-air hero at 256 floor, no overlap). Remaining Phase-G-proper = the bible.html-side + cross-app pass (v1341+). control_ui.html only (219/209). x3 parity.
