...
VERSION = "v1230"   # 👻 GHOST MODE arc round 1 — THE SWITCH + THE GHOST-SHELL CONTAINER (the console TIME MACHINE begins). A 👻 GHOST MODE switch in the header = the console's ONE debugger switch. Flip ON → body.ghost-on RECEDES the whole console (perspective scale .9) + grayscales/dims it into "the past" behind a starfield, with a "you are viewing the past" banner + "✕ back to live" exit + a Time Machine SCRUBBER at the bottom showing the REAL recorded timeline (79 ticks from /api/sessions, playhead at NOW). Flip OFF (switch/exit/Esc/click-starfield) → overlay hidden + body.ghost-on dropped = ZERO TRACE (proven: body class "", .shell transform/filter none, identical outerHTML). Pure ADD-ON — live console NEVER touched; Engine Room untouched. No rewind wiring yet (truthful: "drag-to-rewind lands next"). control_ui.html only. GHOST MODE 1/50+ (v1230→). ×3 parity
