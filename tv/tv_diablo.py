...
VERSION = "v1316"   # 🎨 BIBLE VISUAL-LOCK typography pass 2 - chip/badge/table-header batch. Folded the chip/badge/tag/pill family + table headers (one cohesive visible theme) - 31 weight literals across 31 single-line CSS rules onto --fw-* (.pill/.filter-pill.active/.tc-badge/.tlg-badge/.aura-tag/.sun-pill/.rwc-chip/.vj-chip/.source-chip.best/.aid-tabbar/.nav-chip.active/.ti-chip/.lt-chip/.sig-badge/.gf-chip/.rwn-chip/.pf-chip/.vrg-tag/.tc-apply/.tvb-badge + table.drops th/.ref-tbl th/.sunder-tbl th/.zd-hell-grid thead th/.gic-source-table thead th). Distribution: 13 semibold(700) / 15 medium(600) / 8 bold(800). PROVABLY SAFE: count conserved (733 -> 697 raw + 36 tokenized), every changed line a font-weight identity swap (0 non-font-weight changes on the diff), each token equals the literal so byte-identical render, 16 inline scripts clean, diff 31/31 tight; scoped to single-line CSS rules whose selector matches the family (multi-line + JS-string weights untouched/deferred). No !important weight fights in this batch (the ~15 font-weight:600!important header rules are a separate cluster, audited when reached). Residual 697 raw weights remain, marching toward 0 like the console. bible.html only. x3 parity.
