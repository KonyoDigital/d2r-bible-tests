...
VERSION = "v1341"   # 📈 F6 LIVE INTEREST GAUGE - money-moment needle (sessions-visual, Phase F). A compact semicircular needle gauge, leftmost in the on-air vitals ribbon (the F2 cockpit bar), driven by the engines REAL interest signal (0..1) already on /api/status. The needle swings left->right up the arc as interest climbs - "how badly the AI wants to look right now". Gradient arc track (dim->mint->gold), swinging needle, hub dot, "INTEREST" label. HONEST glanceable levels (off the real value, not fabricated): calm (<.4) needle left/muted green; warm (.4-.7) rising/mint+glow; hot (.7+) up-right/GOLD/hub pulses/label gold = the money-moment; UNKNOWN (interest absent from payload) needle parked left/dimmed grey - does NOT fabricate a 0/"calm", honestly shows "no signal". Needle motion = smooth CSS transition; reduced-motion guard kills the swing + the hot pulse. LIVES IN THE EXISTING CHROME: rides the F2 vitals ribbon (same on-air-only gate, hidden off-air), no new fetch (window.__lastStatus.interest each poll from _tkVitals), zero added layout/scroll. Verified: stepped interest 0.12->0.55->0.9->absent - needle mapped exactly (0.12->-68.4°/calm, 0.9->+68.4°/hot-gold, absent->parked-left/unknown), class+color transitions correct; gold up-swung (hot) vs left-muted (calm). JS 0 errors, visual-lock GREEN, 0 console errors, --fw tokens only. control_ui.html only (49/0). x3 parity.
