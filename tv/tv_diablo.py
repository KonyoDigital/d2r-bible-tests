...
VERSION = "v1228"   # 🧠🖥 ENGINE ROOM arc round 19 — FRAME-STEP SMOOTHNESS (the r6 "watch reads evolve" payoff was janky). Render is ~10ms/step but the frame IMAGE janked: every step rebuilds the detail innerHTML, so the <img> is destroyed+recreated (blank, complete:false) and the box collapses to 0 height while the new image fetches — holding ◀▶ flickered blank→load + the panel jumped each frame. FIX: (1) the frame image RESERVES its box via aspect-ratio (2940/1912 capture ratio) + a faint placeholder bg, so it never collapses/jumps mid-load (measured stable 247px box); (2) on each render, PRELOAD the adjacent frames' images (prev+next) → stepping shows the next frame INSTANTLY from cache (measured 0ms vs a network fetch + blank flash). Holding ◀▶ is now smooth: no flicker, no jump, images pop. Pure client-side (cache-warm + CSS aspect-ratio). control_ui.html only. ×3 parity (56/80 → v1272). [r20 FINALE = whole-cockpit cohesion pass]
