...
VERSION = "v1320"   # 🎨 BIBLE VISUAL-LOCK typography pass 5 - JS-string inline weights (98.8% tokenized). Folded all 113 JS-string inline-style weight literals (style="..font-weight:700" inside template strings that build cards/rows/chips at runtime) onto --fw-*, across 88 lines. All confirmed CSS-in-JS inline styles; var(--fw-*) in an inline style attribute resolves against the :root cascade -> same computed weight. PROVABLY SAFE: count conserved 122 -> 9 raw + 724 tokenized (9+724=733); every changed line a font-weight identity swap (0 non-font-weight changes); 16 inline scripts new Function-compile clean - CRITICAL this batch since folds are INSIDE JS string literals, but var(--fw-semibold) introduces no quotes so string/JS syntax is unaffected (verified); diff 88/88. Residual 9 (from 733) = weight tokenization 724/733 = 98.8% - only the 9 CSS multi-line edge cases remain (font-weight on a rules continuation line; single-line scope safely skipped them -> hand-check next round, not a blind fold). NEXT: pass 6 the 9 multi-line hand-checks (-> 0 residual) + the ~15 font-weight:600!important header cluster audit; pass 7 mirror --ls-*/--lh-* from the console; final getComputedStyle render-verify then the bible type system is locked to match the console. bible.html only. x3 parity.
