...
VERSION = "v1245"   # 👻✨ GHOST MODE arc round 14 - FEEL ALIVE layered on the clean 3-panel structure (decluttered AND alive). THE TRANSITION: flip on = overlay fades in + the ghost replica materializes (rise + un-blur) as the real console recedes; flip off = reverse plays then teardown. Calm + quick, prefers-reduced-motion honored, all animation on #ghost-shell (removed on exit = zero lingering state). KEYBOARD SCRUBBING: <-/-> step the ACTIVE scrubber, up/down jump coarse. Film: frame / reel (skips empty idle reels). Journey: grail tick / ~7 ticks. Click a scrubber to activate; a subtle rim + keyboard hint marks which (film default); film keys also auto-open the inline forensics for that frame. Reconciled the active-scrubber selectors + click listener to the new #gsh-tm-body panel. FIDELITY: deliberately did NOT add the rich duotone-wash/strong-halo/brighter-ribbon (would fight the v1244 de-clutter) - kept the CALM replica treatment, legible-and-calm over rich-and-noisy, per the clarity-wins steer. ZERO-TRACE held through the animated exit: flip off restores all moved nodes to #engine-room, #ghost-shell removed, real .shell outerHTML BYTE-IDENTICAL (21197==21197), 0 gsh-scrub-active / gsh-closing left. One-toggle + docked + inline + truthful labels intact. control_ui.html only (93ins/12del). GHOST MODE 14/50+. x3 parity
