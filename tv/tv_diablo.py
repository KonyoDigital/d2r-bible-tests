...
VERSION = "v1227"   # 🧠🖥 ENGINE ROOM arc round 18 — NUMBER-CONSISTENCY fix (the cockpit's own numbers must agree or Konyo distrusts it). Consistency sweep found a REAL disagreement: for the SAME reel, the scorecard said "🧠 5 retro-caught" but the flag bar said "3" — same label, two numbers. Root cause: the flag bar classified each frame by ONE primary kind in priority order (held>super>caught>stall), so frames that were BOTH gate-held AND KAI-retro-caught counted only as "held" — undercounting retro-caught (scorecard counts kaiMissed independently = 5). FIX: _erReelFlags counts each flag kind INDEPENDENTLY (held + retro-caught are overlapping properties, not exclusive), so flag-bar retro-caught (5) now agrees with the scorecard (5); `total` stays DISTINCT flagged frames (jump set unchanged), only per-kind tallies became honest (kinds may sum > total given overlap = correct). Also swept: live-feed owner/verdict, gate reasons, scorecard metrics — all already agreed. control_ui.html only. ×3 parity (55/80 → v1272)
