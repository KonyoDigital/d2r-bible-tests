...
VERSION = "v1258"   # 🛡🖥 AUDIT FIX A1 - STASH-PANEL-OPEN / IS-D2R GUARD (kills the wallpaper->Gems false positive the swarm caught). Root cause CONFIRMED by reading the frames: a Screen-Recording-TCC drop captured the Mac desktop wallpaper (HK night skyline) instead of D2R; the vivid city lights (frac_chroma 0.736) tripped classify_stash_grid high-chroma gems branch, which had NO is-panel-open precondition -> 69 desktop frames across 2 reels sealed stash-gems -> phantom tally:gems read 0. FIX (engine-only): a cheap geometry guard from the existing pixel pass - real stash chrome = a dark-cell band (frac_dark 0.12-0.62) + grid lattice (dark_cols>=4 dark gridline columns; real gems=19, wallpaper=0); a not_d2r fast-reject (frac_dark<0.05 = a smoothly-lit photo) returns gameplay before ANY stash/vault label. Added "and panel_open" to all four tally branches (high-chroma gems / gems-multihue / runes / materials). VERIFIED: wallpaper reels s_1784734976651 (39f) + s_1784734921329 (30f) -> 0 stash-gems (all gameplay); REAL gems s_1784736270319 -> stash-gems x5 (incl. the task frames f_1784736381363 + f_1784736379052); good reel s_1784636825977 -> gems x10 + runes x10 UNCHANGED. Floor 520 (+9 tests incl. the router invariant _router_conf([grid])==1). DEFERRED as scoped follow-ups: A2 phantom-ocr gate honesty (needs a sanctioned single-signal grid route; the guard already stops FALSE grid reads from reaching that path) + A3 kaiVer 3->4 re-seal (best done with A2). stash_eye.py + tests (NOT control_ui.html). AUDIT FIX A1. x3 parity
