...
VERSION = "v1221"   # 🧠🖥 ENGINE ROOM arc round 12 — READ TEXT vs FRAME (the core forensic comparison: what it SAW vs what it READ). The r7 frame image shows what the AI saw, but what it READ was scattered across evidence rows below — never a headline beside the frame. FIX: a "📖 THE AI READ THIS FRAME AS" callout at the top of the hero's right column, beside the frame image, resolving to the OWNING layer's output (KAI-caught text / super-recovered names / live names, fallback to any read text on the beat) — so it shows what the layer that WON actually read. Konyo compares frame ↔ read at a glance: on a KAI-caught frame it reads "HAkD LEATHEA Akm•A" next to the game frame, so a garbled/wrong read is instantly visible against ground truth (raw OCR shown intentionally = the debug value). Honest empty on gameplay frames ("— no item named"). Caught+fixed a 1-char er-auth class concat regression mid-edit before ship. control_ui.html only. ×3 parity (49/80 → v1272)
