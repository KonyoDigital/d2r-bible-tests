...
VERSION = "v1241"   # 👻🔬 GHOST MODE arc round 11b - INLINE INSPECT (the last no-routing piece). Konyos "one toggle, no routing" fully honored now. Before: "inspect this moment" opened the full-screen #engine-room overlay that REPLACED the ghosted-console view (the thing Konyo dislikes). Now: the er-detail node is docked alongside the scrubbers so inspecting renders the deep forensics (frame image, read, read-chain, owner/verdict, evidence) INLINE right on the ghosted console - #engine-room stays hidden (no overlay), the ghost shell stays visible (view never replaced), scrubbers stay put. Closing the inline detail keeps you in the ghost console; a second Esc exits ghost. Deep view optional + inline, never the required path, never routes. ZERO-TRACE held: flip-off restores er-detail AND scrubbers to #engine-room, removes #ghost-shell, real .shell outerHTML BYTE-IDENTICAL (21197==21197), 0 orphaned data-gh-id. Pure add-on, truthful, control_ui.html only (9ins/5del). GHOST MODE 11b/50+. x3 parity
