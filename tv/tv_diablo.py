...
VERSION = "v1244"   # 👻🧹 GHOST MODE arc round 13 - STRUCTURE & CLARITY pass (Konyo: "the time machine is cool but can we structure it more clearly? it looks messy"). The dock now reads as THREE distinct panels instead of a flat wall of stacked strips (layout/visual only, zero behavior/data change). BEFORE: one column - TIME MACHINE title, FILM, read-chain legend, JOURNEY, narration, REEL scorecard, CURRENT STATE, inspect - all similar strips, no grouping. AFTER: a clean 2-column dock - (1) THE TIME MACHINE = one bordered purple panel with a clear header grouping THE FILM + THE JOURNEY + narration as the rewind controls (per-band accents softened to hairlines since the frame already says "rewind", so borders/badges stop fighting); (2) CURRENT STATE = the not-rewound companion moved to its own amber card in a side column, clearly distinct, no longer buried at the bottom; (3) INSPECT = the deep frame view now has its OWN dedicated full-width zone at the foot that COLLAPSES when closed + expands in place, so auto-open/close never shoves the scrubbers around. Consistent 16px gutters, roomier padding, one calm frame per region, uppercase panel header; side column drops below on narrow (<900px). r13 feel-alive (transition/fidelity/keyboard) preserved as r13_feel_alive.patch to resume on this cleaner base. ZERO-TRACE held: flip-off restores all 4 moved nodes to #engine-room, real .shell outerHTML BYTE-IDENTICAL (21197==21197). One-toggle + docked + inline + truthful labels intact. control_ui.html only (54ins/21del). GHOST MODE 13/50+. x3 parity
