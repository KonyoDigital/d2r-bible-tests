...
VERSION = "v1365"   # 📋 SESSIONS R3 - the rail move (hunt leads off-air). On the Sessions hunt-hub OFF-AIR, the live AI-ENGINES bay (just dark resting rows off-air, competing with the hunt for the whole right column) now COLLAPSES to one quiet dashed line "THE SCANNER · asleep - press ON to wake the engines" - so YOUR HUNT + the Task Force own the eye and the rail is just controls. ON AIR reachable (the primary ON AIR button + Theatre + Shelf stay). Purely a body[data-state=off] treatment - ON-AIR/TV-D is UNTOUCHED (go live -> the full 7-engine bay is back exactly as before, verified). Flagged for Konyos veto (keep = clean hunt-leads / dimmed-mini-bay / hunt-content-in-rail are his options). Verified: off-air -> one asleep line + ON AIR/Theatre/Shelf visible; on-air -> 7 rows back untouched; j6 finds Signal+Watching; no-scroll; invariant GREEN, JS 0. control_ui only. x3.
