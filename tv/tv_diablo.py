...
VERSION = "v1337"   # 🎬 SESSIONS FLAGSHIP D31 - COHESION PASS (the FINISH of the Phase C Sessions D-arc). Dead-code cleanup: removed 4 vars left from the D18 card redesign (art/verdictLine/fpLine/findsLine) - the cinematic card renders from heroOpen + vparts + _fp + the stat grid and dropped the inline finds-strip, so all four were unused; findsLine was needlessly running _findsStrip() on EVERY card render (a real per-frame waste on the shelf) - gone. Full-surface cohesion verification: exercised the ENTIRE Sessions surface in one pass, renders as one system with ZERO console errors on load AND during interaction - shelf (cards: cover=best-find · pin · name · verdict seal · B8 fingerprint + the 5-card highlights strip), dossier (8 sections working together: since-last deltas · wax seal-stamp · beats · grail-progress · decision funnel · native chapters · finds · Diablo-terms fingerprint · recap/pin/name actions), compare overlay (8 rows), area heatmap, live banner native - all open/close clean. THE D-ARC COMPLETE: D14 compare · D16 heatmap · D17 highlights · D18 cinematic card · D19 beats · D20 count-ups · D21 seal-stamp · D22 chapters · D24 live card+native · D25 notes/naming · D26 pin · D27 recap · D28 grail-progress · D29 since-last · D30 cover=best-find · D31 cohesion + B8 fingerprint + B4 native. Every round honest-by-construction (null->hide, no fabricated numbers), var(--fw-*) invariant-green, no-scroll intact. JS 0 errors. control_ui.html only (3/4 net -1). x3 parity. THE SESSIONS TAB IS FLAGSHIP.
