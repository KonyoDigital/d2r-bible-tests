...
VERSION = "v1342"   # ✅ COCKPIT COHESION PASS - PHASE F FINISH (the live console is now flagship, like D31 for Sessions). Full-surface sweep of the LIVE console; VERDICT: reads as one coherent flagship system on-air AND off-air, ZERO console errors both states (incl fresh-load + the NO-SIGNAL fetch-fail path). SWEPT + CLEAN: no-scroll intact (page body overflow 0 both states; the right rail is an intentional pre-existing overflow container, not F-drift); Agent Mind ticker (F3) all 4 lines fit (scrollH==clientH), icons + honest accents render, grail gold pops; vitals ribbon (F2+F6) gauge+sparkline+reads/hr/reads/elapsed on-air-only (CSS-gated hidden off-air), needle maps calm/warm/hot/unknown exactly; intake hero (F5) transient at top:40% vs keep/grail pulse at top:14px (distinct, no overlap, both pointer-transparent); reduced-motion EVERY F-anim guarded (F3 slide/pop, F5 flash/cam/entrance, F6 needle+pulse); palette cohesion (intake violet #b9a5ff across theatre ticks + F5 hero; mind accents + gauge on the gold/mint/violet vocabulary, no off-palette); no dead code (every _mindThought accent branch + gauge level resolves to a defined class). THE FOLD (D31-style): the gauge tkvg-calm level had NO explicit CSS rule (worked by fall-through to the base green, but read as "missing" to a CSS scanner while warm/hot/unknown were explicit) - added an explicit .tkvg-calm rule (restates base green, no glow) + a comment so all 4 honest levels sit in one place; calm renders byte-identical (stroke #8fb89a, filter none). JS 0 errors, visual-lock GREEN, 0 console errors both states. PHASE F COMPLETE - the console + Sessions are both flagship, the type system locked+tested both surfaces, the engine certified. control_ui.html only (+3). x3 parity.
