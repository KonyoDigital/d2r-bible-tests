...
VERSION = "v1257"   # 🗓🏆 CONSOLE round - INBOX CLEAR-ALL + MULTI-CHRONICLE TASK FORCE (two Konyo asks). (#1 INBOX CLEAR ALL) a "clear all" control sits on the Chronicle inbox button (rail): one click -> a light confirm ("Clear all N pending? dismissed from review, NOT written to grail, re-review from the session anytime") -> dismisses the whole pending queue via the board path (kaiChronicleDismissAll or a loop), count -> 0; only shows when pending; stopPropagation so it never opens the modal. Easy on purpose, safe on accident. (#2 DAILY TASK FORCE MULTI-CHRONICLE, Konyo "what about my other chronicles?") the Task Force now shows ALL chronicles and steers the daily focus at the INCOMPLETE ones (where the hunt is): Runewords 99/99 complete (muted DONE), Grail Uniques X/403 with "N left to hunt" + % (gold hunt accent), Sets X/135 with "N pieces left" + % (gold accent) - from d2r_forgeSummary (fs.chron/grail/sets, same source as the ENGINES chips). Kept the forge-ready/one-step/DAILY-PICK/runs-today rows; a chronicle with no data omits its row (truthful). LIGHT (existing poll + forgeSummary). Did NOT touch ENGINE HEALTH / anti-lag / fault lamp / parked GHOST MODE / KAI READER / home layout. control_ui.html only (48/0). Also: full 5-agent audit swarm findings logged (AUDIT_FINDINGS_2026-07-22.md); engine fixes in flight. x3 parity
