...
VERSION = "v1317"   # 🎨 BIBLE VISUAL-LOCK typography pass 3 - card/panel HEADER family. Folded every card/panel/tile header, title, subtitle, name + label - 189 weight literals across 189 single-line CSS rules onto --fw-* (.boss-subtitle/.hero-title/.detail-name/.item-tile-name+rarity/.sec-h/.top-drops-title/.gic-section-title/.wishlist/.puv/.tzt/vault .vfr-vd/.tvb-tvn TV·D/the forge-tab flagship headers/.sc-card-h Sessions/.ars-grp-h/.g4-flags-h). Weights: 79 semibold(700)/60 medium(600)/30 bold(800)/12 regular(400)/8 normal(500). PROVABLY SAFE: count conserved (697 -> 508 raw + 225 tokenized; 508+225=733), every changed line a font-weight identity swap (0 non-font-weight changes on the diff - the mechanical guarantee makes 189 lines reviewable without eyeballing each), each token equals the literal -> byte-identical render, 16 inline scripts clean, scoped to single-line CSS rules matching the header family (multi-line + JS-string untouched). Nothing surfaced that wasnt a clean identity fold - no visual inconsistencies, no broken rules, no !important fights. Residual 508 raw weights (733->728->697->508), marching toward 0. bible.html only. x3 parity.
