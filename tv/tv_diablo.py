...
VERSION = "v1315"   # 🛡 SESSIONS FLAGSHIP D21 - VERDICT SEAL-STAMP POLISH (sessions-visual, Phase C). The dossier verdict seal (was a flat pill) is now an embossed round wax/broadcast STAMP - a 92px circle with a radial-gradient face + inset shadows for pressed depth, a double-ring (border + inner ring), the state icon over an uppercase label: 🛡 CLEAN RUN (gold) / 🚨 N FLAGGED (red) / ◌ UNSEALED (dashed muted). STAMP-IN on open: a press-down animation (enters big+rotated+faint, overshoots, settles ~500ms cubic-bezier), fresh-open only (same freshOpen guard as the D20 count-ups - no re-stamp on the 12s poll). REDUCED-MOTION handled declaratively via @media (prefers-reduced-motion:reduce){animation:none} - reliable CSS, settled instantly. Unified the D18 CARD seal chip with the same pressed-seal shadow language so the corner chip + the dossier stamp read as one system (mini chip on card, full stamp on dossier). Pure UI, no data touched, type system, no-scroll intact. Verified: 92px round embossed gold stamp, dss-in applied on open (border-radius 50% / 92px), screenshot confirms wax-seal look between stats + beats ribbon, JS 0 errors brace 0. The dossier now stacks D19 beats + D20 count-ups + D21 stamp - genuinely cinematic. control_ui.html only (31/10). x3 parity.
