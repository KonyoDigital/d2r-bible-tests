...
VERSION = "v1329"   # 📡 B4 DIABLO-LANGUAGE fully wired end-to-end - LIVE native on /api/status + SESSION_FIELD_CONTRACT doc. /api/status already carried live scene + area (from the agents beat) but no native label - the D24 banner had raw scene not the Diablo phrasing. Added native = _diablo_scene_label(live scene, live area) to status_payload(), reusing the beats scene/area; HONEST-ABSENT: null when off / no live scene/area (never "unclear" live). So the D24 "recording now" banner shows "🏛 ENTERING The Pit" / "⚔ FARMING · Chaos Sanctuary" LIVE, matching the retro routing rows. py_compile OK, 5/5 tests (transition->ENTERING, gameplay-in-farm->FARMING, gameplay-in-Harrogath->TOWN deterministic, st-area fallback, off->null honest; status_payload runs clean off-state native=null). + SESSION_FIELD_CONTRACT.md = the reference sessions-visual uses to surface B4/B8: both payloads with shapes + honest-absent rules (/api/sessions: finds/coverage/classFrames/superRecovery/missedFrames/sealMs/regretItems/sceneFingerprint/sceneReads/areas + each rows native; /api/status: scene/area/native/interest/model) + rendering rules (null=hide never fake 0, farmingPct=of-reads-not-wall-time, native.unclear/live-null=frame-didnt-say-dont-guess, grails-not-zone-pinned). DIABLO-LANGUAGE NOW END-TO-END: engine (B4 labels + B8 fingerprint) · exposed retro (sessions rows) + live (/api/status) · documented. Remaining = the deferred daytime reader-prompt round (dark-frame transition detection). control_app.py + SESSION_FIELD_CONTRACT.md. x3 parity.
