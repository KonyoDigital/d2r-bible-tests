...
VERSION = "v1279"   # 🔒 SESSIONS FLAGSHIP D7 - SEAL-LATENCY CHIP + REGRET SPOTLIGHT (polish-ui-2, UI) + D6 real-data RE-VERIFY. D7: a seal-latency chip in the seal row ("sealed in 3m12s", consumes sm.sealMs, omitted until the engine exposes it); plus a REGRET SPOTLIGHT - a prominent red-rail alert shown ONLY when regrets>0 ("you threw a KEEP - the judge later ruled N items worth keeping") with each thrown-keeper a jump-to-frame chip, degrading to the sm.regrets count when item detail is unavailable, and NOTHING at zero (honestly dormant - no current session has regrets>0). D6 RE-VERIFY on a throwaway :17980 vs the new engine (Konyos :17772 untouched): session 6 shows "recovered 2/3 missed reads - 67%" mint, drill lists the rescued Spirit + Shimmering Small Charm of Balance + one honest "unreadable text" - R/M% and labeled drill both populate. ENGINE FOLLOW-UP (v1280, team-lead): per-session sealMs:<int> (ms run-end->seal, from the report closedAt) + regretItems:[{name,frameId,ts}] (items this run threw that the judge later ruled KEEP) to light the chip + spotlight item chips (both already fall back gracefully). demo 7/7 (:17980), JS/brace/tests green, no page errors, D7 injected-data PASS. control_ui.html only (48/2). x3 parity.
