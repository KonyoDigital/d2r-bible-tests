...
VERSION = "v1367"   # 📋 SESSIONS R4 - honest-idle (Grok U3) + polish. Killed the "-" wall two places: (1) the KPI clusters no-runs state was six "-" gauges -> ONE honest resting line "no runs recorded yet - press ON to start your first hunt · your stats land here" (teaching, no fabricated zeros/gauges); (2) the stage readout (MODE OFF/READS 0/AREA -/SCENE off) read like a dead/broken panel off-air -> now DIMMED (opacity .6, at-rest not broken), snaps to full contrast on-air. Tightened the YOUR HUNT subtitle ("grail progress · what to hunt today" -> "what to hunt today", no edge overflow). DAILY MISSIONS zone reads clearly. Verified: no-data KPI -> resting line not 6 dashes; meters dim off-air/full on-air; demo J2/J5/J6 green, no-scroll, invariant GREEN, JS 0. control_ui only. x3.
