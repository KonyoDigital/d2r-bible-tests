...
VERSION = "v1254"   # 🗣🎮 DIABLO-LANGUAGE arc round 1 - CARRY THE SCENE THROUGH (additive). The AI read already produces a Diablo scene per frame (town|stash|inventory|loot|gameplay|transition) + stashTab + area, but the reconciler collapsed it to generic gameplay/stash/tooltip before the labels/session/retro - so a loading/portal frame surfaced as "gameplay / near black screen". Now _kai_reconcile indexes EVERY deep read that produced a scene (names or NOT - the portal frame reads scene=transition with empty names, the exact frame that was collapsing), a _nearest_scene(ts) helper (plus-minus 4s), and appends scene/tab/area to each returned dict; _kai_build_engine_frames carries them into the sealed kai_report EngineFrame; /api/session footage beats carry scene/tab/area (preferring the sealed reads true scene over the label-derived hint); _theatre_sessions adds sceneReads + tabReads beside the untouched kaiClasses. ADDITIVE + honest: owner/verdict/why byte-for-byte UNCHANGED; no nearby read = scene/tab/area stay None (never invented). Verified vs the GEMS test case (session s_1784736270319): the 4 frames that were "gameplay/near black screen" now carry scene=transition, town frames carry area (Rogue Encampment), stash frames carry their read tab (personal/shared - the gems-tab-detection MISS is R2, R1 faithfully transports whatever the read said). Floor 507 (503 + 4 new). control_app.py + tests (NOT control_ui.html). DIABLO-LANGUAGE 1. x3 parity
