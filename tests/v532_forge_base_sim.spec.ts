...
test('white bases produce NO upgrade tasks — they cannot be cube-upgraded (v534)', async ({ page }) => {
  // Seed every white upgrade-source base at once. The old (wrong) build surfaced ~84 "upgrade" cards; the
  // truth (game-file cubemain.txt) is you CAN'T cube-upgrade a white base → the whole bucket must be empty.
