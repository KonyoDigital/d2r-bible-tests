...
test('Chronicle seeds 49 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.madeCount).toBe(49);   // v578 +Black +Famine
  ...
  expect(r.prog).toContain('49 / 100');         // 49% created (v578)
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 49 → 50
