...
test('Chronicle seeds 45 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.prog).toContain('45 / 100');         // 45% created, matches the Chronicle
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 45 → 46
