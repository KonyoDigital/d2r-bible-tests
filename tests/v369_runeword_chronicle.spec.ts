...
test('Chronicle seeds 82 created, catalog is 99, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.madeCount).toBe(82);   // v653: +Eternity (his 5os Flail forge)
  ...
  expect(r.prog).toContain('82 / 99');          // v653
