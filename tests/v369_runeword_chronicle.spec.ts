...
test('Chronicle seeds 72 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.madeCount).toBe(72);   // v631.1 +Mist +Brand (Jul 9)
  ...
  expect(r.prog).toContain('72 / 100');         // v631.1 +Mist +Brand
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 72 → 73
