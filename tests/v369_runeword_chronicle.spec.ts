...
test('Chronicle seeds 81 created, catalog is 99 (Hustle removed — v651), lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.madeCount).toBe(81);   // v651: 81 seeded; the catalog itself is 99 now
  ...
  expect(r.prog).toContain('81 / 99');          // v651
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 81 → 82
