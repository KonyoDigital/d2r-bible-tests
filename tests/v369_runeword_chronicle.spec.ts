...
// First run seeds Konyo's in-game Chronicle. v424: the seed is a DURABLE FLOOR of 28 created runewords.

const URL = 'file://' + process.cwd() + '/bible.html';

test('Chronicle seeds 28 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  page.on('pageerror', (e) => errs.push(e.message));
  await page.addInitScript(() => {
    // enough runes to make Spirit (Tal+Thul+Ort+Amn) now; leave d2r_rwMade unset → seeds the 28-floor
  ...
  expect(errs).toEqual([]);
  // seeded the 28-runeword durable floor (Konyo's actual created list)
  expect(r.madeCount).toBe(28);
  ...
  expect(r.prog).toContain('28 / 100');         // 28% created, matches the Chronicle
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 28 → 29
