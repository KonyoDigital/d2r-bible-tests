...
test('Chronicle seeds 55 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.madeCount).toBe(55);   // v593 +Flickering Flame, v594 +Coven
  ...
  expect(r.prog).toContain('55 / 100');         // 55% created (v594 +Coven)
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 55 → 56
