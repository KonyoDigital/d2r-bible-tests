...
test('Chronicle seeds 47 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.madeCount).toBe(47);   // v568 — +White +Obedience
  ...
  expect(r.prog).toContain('47 / 100');         // 47% created, matches the Chronicle (v568)
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 47 → 48
