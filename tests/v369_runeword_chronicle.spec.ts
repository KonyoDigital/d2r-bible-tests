...
test('Chronicle seeds 40 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(errs).toEqual([]);
  // seeded the 40-runeword durable floor (Konyo's actual created list, + Zephyr + Memory as of v456/v457)
  expect(r.madeCount).toBe(40);
  ['Beast', 'Chains of Honor', 'Death', 'Mosaic', 'Edge', 'Lore', 'Pride', 'Destruction', "Ancients' Pledge", 'Spirit', 'Grief', 'Stone', 'Enigma', 'Doom', 'Plague', 'Treachery', 'Zephyr', 'Memory'].forEach((n) =>
    expect(r.madeKeys).toContain(n));
  expect(r.rowCount).toBe(100);                 // all 100 runewords listed
  expect(r.prog).toContain('40 / 100');         // 40% created, matches the Chronicle
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 40 → 41
