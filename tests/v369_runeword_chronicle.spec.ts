...
test('Chronicle seeds 44 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.prog).toContain('44 / 100');         // 42% created, matches the Chronicle
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 44 → 45
