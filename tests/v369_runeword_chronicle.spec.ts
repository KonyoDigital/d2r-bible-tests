...
test('Chronicle seeds 74 created, lists all 100, syncs makeable, toggles persist', async ({ page }) => {
  const errs: string[] = [];
  ...
  expect(r.madeCount).toBe(74);   // v631.1 +Mist +Brand +Wisdom +Phoenix (Jul 9)
  ...
  expect(r.prog).toContain('74 / 100');         // v631.1 +Mist +Brand +Wisdom +Phoenix
  ...
  // toggling an UN-seeded runeword to created persists and bumps the count 74 → 75
