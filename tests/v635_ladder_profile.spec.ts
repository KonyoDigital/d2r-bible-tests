...
test('BLEED-PROOF: forge/tally/create in the LADDER profile — the main profile stays byte-identical (and vice versa)', async ({ page }) => {
  ...
  const ladder = await page.evaluate(() => {
    ...
    // live a whole ladder life: own a 3os helm, tally Metamorphosis' runes, forge it
    // (v650 — the old journey word Mania is seeded MADE now; Metamorphosis is the last ladder word)
    w.LSR.setItem('d2r_owned', JSON.stringify(['Great Helm (3os)']));
    w.LSR.setItem('d2r_runeStash', JSON.stringify({ Io: 1, Cham: 1, Fal: 1 }));
    return cleanStart;
  });
  await page.reload(); await page.waitForTimeout(1800);
  const ladderLife = await page.evaluate(() => {
    ...
    const mania = [].concat(sc.now || [], sc.pipeline || [], sc.onestep || []).find((t: any) => t.rw === 'Metamorphosis');
    const laddered = (sc.ladder || []).length;                       // ladder profile: NO locked strip
    w.rwToggleMade('Metamorphosis', mania && mania.base && mania.base.name); // ✓ created — the REAL engine
    const made = Object.keys(JSON.parse(w.LSR.getItem('d2r_rwMade') || '{}'));
    return { maniaKind: mania && mania.kind, maniaBase: mania && mania.base && mania.base.base, laddered, madeN: made.length, hasMania: made.includes('Metamorphosis') };
  });
  ...
  const ladderPersist = await page.evaluate(() => {
    const w: any = window;
    // creating Metamorphosis CONSUMED the Great Helm (the real engine ran) — assert the consume trail
    const used = JSON.parse(w.LSR.getItem('d2r_rwBaseUsed') || '{}');
    const made = Object.keys(JSON.parse(w.LSR.getItem('d2r_rwMade') || '{}'));
    return { madeN: made.length, hasMania: made.includes('Metamorphosis'), owned: JSON.parse(w.LSR.getItem('d2r_owned') || '[]'),
             consumed: used['Metamorphosis'] && used['Metamorphosis'].l };
  });
  ...
  expect(ladder.made).toBe(82);           // v650 — 81 seeded + the Hustle auto-seal, one shared grail ledger
  ...
  expect(ladderPersist.madeN).toBe(83);                       // 82 boot (incl. Hustle auto-seal) + Metamorphosis forged in the journey
