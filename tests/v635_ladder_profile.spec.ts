...
test('BLEED-PROOF: forge/tally/create in the LADDER profile — the main profile stays byte-identical (and vice versa)', async ({ page }) => {
  ...
  const ladder = await page.evaluate(() => {
    ...
    // live a whole ladder life: own a claw, tally Mania's runes, forge it
    w.LSR.setItem('d2r_owned', JSON.stringify(['Blade Talons (3os)']));
    w.LSR.setItem('d2r_runeStash', JSON.stringify({ Shael: 1, Ko: 1, Eld: 1 }));
    return cleanStart;
  });
  await page.reload(); await page.waitForTimeout(1800);
  const ladderLife = await page.evaluate(() => {
    ...
    const mania = [].concat(sc.now || [], sc.pipeline || [], sc.onestep || []).find((t: any) => t.rw === 'Mania');
...
test('main profile is untouched-by-default: no L· keys exist until ladder is ever used; seed stays 75', async ({ page }) => {
  await page.goto(URL); await page.waitForTimeout(1800);
