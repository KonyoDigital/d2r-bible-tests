...
// only NON-seeded un-marks stick. So a stuck-at-19 / reset / restored save always snaps back to the full 49.
test('a stuck-at-19 save snaps up to 49 on load', async ({ page }) => {
  ...
  expect(r.count).toBe(49);   // v578 added Black + Famine to the floor
...
test('floor re-applies even after the stale one-time flag was set (the real stuck case)', async ({ page }) => {
  ...
  expect(n).toBe(49);   // durable floor ignores the flag
});
test('un-marks: a seeded forged RW snaps back (purged), a non-seeded one stays un-marked', async ({ page }) => {
  ...
  expect(r.count).toBe(49);           // exactly the 49 forged seeds
});
