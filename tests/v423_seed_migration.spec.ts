...
// v432/v436/v448 — the seeded runewords (now 31, incl. Ancients' Pledge) are a HARD FLOOR re-applied every
// load. Seeded RWs are FORGED FACT: their un-marks are purged on load (they snap back); only NON-seeded
// un-marks stick. So a stuck-at-19 / reset / restored save always snaps back to the full 31.
test('a stuck-at-19 save snaps up to 31 on load', async ({ page }) => {
...
test('floor re-applies even after the stale one-time flag was set (the real stuck case)', async ({ page }) => {
  ...
  expect(n).toBe(31);   // durable floor ignores the flag
});
test('un-marks: a seeded forged RW snaps back (purged), a non-seeded one stays un-marked', async ({ page }) => {
  ...
  expect(r.count).toBe(31);           // exactly the 31 forged seeds
});
