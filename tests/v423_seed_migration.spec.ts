...
// v432/v436/v448/v451 — the seeded runewords (now 34, incl. Spirit/Grief/Stone) are a HARD FLOOR re-applied
// every load. Seeded RWs are FORGED FACT: their un-marks are purged on load (they snap back); only NON-seeded
// un-marks stick. So a stuck-at-19 / reset / restored save always snaps back to the full 34.
test('a stuck-at-19 save snaps up to 34 on load', async ({ page }) => {
...
test('floor re-applies even after the stale one-time flag was set (the real stuck case)', async ({ page }) => {
  ...
  expect(n).toBe(34);   // durable floor ignores the flag
});
test('un-marks: a seeded forged RW snaps back (purged), a non-seeded one stays un-marked', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    // and purge its stale un-mark, while the NON-seeded Enigma stays un-marked (not floored).
    localStorage.setItem('d2r_rwUnmade', JSON.stringify({ 'Death': 1, 'Enigma': 1 }));
  ...
  const r = await page.evaluate(() => {
    ...
    return { hasDeath: !!s['Death'], hasEnigma: !!s['Enigma'], count: Object.keys(s).length, unmadeDeath: !!u['Death'], unmadeEnigma: !!u['Enigma'] };
  });
  expect(r.hasDeath).toBe(true);      // seeded → re-floored (forged fact)
  expect(r.unmadeDeath).toBe(false);  // its stale un-mark purged
  expect(r.hasEnigma).toBe(false);    // non-seeded → un-mark respected, NOT floored
  expect(r.unmadeEnigma).toBe(true);  // non-seeded un-mark preserved across the reload
  expect(r.count).toBe(34);           // exactly the 34 forged seeds
});
