...
// only NON-seeded un-marks stick. So a stuck-at-19 / reset / restored save always snaps back to the full 55.
test('a stuck-at-19 save snaps up to 55 on load', async ({ page }) => {
  ...
  expect(r.count).toBe(55);   // v593 +Flickering Flame, v594 +Coven
...
test('floor re-applies even after the stale one-time flag was set (the real stuck case)', async ({ page }) => {
  ...
  expect(n).toBe(55);   // durable floor ignores the flag
});
test('un-marks: a seeded forged RW snaps back (purged), a non-seeded one stays un-marked', async ({ page }) => {
  ...
  expect(r.count).toBe(55);           // exactly the 55 forged seeds
});
