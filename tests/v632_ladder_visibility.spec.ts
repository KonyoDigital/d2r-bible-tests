...
test('lifecycle demo to the ENDPOINT (his real Death Mask): Radiance pipeline → ✓ created → Chronicle 76, task gone, invariant still holds', async ({ page }) => {
  await liveState(page);
  const r = await page.evaluate(() => {
    const w: any = window;
  ...
  expect(r.madeCount).toBe(76);                             // 75 + Radiance
