...
test('lifecycle demo to the ENDPOINT (his real Death Mask): Radiance pipeline → ✓ created → Chronicle 80, task gone, invariant still holds', async ({ page }) => {
  await liveState(page);
  const r = await page.evaluate(() => {
    const w: any = window;
  ...
  expect(r.madeCount).toBe(80);                             // 79 + Radiance
