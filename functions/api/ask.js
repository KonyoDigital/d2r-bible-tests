...
export async function onRequestPost(context) {
  ...
  const sysText =
    ...
    + 'FORGE is the headline tool: it reads the player\'s Runes · Gems · Vault · Chronicle and hands them a ranked "do this now" plan with a Chronicle progress meter (N/100 forged). It has a "👉 Do this one thing" hero (the single highest-value next move) and sub-tabs: Make now (forge it — base + sockets + runes all in hand) · One step (go GET the right base, or cube missing runes up) · Pipeline (Larzuk-socket the base you own → then forge) · Crafts · Completed. Unlike the old rune-only "can make now", the Forge knows OWNED BASES, Larzuk-guaranteed-max vs cube-socket-gamble, 1H-player-vs-2H-mercenary bases, and ladder gating. The snapshot carries snapshot.forge (makeNow/pipeline/oneStep) + snapshot.chronicle (made/total) — when the user asks "what should I make / do next", ANSWER FROM snapshot.forge, not the rune-only runewords list. '
    ...
    + 'Rune Stash & Gem Stash & Cube-Up planners (📸 one-photo tally intake → what you can cube up); the CHRONICLE (100-runeword progress tracker — ✓ each one made; a "🔄 Reset to a fresh Chronicle" button lets a NEW player on this browser start their own from zero); 🧠 Smart Insights (Progress · Farm-priority: bases ranked by how many unmade runewords each unlocks + WHERE to farm them · Rune radar: runes you\'re short on, with cube-up); the 🔬 AI Item Checker (drop a MAGIC or RARE item → a keep-or-toss verdict); Horadric Cube recipe browser; Crafted Items Workshop (4 crafts × 9 slots, live cubeable from your tallies); All Runewords + Best Runeword Bases; High-Value Finds. '
