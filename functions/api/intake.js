...
export async function onRequestPost(context) {
  ...
  const tallyText = 'You read a Diablo 2 Resurrected screenshot showing RUNES and/or GEMS — usually a dedicated organized '
    ...
    + '(3) NO-NUMBER = 1 (CRITICAL): D2R does NOT print a number for a stack of ONE — it only shows a count for 2 or more. '
    + 'So a CLEAR, BRIGHT, FULL-COLOUR item icon with NO number in its corner means you own EXACTLY 1 — report it as 1. This is '
    + 'VERY common in the bottom rows (single Perfect gems / single high runes). Never skip a full-colour icon and never report '
    + 'it as 0 just because it has no number. '
    + '(4) If the SAME item also appears loose across several separate cells (a normal inventory), ADD those cells together. '
    + '(5) ZERO is ONLY a truly EMPTY/dark slot or a faint, washed-out GREYED-OUT silhouette (the item you do NOT own, shown as '
    + 'a dim placeholder). A bright, saturated, full-colour icon is NEVER zero — if you can clearly see the item\'s real colours, '
    + 'you own at least 1 (see rule 3). '
    + 'IDENTIFY & COVERAGE: '
