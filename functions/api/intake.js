...
export async function onRequestPost(context) {
  ...
  const itemsText = 'You read Diablo 2 Resurrected screenshots (stash/inventory panels, ground loot, hover tooltips). '
    ...
    + '(2b) SOCKETED-BASE EXCEPTION — a base item WITH SOCKETS is reportable as a GENERIC slot entry. If an item '
    + 'is a white/grey, magic, or rare BASE that has sockets (the tooltip shows "Socketed (N)" or "Sockets (N)", or '
    + 'the listing title says "N Sockets <base>" / "Nos"), report the matching generic vocabulary entry for its SLOT: '
    + '"Socketed Body Armor" (any chest/plate/mail/armor), "Socketed Helm" (any helm/cap/crown/mask/circlet/diadem), '
    + '"Socketed Shield" (any shield), "Socketed Weapon" (any weapon), "Socketed Gloves", "Socketed Belt", or '
    + '"Socketed Boots". Map the base to its slot; do NOT report the specific base name. A base with ZERO sockets is '
    + 'still non-reportable (rule 2). '
