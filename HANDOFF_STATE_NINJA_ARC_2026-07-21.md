...
## ✅ GATED: v1219 (47/80) — 085ec07 — ER arc round 10 (HALFWAY) THE LIVE READ FEED (on-air mint panel: reads flow live w/ frame thumb+owner+verdict+why; symmetric to retro scorecard; uses liveRing). North star now complete BOTH live+retro. Console @ v1219. ER 10/20. Rounds 11-20 = polish/cohesion. Round 11 → v1220.
...
## ✅ GATED: v1224 (52/80) — 06bf67b — ER arc round 15 GATE WHY plain-language (quorum<2→'only one source agreed…', proven→'N sources · conf X'; _GATE_WHY_LONG for drill-down only). Console @ v1224. ER 15/20. Round 16 → v1225 (cohesion/polish, 5 rounds left).
## ✅ GATED: v1225 (53/80) — c5fda38 — ER arc round 16 SIGNAL-FORWARD drill-down (empty '—' rows recede 40% so fired rows lead the eye). Console @ v1225 (NOT restarted — Konyo scanning, supervisor paused). ER 16/20. Round 17 → v1226 (onboarding/cohesion, 4 left).
## 💡 CONSOLE AUTO-TAKEOVER offered to Konyo (2026-07-22): he hit 'already running' opening TV DIABLO.app because the supervisor holds :17772 headless. Offered permanent fix: control_app.py --open path auto-takes-over the port (set pause-flag + quit headless + bind), and app-quit clears the flag so the immortal console returns. Awaiting his go — queue as an upcoming round if yes. Immediate: freed port via pause-flag+pkill.
## ✅ GATED: v1226 (54/80) — bb2b5ec — ER arc round 17 FIRST-OPEN ONBOARDING (one-time '3 moves' orientation strip, localStorage tvd_er_onboarded, bulletproof-wrapped). Console NOT restarted (Konyo scanning). ER 17/20. Round 18 → v1227 (final cohesion/consistency, 3 left).
## ✅ GATED: v1227 (55/80) — d7cb9fe — ER arc round 18 NUMBER-CONSISTENCY (flag-bar counts each kind independently; scorecard retro-caught 5 == flag-bar 5, was 5 vs 3). Console untouched (scanning). ER 18/20. Round 19 → v1228 (cohesion, 2 left).
## ✅ GATED: v1228 (56/80) — deb5dcb — ER arc round 19 FRAME-STEP SMOOTHNESS (img box reserved via aspect-ratio + adjacent-frame preload → holding ◀▶ flicker-free, 0ms cached). Console untouched (scanning). ER 19/20 → FINALE round 20 in flight → v1229.

## ✅🎬 GATED: v1229 (57/80) — HEAD 0af4d40 — ENGINE ROOM ARC ROUND 20 FINALE — ★ 20-ROUND ARC COMPLETE
polish-ui-2 (control_ui.html): cohesion finale — 0 orphan font-sizes (R1-R5 held); unified panel-eyebrow labels (800/.12em/--fs-2xs/uppercase, semantic colors kept) + 4 info-strips (radius 10px, padding 11·14). One designed instrument. ×3 parity, floor control 125 · agent 198, demo+smoke+deploy green. Console untouched (Konyo scanning).
...
## ✅👻 GATED: v1230 — HEAD 0b03a23 — GHOST MODE arc round 1 (the switch + ghost-shell container)
polish-ui-2 (control_ui.html): 👻 GHOST MODE switch in header (ONE debugger switch). ON → console recedes (perspective .9) + grayscale/dim into starfield "past" + Time Machine scrubber (79 real ticks from /api/sessions, playhead NOW) + banner + exit. OFF (switch/exit/Esc/click-starfield) → ZERO TRACE (proven: body '', .shell transform/filter none, outerHTML byte-identical). No rewind wiring yet (truthful). Pure add-on, ER untouched. ×3 parity, floor control 125 · agent 198, demo 7/7, smoke+deploy. Console untouched (Konyo scanning). GHOST MODE 1/50+. Restore point restore-v1229-preghost (local+remote).
polish-ui-2 flagged the spec-conflict (unified ER-absorption vs its "keep ER separate" dispatch) + did NOT rip the ER button unilaterally — gated as-is (option A). NEXT: round 2 = ER ABSORPTION (remove ER live-console entry → clean live console; ER tools reachable INSIDE GHOST MODE; one debugger) → v1231. Then truthful rewind wiring (scan/grail-by-date/forge; approx tallies/vault).

## ✅👻 GATED: v1231 — HEAD 19db240 — GHOST MODE round 2 UNIFIED (one switch, ER absorbed)
Separate ER button REMOVED from live console; one 👻 ghost-switch (_ghostToggle) = sole debugger entry → flip on = console recedes + ER debugger opens on top; flip off = zero-trace. 28ins/80del on v1230. ×3 parity, floor 125/198, demo 7/7, zero-trace proven. Console untouched (scanning). GHOST MODE 2/50+.
## CHANGELOG LESSON (2×): when bumping bible.html D2R_BUILD, REPLACE THE WHOLE note cleanly — do NOT append old_note/OLDNOTE (leaves the prior note dangling; validates but messy; cleaned twice via python). Match the full old note line and replace entirely.
## GHOST MODE next: round 3 = begin the TRUTHFUL Time Machine rewind on the ghost scrubber (scan replay largely exists via absorbed ER retro; GRAIL-by-date is the fresh win — real timestamps in d2r_foundLog; then forge; tallies/vault labelled approx). → v1232.

## ✅👻⏪ GATED: v1232 — HEAD 0c54bfb (deployed 0aacbd9f) — GHOST MODE round 3: TRUTHFUL REWIND BEGINS (grail-by-date) + live overlap fix
polish-ui-2 (control_ui.html, 65ins/9del): (1) GRAIL-BY-DATE — the first truthful rewind. Scrub the Time Machine into the past → purple strip "⏪ THE GRAIL · as of <date> · N/403 uniques found by this moment · from your found-log · real & exact for the found-once-kept case". N = count of d2r_foundLog entries stamped ≤ cursor ts (parsed in-iframe: Date.parse after " · "→" " normalize; _flog* not exposed on iframe window). Verified monotonic 239 by Jun1 → 351 by Aug1. Retro-only (hidden at NOW), absent if no found-log (never faked). Truthful per matrix. (2) LIVE BUG Konyo caught on v1231 FIXED: 👻 switch overlapped 🧰 Tools tab + bled into clock at narrow widths — root cause = header overflow (6-tab rail forced full-size by late !important never shrank → rightmost tabs bled under head-right). Now: switch has OWN divider-separated slot (compact icon <1360px, full label roomy) + tab rail compacts/scrolls in own lane (higher-specificity beats old !important). NO overlap 1000–2560px (30–103px gaps verified). ×3 parity v1232, floor 323 (agent 198 + control 125), demo skipped (console down/scanning), smoke 64 GREEN, deployed live. Zero-trace intact. Console NOT restarted (Konyo scanning, pause-flag set). GHOST MODE 3/50+.
## GHOST MODE next: round 4 (→v1233) DISPATCHED to polish-ui-2 = EXTEND SCRUBBER across full recorded range (earliest found-log stamp → now, so grail journey is dramatic across months) [recommended first] and/or FORGE layer (derives from stash → truthful iff tallies rewind; tallies approx-only → label honest or gate behind tally-snapshot decision). Same ironclad rules + gate pattern.

## ✅👻⏪ GATED: v1233 — HEAD 5d1ac99 (deployed 8f948b1c) — GHOST MODE round 4: THE GRAIL JOURNEY (full-range months-scale rewind axis)
polish-ui-2 (control_ui.html, 74ins/0del): a DEDICATED draggable Time Machine axis spanning the whole grail journey (earliest found-log stamp → now), seated below the frame timeline. Two truthful scales kept SEPARATE (frame scrubber = seconds/reels for engine debug; journey axis = months for the grail arc) — extending the reel-proportional frame axis to months would've shrunk reels to slivers + wrecked the 20-round frame-debug UX, so a dedicated axis was the clean call. All 351 ticks = real finds at real dates (finding pace visible; dense clusters = grind sessions). Drag → grail climbs monotonically from real data: 29 by May15 → 299 by Jun16 → 351 today; readout "N/403 uniques found by <date>". r3 frame-scrub grail-at-T coexists (verified). Truthful per matrix (real ticks/counts, no fabricated points, absent if no found-log; only caveat = un-tick edge, labeled). ×3 parity v1233, floor 323, smoke 64 GREEN, deployed live. Zero-trace intact. Console NOT restarted (scanning, pause-flag). GHOST MODE 4/50+.
## GHOST MODE next: round 5 (→v1234) DISPATCHED = COHESION PASS — tie the two grail views (r3 frame-scrub grail-at-T + r4 months journey) into ONE coherent Time Machine cockpit: per-axis labels ("FRAMES · this session" vs "GRAIL · your journey"), consistent marker/readout styling, legible relationship (or clean decouple), tighter ghost-shell framing. THEN r6 = Forge layer (approx, derived-from-stash, labeled honest). Cohesion-before-more-features.

## ✅👻⏪ GATED: v1234 — HEAD 9b400a0 (deployed ffb6f01b) — GHOST MODE round 5: COHESION / Time Machine unification
polish-ui-2 (control_ui.html, 7ins/26del NET SIMPLER): after r3+r4 there were TWO stacked grail readouts (frame-scrub grail-at-T + months journey), both N/403 = redundant. Unified into ONE Time Machine cockpit, two scales: (1) removed the redundant r3 grail strip (element+builder+CSS) → GRAIL JOURNEY is the single grail display; (2) LINKED scales — scrubbing THE FILM (frame axis, this session) drives the JOURNEY marker to that instant (verified 100%→97.5%), dragging journey directly still sweeps 29→351; (3) one visual language — both axes same accent-band + scale label: "⏱ THE FILM · this session, frame by frame" (secs/reels) vs "🏆 THE JOURNEY · your grail across all time — drag the marker, or scrub the film above" (months). Reads as one instrument, two scales. ×3 parity v1234, floor 323, smoke 64 GREEN, deployed. Zero-trace intact. Console NOT restarted (scanning). GHOST MODE 5/50+.
## GHOST MODE next: round 6 (→v1235) DISPATCHED = THE FORGE LAYER. Truthful-matrix: Forge "make now" derives purely from stash (no store) → rewinds truthfully IFF tallies rewind, but tallies are APPROX-only (manual +1/-1 & clears unjournaled). STEER = OPTION A (honest): show Forge craftability as-of-NOW while scrubbing, labeled "🔨 FORGE · current craftability (stash isn't time-stamped — shown as of now, not rewound)". A rewound Forge past would LIE (violates rule #3). Option B (intake-only ts'd slice) only if cleanly gated to intake-only sessions — don't force.

## ✅👻🔨 GATED: v1235 — HEAD df6aedd (deployed 6d9c5d4f) — GHOST MODE round 6: THE FORGE LAYER (Option A, truthful current-state)
polish-ui-2 (control_ui.html, 38ins/0del): d2r_forgeSummary is a snapshot derived from the un-timestamped stash → genuinely CANNOT rewind (no per-moment history); a rewound Forge would lie. So OPTION A (honest current-state): a "🔨 THE FORGE · what you can make right now" strip with an amber "CURRENT · NOT REWOUND" badge (visually distinct from the rewindable purple FILM/JOURNEY) — N craftable now · M one-step-out (+ item names, honest empty state) + note "the stash isn't time-stamped so THE FORGE can't rewind — craftability AS OF NOW, real & current, not a rewound past". STAYS PUT when scrubbing film/journey (verified before==after) — never fakes a rewound past. Three layers now: FILM (rewind) · JOURNEY (rewind) · FORGE (current, honest). Option B (partial intake-only ts'd rewind) flagged NOT built — needs Konyo go on additive record-path snapshot (rule #1). ×3 parity v1235, floor 323, smoke GREEN, deployed. Zero-trace intact. Console NOT restarted (scanning). GHOST MODE 6/50+.
## GHOST MODE next: round 7 (→v1236) DISPATCHED = TALLIES + VAULT LAYER (same honest OPTION-A current-state, amber badge). Tallies (rune/gem/material) + vault approx-only (manual edits/clears/vault-removals unjournaled) → can't rewind → show current, labeled, stays-put. Completes matrix coverage (FILM/JOURNEY=rewind, FORGE/TALLIES+VAULT=current). Optional grouping: "CURRENT STATE (not rewound)" section vs rewindable "TIME MACHINE" section. THEN r8 = cohesion/visual pass on full cockpit.

## ✅👻🗃 GATED: v1236 — HEAD e1167b2 (deployed 3c70f218) — GHOST MODE round 7: TALLIES + VAULT layer (grouped CURRENT STATE, matrix complete)
polish-ui-2 (control_ui.html, 42ins/34del): tally/vault data lives in the board (bible iframe), read live read-only via _runeCount/_gemCount/_materialCount + owned.size. All current-only maps, no timeline → can't rewind. Folded FORGE(r6)+TALLIES+VAULT into ONE grouped "🗃 CURRENT STATE · not rewound" section (single amber badge, same language as r6): FORGE row (now/one-step), TALLIES row (runes/gems/materials live), VAULT row (items via owned.size), + honest note (stash/vault not ts'd → can't rewind, holdings as of now). Stays put on scrub (verified), unreachable rows don't render. MATRIX COVERAGE COMPLETE — two clean families: TIME MACHINE (rewind: FILM+JOURNEY) vs CURRENT STATE (not rewound: FORGE+TALLIES+VAULT). ×3 parity v1236, floor 323, smoke GREEN, deployed. Zero-trace intact. Console NOT restarted (scanning). GHOST MODE 7/50+.
## GHOST MODE next: round 8 (→v1237) DISPATCHED = COHESION/VISUAL PASS on full cockpit. Make all 5 layers ONE deliberate instrument: visual hierarchy (TIME MACHINE hero/primary, CURRENT STATE secondary companion — "top=scrub time, bottom=true now" in one glance), tighter ghost-shell framing (receded console/starfield/header/chrome = stepping into a time machine not a widget panel), unified spacing/type/badge/label/section-header consistency (r2-r7 were incremental → one designed product), truthful labels intact, zero data changes.

## ✅👻✨ GATED: v1237 — HEAD 011a354 (deployed b3d82f16) — GHOST MODE round 8: cohesion / visual pass (one instrument, two families)
polish-ui-2 (control_ui.html, 25ins/11del, presentation-only zero data changes): debugger now reads top→bottom as the concept. (1) STRUCTURAL: CURRENT STATE moved to the FOOT below the whole rewindable Time Machine. (2) FAMILY ACCENT = information: band left-accent encodes rewindability — purple (.er-fam-tm) on FILM+JOURNEY (rewindable), amber (.er-fam-now) on CURRENT STATE (not rewound), engine spine neutral gold (always-live loop). Read accent → know if scrubbing moves it. (3) HIERARCHY: CURRENT STATE recessed into secondary amber companion card → Time Machine is hero. (4) GHOST-SHELL: purple rim + violet halo + starfield .5→.62, all scoped body.ghost-on → zero-trace holds. ×3 parity v1237, floor 323, smoke GREEN, deployed. Zero-trace re-verified. Console NOT restarted (scanning). GHOST MODE 8/50+.
## GHOST MODE next: round 9 (→v1238) DISPATCHED = JOURNEY TICK STORYTELLING. Each of 351 journey ticks is a real find at a real date but anonymous → make them narrate: hover/land marker on a tick → show WHICH item(s) found that date from d2r_foundLog ("Jun 22 · Wormskull" / "…+N more"). Scrub = watch actual grails land, not just count climb. Truthful: only found-log data, honest gap days, un-tick caveat applies. Pure read + pure add-on.

## ✅👻🏆 GATED: v1238 — HEAD a626c0d (deployed 4bb2d852) — GHOST MODE round 9: journey tick storytelling
polish-ui-2 (control_ui.html, 62ins/11del, pure-read d2r_foundLog): THE JOURNEY narrates the actual hunt. Marker names the grail(s) at its date ("Jun 15 landed Blackhorn's Face +2 more" / "Jun 5 found Wolfhowl") in purple TM voice; busy days grouped newest-first up to 2 + "+N more" (real counts); hover preview tooltip names nearest find w/o moving committed marker (1px inspectable); honest gaps pre-first-find; un-tick caveat unchanged. Verified vs Konyo's ACTUAL found-log (hundreds narrated). ×3 parity v1238, floor 323, smoke GREEN, deployed. Zero-trace (tooltip only in hidden overlay, outerHTML byte-identical). Console NOT restarted (scanning). GHOST MODE 9/50+.

## ⚠️👻 KONYO REDIRECT (2026-07-22, mid-r10): GHOST MODE FACE IS WRONG — must be a ghost REPLICA of the console, not the ER cockpit
Konyo: "the ghost mode is suppose to be clicked on and then a SHELL ON TOP OF THE ORIGINAL SHELL console so visually it renders on top.. i don't see it like that, it's STILL LIKE THE ENGINE ROOM." GAP: flipping GHOST MODE opens the ER debugger (#tvd-eng) as the on-top face → reads as the old ER cockpit. Konyo's consistent vision = "ghost shells the ENTIRE console, a REPLICATED version of the console, a shell on top of the shell." ROUND 10 REDIRECTED (film storytelling ON HOLD) → v1239 = THE GHOST-SHELL REPLICA: on ghost-on, render a visual ghost CLONE of the .shell (cloneNode, namespaced IDs, translucent/desaturated + purple rim/halo/float) OVERLAYING the real shell (which recedes beneath). The ghosted console IS the face; TIME MACHINE scrubbers + CURRENT STATE + r9 narration frame/dock it; ER deep tools become a secondary drill-in. IRONCLAD: flip-off tears down the clone = ZERO TRACE (outerHTML byte-identical), pure add-on (clone display-only, no dup handlers on real console), truthful, control_ui.html only. Fallback if stage canvas/iframe won't clone: CSS-built ghost console frame mirroring the shell layout. THEN resume r10 film storytelling as a drill-in.

## ✅👻🖥 GATED: v1239 — HEAD 33df64f — GHOST MODE round 10: THE GHOST-SHELL REPLICA (a shell on top of the shell)
Konyo redirect fix. Flip ON → cloneNode(true) the .shell into #ghost-shell: whole console ghosted on top (desaturated/translucent, purple rim/halo/float, "viewing the past" ribbon), real shell recedes (scale .9, dim) behind + shows through starfield. Clone display-only (on-handlers stripped, IDs→data-gh-id, ghost-switch removed, inert/aria-hidden; 0 IDs/0 handlers/0 dup-id). Time Machine was a drill-in via "enter the time machine". ZERO-TRACE byte-identical (21197==21197). ×3 parity, floor 323, deployed. GHOST MODE 10/50+.
## ✅👻⏪ GATED: v1240 — HEAD (this) — GHOST MODE round 11: ONE-TOGGLE Time Machine (scrubbers docked on replica)
Konyo: "enter the time machine defeats the purpose... one toggle, console turns into a time machine in place, no routing between tabs." KILLED the enter-button. Flip ON → ghosted replica floats WITH FILM+JOURNEY+CURRENT+r9 narration docked at foot, immediate, no 2nd tap. SAFE PATH: relocate live wired scrubber nodes (er-film/er-gj/er-reelcard/er-livefeed/er-current) into dock (moved not cloned), record each home (parent+nextSibling), restore on flip-off. Deep forensics behind optional "🔬 inspect this moment" (never required path, never routes). _engineRoomOpen→_engineRoomEnsure (build/wire/init w/o show); _engineRoomRefresh updates while ghost-on. ZERO-TRACE held byte-identical (21197==21197), scrubbers restored home, re-open repopulates cleanly. ×3 parity, floor 323. GHOST MODE 11/50+.
## GHOST MODE next: round 12 (→v1241) DISPATCHED = resume FILM STORYTELLING (scratchpad/r10_film_storytelling.patch) as the "🔬 inspect this moment" DEEP drill-in from the docked FILM scrubber (real frame image + read raw→resolved + owner/verdict, honest gaps). Docked scrubbers stay default (one-toggle); inspect = optional deep-dive. Reconcile patch vs v1240 refactor (_engineRoomEnsure/_engineRoomRefresh/relocated nodes) — rebuild fresh if patch conflicts. Truthful, zero-trace + one-toggle intact.

## ✅👻 v1242-v1244 (2026-07-22): film storytelling → auto-open → STRUCTURE pass
- v1242 (3945244): film storytelling as ON-DEMAND inline inspect ("THIS FRAME · what the AI saw & decided"), frame-side sibling of r9 journey narration. GHOST MODE 12.
- v1243 (981fe31): film storytelling AUTO-OPENS on scrub (symmetric w/ journey auto-narration since r9 = one instrument); inline (no routing), closes on snap-to-NOW. Team-lead call over on-demand for coherence; flagged Konyo w/ one-flip revert offer. GHOST MODE 12b.
- v1244 (this): STRUCTURE & CLARITY pass (Konyo "cool but looks messy"). Dock → 3 clean panels: THE TIME MACHINE (bordered purple panel, FILM+JOURNEY+narration, accents→hairlines), CURRENT STATE (amber side-card), INSPECT (own collapsing full-width zone, no longer shoves scrubbers). 16px gutters, calm frames. structure-only 54/21. r13 feel-alive preserved as r13_feel_alive.patch. ZERO-TRACE byte-identical (21197). GHOST MODE 13.
## GHOST MODE next: round 14 (→v1245) DISPATCHED = resume FEEL-ALIVE (r13_feel_alive.patch) on the clean 3-panel base: step-into transition (flip on/off, prefers-reduced-motion), ghost fidelity (KEEP tasteful, don't re-clutter), keyboard scrub ←/→ step + ▲/▼ coarse + active affordance + restore the "◀▶" hint. Reconcile vs v1244 panels; keep one-toggle+docked+inline+zero-trace.

## 🖥⚡ CONSOLE UPGRADE ARC (Konyo 2026-07-22: "full on upgrade konyo style for the console. dont focus anymore on the time machine")
GHOST MODE / Time Machine PARKED at v1246 (great state, untouched). New arc = the LIVE console cockpit. Full plan + audit-ranked opportunities in CONSOLE_UPGRADE_ARC.md. Theme: make the live ENGINES + READER NET visible/gorgeous. Ironclad: LIGHT (ride existing /api/status poll, no new timers/fetches — anti-lag sacred), TRUTHFUL (real data, eyes-pulse honesty, liveRing=LIVE GUESS since sealed wins retro), DON'T touch ENGINE HEALTH data model / anti-lag / fault lamp / parked GHOST MODE.
- ✅ v1247 (f8ca17b) — CONSOLE r1 THE LIVE PULSE (control_ui.html): live read feed (st.liveRing under eyes, LIVE GUESS labeled), capture lock-on pill (Locked/whole-screen/holding), read-landed stage pulse, live-eye thinking-pulse. FLAGGED: #2 reader-pool eyes need poolInFlight/poolN/poolWarm forwarded in control_app.py first.
- ✅ v1248 (47dbcde) — CONSOLE r2 THE TAKEOVER (control_app.py + tests, NOT UI). Konyo "the console is already open": double-click TV DIABLO.app → --open bind-fail on the headless supervisor console used to REFUSE (no window); now ATTACHES a window (window-only takeover) unless a real window exists (v781 refuse kept). Self-healing pid lock (.tvd_window.pid, gitignored). Guarded engine-driver/closer on not _WINDOW_ONLY (no double-driver; fixes latent --window-only bug). Launcher git-pulls so it lands on next double-click. Floor 325 (+3 tests). NEEDS KONYO DOUBLE-CLICK TEST.
## RESOLVED (not a version): ghost-mode "screenshots missing" = the CONSOLE SERVER WAS DOWN (:17772), so /hist/ frames couldn't serve — not a UI bug (v1246 scroll-fix was verified synthetic). Brought console up headless (v1247), verified real reel frame serves HTTP 200, cleared pause-flag. Konyo: ⌘⇧R the tab.
## CONSOLE next: backend forward poolInFlight/poolN/poolWarm in control_app.py health → then polish-ui-2 wires #2 reader-pool eye-dots. Then r3 = Agent Mind glow-up + accuracy-gate "🛡 KAI caught N misreads" badge + reads sparkline (audit #5/#6/#7).

- ✅ v1249 (dfe7757) — CONSOLE r3 DISCOVERABILITY (control_ui.html). Konyo saw v1247 → "where is it?" (root cause: live feed hidden off-air). Fix: (#1) feed ALWAYS present — at rest a labeled "KAI READ FEED · AT REST · press ON to wake the eyes" placeholder + faded last-session preview; on-air = live stream. (#2) reader-pool eye-dots "3 of 6 eyes reading · 2 warm" (poolInFlight/poolN/poolWarm, on-air; NO backend needed — data already forwarded). (#3) mint "🧠 THE KAI READER · live engine" section header (findable). (#4) capture pill legible at rest. Floor 325, both states verified. CONSOLE UPGRADE 3.
## CONSOLE serving model CONFIRMED: control_app.py serves control_ui.html FRESH from disk per-request (UI_PATH read each GET) — Konyo sees UI upgrades with a ⌘⇧R, NO console restart needed. (Live process backend may lag a version in-memory — cosmetic ver stamp; self-resolves on supervisor restart. UI = latest on disk.)
## CONSOLE next: HOLDING for Konyo's read of v1249 (findable now?). Then r4 = Agent Mind glow-up (#5) + accuracy-gate "🛡 KAI caught N misreads" badge (#6) + reads sparkline (#7). Also pending: v1248 takeover needs Konyo's double-click test.
